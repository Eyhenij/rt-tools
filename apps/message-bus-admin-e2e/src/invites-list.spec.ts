...
test.describe('раздел приглашений', () => {
    test('SC-MB-128 — список называет имя, состояние и сроки, а кода приглашения на экране нет', async ({ page }: { page: Page }) => {
        ...
        // Срок закрывается маской: стенд засевает его от текущего момента — «через двое суток», —
        // и в кадре он меняется от прогона к прогону по существу, а не оттого, что не успел
        // встать. Остальные даты раздела засеяны постоянными и маски не требуют.
        await expectScreen(page, 'list-invites', { mask: [qa(page, 'invites-cell-expires')] });
    });
