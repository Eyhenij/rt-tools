...
const storeDir = path.resolve(__dirname, './projects/store');
const packageJsonPath = path.join(storeDir, 'package.json');

// Read package.json
if (!fs.existsSync(packageJsonPath)) {
    console.error(`Error: package.json not found in ${storeDir}`);
    process.exit(1);
}
...
updateDependency('./projects/ui-kit/package.json', '@rt-tools/store', newVersion);
