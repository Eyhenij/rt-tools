...
describe('postmortemArrivalUpdate', () => {
    it('SC-MB-169 — приезд с другим текстом возвращает разбор в «новое»', () => {
        expect(postmortemArrivalUpdate({ text: TEXT, closedByPublisher: false }, 'исправленный текст')).toEqual({
    ...
    it('SC-MB-170 — приезд с тем же текстом состояния не называет вовсе', () => {
        expect(postmortemArrivalUpdate({ text: TEXT, closedByPublisher: false }, TEXT)).toEqual({ text: TEXT });
    });

    it('SC-MB-323 — приезд с другим текстом не возвращает в «новое» запись, закрытую издателем', () => {
        expect(postmortemArrivalUpdate({ text: TEXT, closedByPublisher: true }, 'исправленный текст')).toEqual({
            text: 'исправленный текст',
        });
    });
