 ...
 *
 * Набор повторяет тот, что принимает приёмник: поле не из него он отбивает отказом. Текста
 * предложения здесь нет — порядок по колонке, которой нет в строке списка, человеку нечем
 * объяснить.
 */
export const PROPOSALS_SORTABLE: readonly string[] = ['arrivedAt', 'resource', 'address', 'tree'];
