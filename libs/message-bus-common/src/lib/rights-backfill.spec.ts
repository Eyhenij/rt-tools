...
describe('перенос прежнего доступа на модель прав', () => {
    it('SC-MB-370 — роль владельца получает все права закрытого набора: переносом и дописками после него', () => {
    ...
    it('SC-MB-370 — право из набора не потеряно ни одно: каждое выдано хотя бы одной миграцией', () => {
        RIGHTS.forEach((right: TRight): void => {
            expect(ALL_MIGRATIONS.some((text: string): boolean => text.includes(`'${right}'`))).toBe(true);
        });
    });

    it('SC-MB-370 — роль разбора груза несёт только чтение и пометку груза', () => {
    ...
    it('SC-MB-370 — роль достаётся записям без роли и не трогает остальные', () => {
        expect(MIGRATION).toMatch(/UPDATE\s+"account"/);
        expect(MIGRATION).toMatch(/WHERE\s+"roleId"\s+IS\s+NULL/);
    });

    it('SC-MB-370 — служебная запись разбора получает роль раньше общей раздачи владельца', () => {
    ...
    it('SC-MB-370 — повторное применение не заводит вторую роль', () => {
