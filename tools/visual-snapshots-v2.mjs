...
// Повторов у прогона нет намеренно: повтор превращает мигающую историю в зелёную со второго
// раза, и правило про детерминированность кадра становится непроверяемым.
const runnerArgs = ['test-storybook', '--config-dir', CONFIG_DIR, '--url', URL];
