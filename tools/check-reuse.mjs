#!/usr/bin/env node
// rt-kit v0.23.0 · checks/check-reuse.mjs · d95564dc9708 · правится надстройкой, не здесь
/**
...
const problems = [
    // Готовое названо оговоркой намеренно: признак судит тег, а место показа — нет. Замена своего
    // абзаца отказа на готовый компонент того же экрана закрывает признак и оставляет нарушение
    // на месте — так починили тринадцать шаблонов сразу. Строка вывода — измерение, а не задание.
