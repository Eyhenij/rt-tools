 ...
 *
 * Тем же порядком судится и прямое обращение к ступени скругления, тени, толщины рамки и
 * длительности: между ступенью и местом употребления стоит своё свойство компонента. Правило
 * заведено в том же конфиге, находки ложатся в тот же список принятого — второго перечня
 * накопленного не заводится.
 *
...
console.log(`check-tokens-styles: литералов и прямых ступеней в наборе ${seen.size}, все приняты списком — новых нет`);
