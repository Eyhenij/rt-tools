...
try {
    ...
    if (early === late) {
        console.error(
            `\n  Проба отрисовки: кадр до ожидания и после совпали — оба ${early}.\n` +
                ...
                `  показывать надписи. Расхождение, которое это ожидание чинит, выпадает раз на полсотни\n` +
                `  прогонов — снимками его не поймать, поэтому проба стоит отдельно.\n`
        );
        process.exit(1);
    }

    console.log(`Проба отрисовки: кадр до ожидания ${early}, после — ${late}. Ожидание работает.`);
} finally {
    await browser.close();
}
