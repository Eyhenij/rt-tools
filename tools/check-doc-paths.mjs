#!/usr/bin/env node
// rt-kit v0.23.0 · checks/check-doc-paths.mjs · c93de736f809 · правится надстройкой, не здесь
/**
...
if (problems.length > 0) {
    ...
    // Ходов три, и третий назван вслух: список известного копит принятое, а не показания
    // сломанного прибора. Прежний текст звал в список всё спорное — то есть учил ровно тому
    // обходу, который закон о проверяемости рядом запрещает, и один разбор дал 51 ложный отказ
    // из 264, каждый на живой адрес.
    console.error(
        `\nХодов отсюда три: поправить устаревший адрес; внести имя в ${ALLOWLIST}, если документ` +
            `\nописывает ещё не созданное; починить саму проверку, если врёт она, — разбор отказов` +
            `\nпоимённо и показ разбора владельцу. Спорное в список не вносится.`
    );
}
