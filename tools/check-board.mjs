#!/usr/bin/env node
// rt-kit v0.23.0 · checks/check-board.github.mjs · bfbe0c3681bd · правится надстройкой, не здесь
/**
...
function checkHeadRun(pull, options) {
    ...
    // Заявка поверх соседней прогона не получает никогда: рабочий поток слушает заявки в главную
    // ветку, и события про другую базу он не видит вовсе. Строка о потерянном событии тут врёт
    // дважды — событие не терялось, а перезакрытие его не вернёт: совет исполняется буквально,
    // прогон не встаёт, и на второй попытке поломку начинают искать в хостинге.
