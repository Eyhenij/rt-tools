#!/usr/bin/env node
// rt-kit v0.23.0 · checks/check-prose-style.mjs · 0a3f7e739e94 · правится надстройкой, не здесь
/**
...
const MARKS = [
    ...
    // Судится она поэтому по тому, что стоит следом: местоимением она читается перед словом,
    // которое сама и определяет. Перечень корней короток нарочно — ловится то, что встречалось,
    // а ложный отказ здесь дороже пропуска: проверка стоит в наборе гейта пуша, и красное на
    // слове, которому нет замены, останавливает работу целиком.
    [
