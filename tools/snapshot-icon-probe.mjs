 ...
 *
 * Зачем она есть. Набор значков едет по сети — три сотни файлов, склеенных реестром в один
 * `<svg id="rt-icon-sprite">`, — и до его прихода `<use href="#…">` не рисует ничего. Кадр
 * выходит без значков и с поехавшей раскладкой ряда, а прогон при этом зелёный: эталон,
 * закреплённый таким кадром, дальше сходится сам с собой. Волновая пересъёмка закрепила так
 * четыре кадра, и нашла это повторная сверка, а не прогон.
 *
 * Как она судит. Набор придерживается на подходе, и одна и та же история снимается дважды: раз
 * без ожидания значков, раз с ним. Кадры обязаны разойтись — это и значит, что ожидание
 ...
 *
 * Каждый кадр снимается в своей свежей странице, а не два подряд в одной: набор придерживается
 * на подходе, и второй кадр в той же странице пришёл бы уже по приехавшему набору.
 *
...
/**
 * На сколько придерживается набор значков.
 *
 * Проба судит не скорость сети, а порядок: кадр без ожидания обязан уйти раньше набора. Задержка
