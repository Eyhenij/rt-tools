...
export class RtPaginationComponent {
    ...
    protected readonly rangeLabel: Signal<string> = rtKitLabel(
        'uiRangeOf',
        computed((): TRtKitLabelParams => ({ from: this.rangeFrom(), to: this.rangeTo(), total: this.pageModel().totalCount }))
    );

    protected readonly pageLabel: Signal<string> = rtKitLabel(
        'uiPageOf',
        computed((): TRtKitLabelParams => ({ page: this.pageModel().pageNumber, last: this.lastPage() }))
    );
