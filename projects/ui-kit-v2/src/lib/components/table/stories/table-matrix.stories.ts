 ...
 * место, а заданный отбор строк не сужает — набор уходит наружу целиком.
 *
 * На узком кадре строки отбора нет ни в одной ячейке: там таблица показывает карточки, столбцов
 * не остаётся вовсе, и ряд ячеек над списком карточек был бы обрывком другой разметки. Отбор на
 * узком показе — работа своя, она стоит открытым вопросом описания.
 */
