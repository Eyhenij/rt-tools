...
export namespace IRtPageHeader {
    /**
     * Чем панель второго уровня держится открытой.
     *
     * `hover` — панель живёт в оверлее: наведение открывает, уход закрывает.
     * `pinned` — панель стоит в потоке разметки и держится открытой, пока
     * потребитель не сменит моду. Своего предпочтения компонент не хранит:
     * мода приходит входом, нажатие переключателя уходит выходом.
     */
    export type PanelMode = 'hover' | 'pinned';
