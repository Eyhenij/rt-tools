...
/**
 * Сколько ждать значки. Они не шрифт: реестр забирает файл на каждое имя, которое попросила
 * разметка, и дописывает символ в `<svg id="rt-icon-sprite">` в начале `body`. Пока символ не
 * приехал, `<use href="#…">` не рисует ничего — кадр выходит без значков, и раскладка ряда
 * съезжает.
 */
const ICONS_TIMEOUT_MS: number = 30_000;

/**
 * Признак спрайта, в который реестр складывает пришедшие символы. Берётся там же, где его
 ...
 *
 * Значки едут по сети по одному — файл на каждое имя, которое попросила разметка, — и до прихода
 * символа `<use href="#…">` не рисует ничего: кадр выходит без значков, а раскладка ряда
 * съезжает. Волновая пересъёмка закрепила четырьмя такими кадрами именно это.
 *
 * Ждётся тот же предмет, о котором отказ: сам нарисованный значок, а не косвенный его признак.
 * Наличие спрайта в разметке им не является — спрайт дорастает по ходу, и первый пришедший символ
 * заводит его целиком, пока остальные ещё в пути.
 *
