...
export class BreakpointService {
    ...
    public readonly isDesktopSource: Signal<Nullable<boolean>> = toSignal<Nullable<boolean>>(
        this.#breakpointObserver.observe(this.#desktopQuery).pipe(map(({ matches }: BreakpointState) => matches))
    );

    public readonly isSmallDesktopSource: Signal<Nullable<boolean>> = toSignal<Nullable<boolean>>(
        this.#breakpointObserver.observe(this.#smallDesktopQuery).pipe(map(({ matches }: BreakpointState) => matches))
    );

    public readonly isMobileSource: Signal<Nullable<boolean>> = toSignal<Nullable<boolean>>(
        this.#breakpointObserver.observe(this.#mobileQuery).pipe(map(({ matches }: BreakpointState) => matches))
    );

    public readonly isTabletSource: Signal<Nullable<boolean>> = toSignal<Nullable<boolean>>(
        this.#breakpointObserver.observe(this.#tabletQuery).pipe(map(({ matches }: BreakpointState) => !matches))
    );

    public readonly isSmallTabletSource: Signal<Nullable<boolean>> = toSignal<Nullable<boolean>>(
