...
# --- SC-AK-1074…1076. Форма имени судится деревом, в котором идёт команда ---------------------
# Сессия работает в соседнем дереве по прямому слову владельца: задача заведена там, номер и
# краткое имя верны, а ключ задач у соседа свой. Форма, спрошенная у профиля дерева сессии,
# отбивала законное имя, и обхода у такого отказа не было — работа вставала целиком.
NEIGHBOUR_TREE="$(fixture_repo main)"
mkdir -p "$NEIGHBOUR_TREE/.claude/rt-kit"
cat > "$NEIGHBOUR_TREE/.claude/rt-kit/project.sh" <<'EOF'
rt_task_branch_ok() { printf '%s' "$1" | grep -qE '^RT-[0-9]+-[A-Za-z]+$'; }
EOF
BARE_TREE="$(fixture_repo main)"

d "SC-AK-1074 — имя по форме соседнего дерева проходит" \
    "cd $NEIGHBOUR_TREE && git checkout -b RT-9-GuestToken" PASS
d "SC-AK-1075 — то же имя в своём дереве отбивается как прежде" \
    'git checkout -b RT-9-GuestToken' deny
d "SC-AK-1076 — у дерева без профиля форма не судится вовсе" \
...
mg "SC-AK-195 — подстановка вместо причины обходом не считается" "$LYING" \
    'gh pr merge 42 --merge # Task-folder-skip: <reason>' deny
