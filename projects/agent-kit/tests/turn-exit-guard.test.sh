...
expect_stop "SC-AK-873 — работа до отдачи ход не отпускает" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(ran 'git commit -m проба')")")" BLOCK
expect_reason "SC-AK-873 — отказ называет обязательное действие состояния" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(reply)")")" 'прогнать набор'
...
expect_stop "SC-AK-873 — открытая заявка с начатой следующей работой ход отпускает" \
...
expect_stop "SC-AK-873 — разобранный ход без заявки тоже отбивается" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(ran 'git push')")")" BLOCK
expect_stop "SC-AK-873 — слово владельца об остановке отпускает и это состояние" \
...
state_is 'этап-идёт'
expect_stop "SC-AK-876 — вопрос прозой после отказа гарда разговора ход не отпускает" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(denied_ask)" "$(said_prose 'Какой стенд поднимать?')")")" BLOCK
expect_stop "SC-AK-876 — тот же ответ без отказа гарда разговора судится прежним признаком" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(edited)" "$(said_prose 'Какой стенд поднимать?')")")" PASS
expect_stop "SC-AK-876 — отказ гарда разговора без вопроса прозой ход отпускает" \
