...
case "$out" in
    *'длинное: 400 characters'*) report 'SC-AK-635 — описание длиннее предела названо поимённо' 'названо' 'названо' ;;
    *) report 'SC-AK-635 — описание длиннее предела названо поимённо' 'молчит' 'названо' ;;
esac
...
case "$out" in
    *'короткое'*) report 'SC-AK-636 — описание в пределе проверку не тревожит' 'названо' 'молчит' ;;
    *) report 'SC-AK-636 — описание в пределе проверку не тревожит' 'молчит' 'молчит' ;;
esac
...
case "$out" in
    *'безописания'*) report 'SC-AK-638 — скил без описания проверке не интересен' 'названо' 'молчит' ;;
    *) report 'SC-AK-638 — скил без описания проверке не интересен' 'молчит' 'молчит' ;;
esac
...
if [ "$code" -eq 0 ] && [ "${out#*debt длинное}" != "$out" ]; then
    report 'SC-AK-637 — принятый долг проверку не роняет' 'ноль и строка долга' 'ноль и строка долга'
else
    report 'SC-AK-637 — принятый долг проверку не роняет' "код $code" 'ноль и строка долга'
fi
