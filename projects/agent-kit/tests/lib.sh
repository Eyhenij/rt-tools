...
# Поля дерева снимаются с окружения набора все разом, а не по именам. Набор бывает запущен изнутри
# живого захода — гейт пуша идёт из-под самого диспетчера, — и тогда его гарды унаследовали бы и
# разобранный ввод той сессии, и профиль дерева: почту подписи, метку эпика, адреса стендов. Гард
# судил бы живой вызов вместо фикстуры. Снимается всё с приставкой профиля: набор ставит своё сам,
# и список по именам отставал бы от профиля на каждое новое поле.
for rt_name in $(env | sed -n 's/^\(RT_[A-Za-z0-9_]*\)=.*/\1/p'); do
    unset "$rt_name"
done
unset rt_name

# Адрес хранилища снимается с окружения по той же причине, и цена промаха здесь выше. Набор
# бывает запущен из-под крючка отправки, а тот выставляет `GIT_DIR` на настоящее дерево: вызовы
# фикстуры уходят в него мимо своего репозитория — заводят там ветки, двигают ссылки на удалённые
# и переставляют ветку, на которой идёт работа. Гард судит фикстуру, а не дерево, где его запустили.
unset GIT_DIR GIT_WORK_TREE GIT_INDEX_FILE GIT_OBJECT_DIRECTORY GIT_ALTERNATE_OBJECT_DIRECTORIES
unset GIT_COMMON_DIR GIT_NAMESPACE GIT_CEILING_DIRECTORIES GIT_PREFIX

# Корень дерева снимается по той же причине, и промах здесь тише всех. Гарды берут корень из
# `CLAUDE_PROJECT_DIR`, а не из положения вызова, и под живым заходом она указывает на настоящий
# репозиторий: гард читает его профиль вместо профиля фикстуры и судит настоящую очередь работ.
# Набор тогда краснеет по причине, которой в фикстуре нет вовсе, — и только изнутри захода: при
# прямом запуске переменной нет, и всё сходится.
...
. "$ASSETS/hooks/utf8.sh" 2>/dev/null || true
...
# Порт, который набор берёт у системы, а не назначает числом.
#
# Раннер у дерева свой и стоит на одной машине: рядом идёт второй прогон, и назначенный числом
# порт у него занят. Сервер падает с `EADDRINUSE`, а вместе с ним восемь проверок — по причине,
# которой в правке нет вовсе. Отчёт при этом называет провалившиеся проверки, а настоящая
# причина лежит строкой ниже, в чужом наборе, и ветка ждёт, пока кто-нибудь позовёт перезапуск
# руками.
#
# Приём один на оба набора груза: сервер слушает нулевой порт, система выдаёт свободный, и
# выбранный номер уезжает в файл. Оболочка ждёт этого файла и читает номер из него — угадать его
# нечем, а разойдясь, обе стороны молчат: сервер поднят, а стучатся не туда.
#
# Ждём до пяти секунд: дольше означает, что сервер не поднялся вовсе, и ждать его нечего.
wait_for_port() {
    _file="$1"
    _left=50
    while [ "$_left" -gt 0 ]; do
