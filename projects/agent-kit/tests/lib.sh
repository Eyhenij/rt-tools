...
# Поля ввода, разобранные диспетчером, снимаются с окружения набора. Набор бывает запущен изнутри
# живого захода — гейт пуша идёт из-под самого диспетчера, — и тогда его гарды унаследовали бы
# ввод той сессии вместе с признаком разбора: судили бы живой вызов вместо фикстуры и краснели
# двадцатью наборами разом. Проверяется здесь механика гарда, а не окружение, в котором его
# запустили.
