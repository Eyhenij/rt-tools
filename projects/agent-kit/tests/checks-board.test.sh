...
# --- SC-AK-945 — задача без эпика и без слова владельца ---------------------------------
#
...
export STUB_ISSUES="$(epic_issues 'Повод и разбор')"
report "SC-AK-945 — задача без эпика названа" "$(board_says '#702: the task names no epic')" 1
report "SC-AK-945 — названы обе строки, которыми это чинится" "$(board_says 'Работа вне эпика')" 1
...
report "SC-AK-945 — со словом владельца сверка молчит" "$(board_says '#702: the task names no epic')" 0

# Карточка самого эпика задачей не считается: эпика у эпика нет.
report "SC-AK-945 — карточка эпика этой строки не получает" "$(board_says '#700: the task names no epic')" 0
...
# --- SC-AK-947 --- ветка эпика и его заявка ----------------------------------------------------
...
report "SC-AK-947 — эпик без ветки в заявках назван" "$(board_says '#700: the epic has no branch in the requests')" 1

# Заявка задачи в ветку эпика — ветка есть.
export STUB_PULLS="$(epic_pull RT-700-work-by-epics)"
report "SC-AK-947 — заявка в ветку эпика ветку показывает" "$(board_says '#700: the epic has no branch in the requests')" 0
...
report "SC-AK-947 — эпик с кончившимися задачами назван" "$(board_says '#700: the tasks of the epic are over')" 1
...
report "SC-AK-947 — с открытой заявкой эпика сверка молчит" "$(board_says '#700: the tasks of the epic are over')" 0
