...
# SC-AK-410 — конфликт приезжает в отданную заявку чужим слиянием, и своего хода у него нет:
...
report "SC-AK-410 — конфликтующая заявка названа расхождением" "$(board_code)" 1
report "SC-AK-410 — сказано, с чем конфликт" "$(board_says 'конфликтует с главной веткой')" 1

# SC-AK-411 — «ещё не посчитано» конфликтом не считается: хостинг считает сливаемость заново
...
report "SC-AK-411 — неизвестная сливаемость расхождением не считается" "$(board_code)" 0
export STUB_PULLS="$(conflicting_json MERGEABLE)"
report "SC-AK-411 — сливаемая заявка молчит" "$(board_code)" 0
