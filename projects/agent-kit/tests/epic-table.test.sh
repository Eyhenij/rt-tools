...
# --- SC-AK-1139 — на ветке самого эпика номер берётся из его карточки -----------------------
# Карточка эпика не объявляет другого эпика — она и есть эпик. Прежде отсюда шёл отказ «задача
# не объявляет эпика», и он читался как сломанная карточка, а не как неверно заданный вопрос.
git -C "$ET_TREE" checkout -q -b RT-900-probe-epic 2>/dev/null
report "SC-AK-1139 — код нулевой" "$(et_code)" 0
report "SC-AK-1139 — назван эпик ветки" "$(et_says 'Эпик «Эпик пробы» \(RT-900\)')" 1
report "SC-AK-1139 — отказа об объявлении нет" "$(et_says 'не объявляет эпика')" 0
git -C "$ET_TREE" checkout -q RT-902-probe 2>/dev/null
