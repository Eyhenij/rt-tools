...
# SC-AK-885. Команда линтера, не назвавшая правленый файл, названа в потоке ошибок один раз за
...
case "$(gap 'printf "x" > src/a.ts' заход-1)" in
    *"не назвала правленый файл"*) report "SC-AK-885 — команда без пути названа" да да ;;
    *) report "SC-AK-885 — команда без пути названа" нет да ;;
esac

report "SC-AK-885 — за тот же заход второй раз молчит" "$(gap 'printf "x" > src/a.ts' заход-1)" ''
...
report "SC-AK-885 — команда с путём молчит" "$(gap 'printf "x" > src/a.ts' заход-2)" ''
