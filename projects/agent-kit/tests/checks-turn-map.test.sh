...
out="$(run)"
case "$out" in
    *'it matches'*) report 'SC-AK-643 — состояния, записанные списком, проверка читает' 'сошлось' 'сошлось' ;;
...
out="$(run)"
case "$out" in
    *'it matches'*) report 'SC-AK-644 — прежняя форма таблицей читается по-прежнему' 'сошлось' 'сошлось' ;;
...
out="$(run)"
case "$out" in
    *'влито: the state is declared by the rule and forgotten in the map'*)
...
out="$(run)"
case "$out" in
    *'it matches'*) report 'SC-AK-905 — выходы хода под английскими именами читаются' 'сошлось' 'сошлось' ;;
