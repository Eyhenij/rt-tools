...
# рукой отклоняются. Закрытый эпик отпускает ход целиком, нечитаемый — судится как прежде.
#
...
# --- SC-AK-1129 — закрытый эпик отпускает ход целиком ------------------------------------------
# Конец эпика — законная остановка: ждать слова владельца там и есть работа. Таблица прочитана и
# пуста — отказ любого яруса отпускает ход: пустой ход, второй проход, передача рукой, слова об
# ожидании. Чтение одно на ход и только по дороге к отказу.
export STUB_LEFT='' STUB_CODE=0
expect_stop "SC-AK-1129 — при закрытом эпике ход с работой отпускается" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(edited)")")" PASS
expect_stop "SC-AK-1129 — при закрытом эпике пустой ход отпускается" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(reply)")")" PASS
expect_stop "SC-AK-1129 — при закрытом эпике второй проход отпускается" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(edited)" "$(said 'Готово.')")" true)" PASS
expect_stop "SC-AK-1129 — при закрытом эпике передача рукой отпускает ход" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(edited)" "$(ran 'npm run handoff')")")" PASS
expect_stop "SC-AK-1129 — при закрытом эпике слова об ожидании слова владельца отпускают ход" \
    "$(input_stop "$(transcript "$(say 'работай без остановок')" "$(edited)" "$(said 'Эпик закрыт, жду твоего слова.')")")" PASS

# --- SC-AK-1119 — нечитаемый эпик: прежнее поведение --------------------------------------------
# Ненулевой код таблицы или её отсутствие — эпик прочесть нельзя: так стоит ход на main и в дереве
# без таблицы. Отпускать по такому чтению значило бы отпускать всегда.
...
expect_stop "SC-AK-1119 — нечитаемая таблица: второй проход отпускается" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(edited)" "$(said 'Готово.')")" true)" PASS
expect_stop "SC-AK-1119 — нечитаемая таблица: пустой ход не отпускается" \
...
expect_stop "SC-AK-1119 — без таблицы вовсе: ход с работой отпускается" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(edited)")")" PASS
expect_stop "SC-AK-1119 — без таблицы вовсе: пустой ход не отпускается" \
    "$(input_stop "$(transcript "$(say 'продолжай')" "$(reply)")")" BLOCK


# --- SC-AK-1128 — ветка эпика не читается как задача без папки -----------------------------------
...
expect_stop "SC-AK-1128 — план с другой веткой ход на ветке без папки не отпускает" \
...
expect_stop "SC-AK-1128 — ветка, названная планом эпика, ярусом взятой задачи не судится" \
    "$(input_epic_branch "$(transcript "$(say 'продолжай')" "$(ran 'git commit -m x')")")" PASS
expect_stop "SC-AK-1128 — пустой ход на ветке эпика не отпускается" \
