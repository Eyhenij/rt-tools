...
# SC-AK-689 — набор, копящий счёт проверок, обязан этот счёт отдать.
#
# `report` копит число провалов в переменной обвязки, а на код возврата набора её переносит одна
# `suite_result`. Без неё набор кончается тем, что стояло последним — снятием двойника, уборкой
# дерева, — то есть всегда нулём: провалившаяся проверка печатается строкой и на цвет прогона не
# влияет никак. Два набора так и жили, и оба считались зелёными всегда.
#
# Судятся только наборы, зовущие `report`: тот, кто ведёт свой счёт сам и сам же отдаёт код,
# обвязке ничего не должен.
for file in "$(dirname "${BASH_SOURCE[0]}")"/*.test.sh; do
    grep -qE '(^|[[:space:];&|])report[[:space:]]' "$file" || continue
    if grep -q 'suite_result' "$file"; then
        report "SC-AK-689 — набор отдаёт свой счёт: ${file##*/}" PASS PASS
    else
        report "SC-AK-689 — набор отдаёт свой счёт: ${file##*/}" FAIL PASS
    fi
done
