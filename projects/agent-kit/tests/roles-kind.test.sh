...
report "SC-AK-1141 — описание есть у каждой команды" "$half_declared" 0

# --- SC-AK-1142 — заготовка держит места для заполнения -----------------------------------------
without_places=0
for blank in "$TEMPLATES"/*.md "$SAMPLES"/*/_template/*.md; do
    [ -f "$blank" ] || continue
    grep -qE '<[^>]+>' "$blank" || without_places=$((without_places + 1))
done
report "SC-AK-1142 — места для заполнения есть у каждой заготовки" "$without_places" 0

# --- SC-AK-1143 — заготовка папки задачи заголовка раскладки не держит ---------------------------
stamped=0
for blank in "$SAMPLES"/tasks/_template/*.md; do
    [ -f "$blank" ] || continue
    head -1 "$blank" | grep -q '^<!-- rt-kit v' && stamped=$((stamped + 1))
done
report "SC-AK-1143 — заголовка раскладки в заготовке нет" "$stamped" 0

# --- SC-AK-1144 — объявление дерева читается как данные -----------------------------------------
not_data=0
jq -e . "$ASSETS/variants.json" >/dev/null 2>&1 || not_data=$((not_data + 1))
jq -e . "$ASSETS/traits.json" >/dev/null 2>&1 || not_data=$((not_data + 1))
report "SC-AK-1144 — объявления дерева читаются как данные" "$not_data" 0

# --- SC-AK-1145 — вход в спеки отвечает нулём по имени всех четырёх родов ------------------------
...
report "SC-AK-1145 — ролей и навыков без спеки не осталось" "$uncovered" 0

# --- SC-AK-1146 — мера непокрытых показывает ноль ------------------------------------------------
left=0
if [ -f "$ROOT_TREE/tools/specs-for.mjs" ]; then
    left="$(cd "$ROOT_TREE" && node tools/specs-for.mjs 2>/dev/null | grep -c . || true)"
fi
report "SC-AK-1146 — непокрытых ресурсов не осталось" "$left" 0
