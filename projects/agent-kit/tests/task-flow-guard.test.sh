...
# SC-AK-872 — снятие перестоявших записей о законченных работах гардом не судится
...
b "SC-AK-872 — снятие перестоявших записей проходит" "node tools/archive-prune.mjs --apply" PASS
b "SC-AK-872 — правка кода той же оболочкой отбивается по-прежнему" \
...
b "SC-AK-872 — снятие в связке с правкой кода отбивается" \
...
printf '\n' > "$REPO/.claude/rt-kit/project.sh"
b "SC-AK-872 — без объявления команды вывода нет" \
    "node tools/archive-prune.mjs --apply && echo x > libs/site/x/ui/src/lib/a.component.ts" deny
