...
# Гейт цеплялся за расширение правимого файла и на такой команде молчал вовсе; читает этот текст
# человек, и владелец прочитал семь своих задач и две заявки прежде, чем сказать о языке в них.
gate_session_reset
...
# Вызов клиента без тела правила слога не требует: судятся оба признака сразу.
c "SC-AK-850 — заявка без тела слога не требует" 'gh pr view 12' Bash PASS
# Упоминание тела в чужой команде вызовом не бывает.
