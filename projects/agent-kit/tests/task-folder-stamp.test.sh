...
report "SC-AK-569 — снятие называет файлы, с которых шапка ушла" \
    "$(strip_says)" "grill.md,plan.md"
report "SC-AK-569 — в разборе просьбы первой строкой стоит заголовок" \
    "$(first_line grill.md)" '# Разбор просьбы'
report "SC-AK-569 — и в замысле тоже" \
