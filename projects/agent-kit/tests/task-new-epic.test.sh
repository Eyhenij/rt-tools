...
# --- SC-AK-936 — команда заводит эпик -------------------------------------------------
#
...
report "SC-AK-936 — в заготовке стоит таблица состава" "$got" "есть"
...
report "SC-AK-936 — у обычной задачи база ветки не печатается" "$got" "есть"

# --- SC-AK-937 — вызов отбивается до создания карточки --------------------------------
#
...
report "SC-AK-937 — без метки эпика карточка не создаётся" "$got" "нет"
if printf '%s' "$TN_OUT" | grep -q 'board.epicLabel'; then got="есть"; else got="нет"; fi
report "SC-AK-937 — отказ называет ключ настройки метки" "$got" "есть"

tn_config '' 'epic'
TN_OUT="$(tn_run --epic --title 'Поставка идёт эпиками' --slug work-by-epics)"
if [ -f "$TN_CREATED" ]; then got="создана"; else got="нет"; fi
report "SC-AK-937 — без каталога планов карточка не создаётся" "$got" "нет"
if printf '%s' "$TN_OUT" | grep -q 'plansDir'; then got="есть"; else got="нет"; fi
report "SC-AK-937 — отказ называет ключ каталога планов" "$got" "есть"

tn_config
TN_OUT="$(tn_run --epic --title 'Поставка идёт эпиками')"
if [ -f "$TN_CREATED" ]; then got="создана"; else got="нет"; fi
report "SC-AK-937 — без короткого имени карточка не создаётся" "$got" "нет"
if printf '%s' "$TN_OUT" | grep -q -- '--slug'; then got="есть"; else got="нет"; fi
report "SC-AK-937 — отказ называет недостающий довод" "$got" "есть"

# --- SC-AK-938 — задача заводится под эпиком ------------------------------------------
#
...
if [ -f "$TN_CREATED" ]; then got="создана"; else got="нет"; fi
report "SC-AK-938 — эпик и задача эпика разом не заводятся" "$got" "нет"
if printf '%s' "$TN_OUT" | grep -q -- '--epic and --epic-of'; then got="есть"; else got="нет"; fi
report "SC-AK-938 — отказ называет оба довода" "$got" "есть"

# --- SC-AK-939 — без эпика задача не заводится ----------------------------------------
#
...
if [ -f "$TN_CREATED" ]; then got="создана"; else got="нет"; fi
report "SC-AK-939 — без эпика карточка не создаётся" "$got" "нет"
...
report "SC-AK-939 — отказ называет первый выход: назвать эпик" "$got" "есть"
if printf '%s' "$TN_OUT" | grep -q -- '--outside-epic'; then got="есть"; else got="нет"; fi
report "SC-AK-939 — отказ называет второй выход: слово владельца" "$got" "есть"
...
if [ -f "$TN_CREATED" ]; then got="создана"; else got="нет"; fi
report "SC-AK-939 — с пустым доводом карточка не создаётся" "$got" "нет"
...
report "SC-AK-939 — довод владельца записан в тело задачи" "$got" "есть"
