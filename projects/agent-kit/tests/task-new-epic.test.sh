...
tn_run() {
    ...
    ( cd "$TN_TREE" && TN_CREATED="$TN_CREATED" GH_BIN="$TN_TREE/gh" RT_GH_RETRY_MS=1 \
...
if grep -q 'RT-4242 · \*\*Ветка:\*\* RT-4242-work-by-epics' "$TN_TREE/docs/plans/work-by-epics.md" 2>/dev/null; then
    got="есть"
else got="нет"; fi
