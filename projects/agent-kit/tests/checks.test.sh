...
# --- SC-AK-185 — очередь работ обходится без токена машинной записи -----------------------
#
...
report "SC-AK-185 — токена не требует" "$(printf '%s' "$BOARD_SAID" | grep -c 'нет токена бота')" 0
report "SC-AK-185 — хостинг спрошен" "$(cat "$BOARD_TREE/seen" 2>/dev/null)" 'залогиненный'
