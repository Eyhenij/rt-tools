...
# --- SC-AK-948 — объявление эпика читается одним видом ---------------------------------
#
...
report "SC-AK-948 — номер эпика прочитан из тела задачи" \
    "$(GE_BODY='Задача эпика #1921, замысел — docs/plans/work-by-epics.md' ge_read)" 1921
report "SC-AK-948 — ключ задачи вместо решётки читается так же" "$(GE_BODY='Задача эпика RT-1921' ge_read)" 1921
report "SC-AK-948 — падеж «задачи эпика» читается так же" "$(GE_BODY='Первая задачи эпика #7' ge_read)" 7

# Голое упоминание номера — не объявление: номер стоит в рассуждении, в цитате отказа и в списке
# того, чего работа не делает.
report "SC-AK-948 — голое упоминание номера не считается объявлением" "$(GE_BODY='Похоже на #1921, но это не эпик' ge_read)" 'нет'
report "SC-AK-948 — пустое тело не даёт эпика" "$(GE_BODY='' ge_read)" 'нет'

# --- SC-AK-949 — состояние задачи несёт номер эпика ------------------------------------
#
...
report "SC-AK-949 — состояние задачи несёт номер её эпика" "$(ge_state '"Задача эпика #1921"')" 1921
report "SC-AK-949 — у задачи вне эпика поле пустое" "$(ge_state '"Работа вне эпика — владелец попросил отдельно"')" 'нет'

# --- SC-AK-950 — ветка задачи берётся от ветки эпика ------------------------------------
#
...
report "SC-AK-950 — ветка задачи от главной отбита" "$got" "отбито"

if printf '%s' "$GE_OUT" | grep -q 'RT-1921-work-by-epics'; then got="есть"; else got="нет"; fi
report "SC-AK-950 — отказ называет ветку эпика" "$got" "есть"

if [ -z "$(ge_decision "$GE_WITH_EPIC" origin/RT-1921-work-by-epics)" ]; then got="прошло"; else got="отбито"; fi
report "SC-AK-950 — ветка от ветки эпика проходит" "$got" "прошло"

if [ -z "$(ge_decision "$GE_NO_EPIC" origin/main)" ]; then got="прошло"; else got="отбито"; fi
report "SC-AK-950 — у задачи без эпика основание судится по главной, как прежде" "$got" "прошло"


# --- SC-AK-951 — заявка задачи идёт в ветку эпика ---------------------------------------
#
...
report "SC-AK-951 — заявка с основанием «главная» отбита и названа ветка эпика" "$got" "отбито"

GE_OUT="$(ge_pull "$GE_WITH_EPIC" "$GE_PR_EPIC")"
if printf '%s' "$GE_OUT" | grep -q 'базе\|base here\|--base'; then got="отбито"; else got="прошло"; fi
report "SC-AK-951 — заявка с основанием ветки эпика по основанию не отбита" "$got" "прошло"

GE_OUT="$(ge_pull "$GE_NO_EPIC" "$GE_PR_MAIN")"
if printf '%s' "$GE_OUT" | grep -q 'RT-1921-work-by-epics'; then got="отбито"; else got="прошло"; fi
report "SC-AK-951 — у задачи без эпика основание заявки не судится" "$got" "прошло"


# --- SC-AK-952 — заявка эпика ждёт разбора папок его задач ------------------------------
#
...
report "SC-AK-952 — заявка эпика с папкой задачи отбита и папка названа" "$got" "отбито"
...
report "SC-AK-952 — без папок заявка эпика по этому условию не отбита" "$got" "прошло"
...
if printf '%s' "$GE_OUT" | grep -q 'RT-1930-something'; then got="отбито"; else got="прошло"; fi
report "SC-AK-952 — у карточки без метки эпика это условие не судится" "$got" "прошло"
