...
cp "$CHECKS/board-epic-plan.github.mjs" "$GE_TREE/tools/board-epic-plan.mjs"
...
# --- SC-AK-991 — объявление эпика читается одним видом ---------------------------------
#
...
report "SC-AK-991 — номер эпика прочитан из тела задачи" \
    "$(GE_BODY='Задача эпика #1921, замысел — docs/plans/work-by-epics.md' ge_read)" 1921
report "SC-AK-991 — ключ задачи вместо решётки читается так же" "$(GE_BODY='Задача эпика RT-1921' ge_read)" 1921
report "SC-AK-991 — падеж «задачи эпика» читается так же" "$(GE_BODY='Первая задачи эпика #7' ge_read)" 7

# Голое упоминание номера — не объявление: номер стоит в рассуждении, в цитате отказа и в списке
# того, чего работа не делает.
report "SC-AK-991 — голое упоминание номера не считается объявлением" "$(GE_BODY='Похоже на #1921, но это не эпик' ge_read)" 'нет'

# Те же два слова посреди предложения — рассказ об эпике, а не объявление принадлежности. Задача,
# в описании которой стояло «семь задач эпика #1870», читалась как его задача, и гард поставки
# отбил её ветку от главной. Объявление узнаётся местом в строке: оно её открывает.
report "SC-AK-991 — те же слова посреди предложения объявлением не считаются" \
    "$(GE_BODY='Все семь — задачи эпика #1870, их правки давно влиты' ge_read)" 'нет'
report "SC-AK-991 — объявление пунктом перечня читается" \
    "$(GE_BODY='- Вторая задача эпика #7, идёт после первой' ge_read)" 7
report "SC-AK-991 — пустое тело не даёт эпика" "$(GE_BODY='' ge_read)" 'нет'

# --- SC-AK-992 — состояние задачи несёт номер эпика ------------------------------------
#
...
report "SC-AK-992 — состояние задачи несёт номер её эпика" "$(ge_state '"Задача эпика #1921"')" 1921
report "SC-AK-992 — у задачи вне эпика поле пустое" "$(ge_state '"Работа вне эпика — владелец попросил отдельно"')" 'нет'

# --- SC-AK-993 — ветка задачи берётся от ветки эпика ------------------------------------
#
...
report "SC-AK-994 — заявка с основанием «главная» отбита и названа ветка эпика" "$got" "отбито"
...
# --- SC-AK-995 — заявка эпика ждёт разбора папок его задач ------------------------------
#
...
report "SC-AK-995 — заявка эпика с папкой задачи отбита и папка названа" "$got" "отбито"
...
report "SC-AK-995 — без папок заявка эпика по этому условию не отбита" "$got" "прошло"
...
report "SC-AK-995 — у карточки без метки эпика это условие не судится" "$got" "прошло"
