...
# SC-AK-1083 — раздел, набранный таблицей, читается наравне со списком пар. Набранный таблицей, он
# читался как пустой: проверка объявляла пропуск и стояла в наборе рядом с пройденными, не судя ни
# одного слова.
printf '# Словарь\n\n## Так не пишем\n\n| Не пишем | Пишем |\n| --- | --- |\n| **таска** | задача |\n' \
    > "$GL_TREE/docs/GLOSSARY.md"
printf '# Домен\n\nЗаведена таска на разбор.\n' > "$GL_TREE/docs/specs/domain.md"
git -C "$GL_TREE" add -A 2>/dev/null
report "SC-AK-1083 — запретное слово из таблицы краснеет" \
    "$(glossary_says 'docs/specs/domain.md.*таска')" 1
printf '# Домен\n\nЗаведена задача на разбор.\n' > "$GL_TREE/docs/specs/domain.md"
git -C "$GL_TREE" add -A 2>/dev/null
report "SC-AK-1083 — принятое слово при таблице молчит" "$(glossary_code)" 0
printf '# Словарь\n\n## Так не пишем\n\n- **таска, тикет** — задача\n- **спека (о тесте)** — тест\n' \
    > "$GL_TREE/docs/GLOSSARY.md"
git -C "$GL_TREE" add -A 2>/dev/null
