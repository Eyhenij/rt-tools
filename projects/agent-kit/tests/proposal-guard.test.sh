...
# Сводка сжатия: приходит ролью владельца и пересказывает ходы, которые уже кончились.
summary() { jq -c -n --arg t "$1" '{type:"user",isCompactSummary:true,message:{content:[{type:"text",text:$t}]}}'; }
...
# Она приходит ролью владельца и пересказывает всю сессию разом: просьба, исполненная вчера,
# читается в ней как сказанная сейчас. Ход, где о предложениях не сказано ни слова, отбивался на
# пересказе чужой просьбы.
expect_stop "SC-AK-816 — пересказ просьбы в сводке ход не отбивает" \
    "$(input_stop "$(transcript "$(say 'подтяни свежий main')" "$(summary 'Владелец просил отправить пропозал по гарду; работа отдана.')" "$(reply 'Подтянул.')")")" PASS
# Настоящая реплика владельца после сводки судится как обычно: отсев касается только сводки.
expect_stop "SC-AK-816 — просьба после сводки судится по-прежнему" \
    "$(input_stop "$(transcript "$(summary 'Пересказ прошлых ходов.')" "$(say 'отправь пропозал по этому месту')" "$(reply 'Написал файл.')")")" BLOCK
