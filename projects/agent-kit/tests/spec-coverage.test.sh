...
# --- SC-AK-1058 — непокрытых нет: проверка молчит и пропускает ---------------------------------
sc_config '{
    "portableDirs": ["pkg/assets"]
}'
report "SC-AK-1058 — при полном покрытии отправка идёт" "$(sc_code)" 0
report "SC-AK-1058 — сказано, что покрыто всё" "$(sc_says | grep -c 'spoken of by a spec')" 1

# --- SC-AK-1059 — непокрытый ресурс отбивает отправку -------------------------------------------
printf 'deny() { :; }\n' > "$SC_TREE/pkg/assets/hooks/b-guard.sh"
report "SC-AK-1059 — непокрытый ресурс отбит" "$(sc_code)" 1
report "SC-AK-1059 — названо число непокрытых" "$(sc_says | grep -c 'no spec speaks of 1')" 1
report "SC-AK-1059 — назван сам ресурс" "$(sc_says | grep -c 'b-guard.sh')" 1
rm -f "$SC_TREE/pkg/assets/hooks/b-guard.sh"

# --- SC-AK-1060 — дерево без объявленных папок пакета не судится --------------------------------
sc_config '{
    "portableDirs": []
}'
printf 'deny() { :; }\n' > "$SC_TREE/pkg/assets/hooks/c-guard.sh"
report "SC-AK-1060 — дерево без папок пакета пропущено" "$(sc_code)" 0
report "SC-AK-1060 — сказано, почему пропущено" "$(sc_says | grep -c 'nothing to judge')" 1

# --- SC-AK-1061 — входа в спеки нет: проверка говорит об этом вслух ------------------------------
sc_config '{
    "portableDirs": ["pkg/assets"]
}'
mv "$SC_TREE/tools/specs-for.mjs" "$SC_TREE/specs-for.away"
report "SC-AK-1061 — без входа в спеки отправка идёт" "$(sc_code)" 0
report "SC-AK-1061 — сказано, что спросить нечем" "$(sc_says | grep -c 'nothing to ask with')" 1
