...
dlv "SC-AK-408 — конфликтующая заявка черновик не снимает" "$CONFLICT" 'gh pr ready 917' deny
dlv_reason "SC-AK-408 — отказ называет заявку и конфликт" "$CONFLICT" 'gh pr ready 917' \
...
dlv "SC-AK-409 — молчание о сливаемости снятие не задерживает" "$NO_FIELD" 'gh pr ready 917' PASS
rm -rf "$NO_FIELD"
