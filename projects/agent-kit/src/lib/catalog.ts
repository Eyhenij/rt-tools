 ...
 * `**Требует:**`, у исполняемого файла — комментарий `# Требует:`. Слово читается на обоих языках:
 * ресурсы пакета переведены, а свои ресурсы дерева пишут его по-русски.
 */
const REQUIRES: RegExp = /^(?:#[^\S\n]*(?:Требует|Requires):|\*\*(?:Требует|Requires):\*\*)([^\n]*)$/m;
