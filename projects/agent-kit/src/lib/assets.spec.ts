...
describe('исполняемые ресурсы пакета', (): void => {
    ...
    it('гард замысла требует папку задачи, замысел и состояние — и ровно их', (): void => {
        expectGreen('task-flow-guard.test.sh');
    }, 120_000);

    it('SC-AK-743 — гард договорённости требует её одну и чужого не отбивает', (): void => {
        expectGreen('task-flow-draft-guard.test.sh');
    }, 120_000);
...
describe('холодная часть правила', (): void => {
    it('SC-AK-517 — холодная часть ложится третьим файлом в каталог своего правила', (): void => {
    ...
    it('SC-AK-518 — холодная часть за имя файла с самим правилом не спорит', (): void => {
