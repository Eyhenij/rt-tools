 ...
 * у правила пакета имя английское, у правила, написанного деревом до перевода слоя, — русское.
 */
function statementsOf(rule: string): readonly string[] {
    const section: RegExpExecArray | null =
        /^## (?:How the law applies here|Как закон применяется здесь|Что здесь действует)$([\s\S]*?)(?=^## |$(?![\s\S]))/m.exec(rule);
    if (!section) {
        return [];
    }
