...
- **Задача, оставшаяся в первой колонке, к поставке не готова.** Гард поставки называет её
  колонку и команду перевода: по очереди работ такая задача читается как невзятая, хотя работа
  по ней сделана и выложена. Имя первой колонки дерево называет само — не названо, колонка не
  судится вовсе: колонки каждое дерево зовёт своими словами, и выдуманное имя не совпало бы ни
  с чем.
...
- **Черновик не снимается, пока у PR нет разбора.** Гард поставки смотрит запрошенного ревьювера
  ...
  этого хода спросить негде — до открытия PR ревьювера нет вовсе.
...
которую печатает команда заведения задачи. Задачу, оставшуюся в первой колонке, гард при этом
называет — и на заведении ветки, и на открытии PR; прочие расхождения колонки находит сверка
очереди, уже после того, как PR открыт.
