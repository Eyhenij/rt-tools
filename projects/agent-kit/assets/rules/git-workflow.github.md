...
- **Главная ветка влита в ветку задачи до открытия PR.** Гард отбивает открытие, пока вершина
  главной не стала предком текущей: иначе ревьювер видит свою правку вперемешку с чужой, а
  проверено всё было от основания, которого уже нет.
- **Волна веток от одной главной проверяется пробным слиянием, а не по одной:** каждая зелена
  сама по себе, а сталкиваются тем, чего на отдельной ветке не видно.
- **Ветка следующей работы отводится от предыдущей, пока череда не прервалась.** `git checkout -b
  <КЛЮЧ>-<номер>-<slug> <предыдущая ветка>` вместо `origin/main`; от главной — первая работа
  череды. Иначе первое слияние разводит все остальные разом.
- **У заявки в череде основанием стоит предыдущая ветка, а не главная.** `gh pr create --base
  <предыдущая ветка>` — иначе разбор показывает правку вперемешку со всем, что под ней. Базу
...
- **Несошедшиеся условия поставки называются одним отказом.** Гард копит их и печатает разом: всё
  несошедшееся известно на первом вызове.
- **Условие, известное в начале работы, спрашивается в начале.** Заведение ветки — и с флагом
  между глаголом и `-b` — отбивает основание без вершины главной и чужую почту в подписи: на
  пуше это чинится дороже.
...
- **Ветка без номера задачи условий поставки не получает.** Локальная ветка под пробу законна, и в главную она не поедет.
...
- **Незаданный ключ отбивает работу с очередью на месте.** Умолчания нет намеренно: заголовок из пустого значения
  не совпадает ни с чем, и настоящее расхождение тонет среди ложных строк.
...
- **Видимость заведённого проверяется той стороной, которой оно предназначено.** Заявку читает
  человек, задачу — очередь работ, и оба читают не тем токеном, которым заводили. Чьими глазами
  снят ответ, говорит поле `viewer` в нём.
...
- **На борде стоят задачи, а не PR о них.** Карточка PR колонки не имеет, из очереди не уходит и
  остаётся навсегда. Находит такие карточки сверка очереди — строкой на каждую.
- **Отставшая колонка находится сверкой очереди, а не глазами.** Она судит колонку по PR в обе стороны: открытый PR при задаче не в разборе и разбор без открытого PR.
...
- **Связь задачи с эпиком сверка читает в обе стороны.** Односторонняя привязка выглядит целой
  так же, как двусторонняя: читатель приходит то от замысла эпика, то от карточки.
...
- **Заявка, чья база — не главная ветка, проверяется тем же набором, что и заявка в главную.**
  Условие запуска конвейера читает базу, и заявка в соседнюю ветку не поднимает его вовсе: пустое
  поле проверок читается как ожидание очереди. Спрашивается это до открытия первой заявки стопки,
  и заявка без прогона на вершине не сливается — приём в паттерне `git-workflow-stack`.
...
- **Черновик при зелёном прогоне на вершине — расхождение сверки.** Зелёная страница ничего не
  разрешает: кнопка заблокирована хостингом. В ходе это закрывает гард, между ходами — сверка.
- **Свои черновики судятся все разом, а не тот один, чья ветка взята.** Признака два — зелёный
  прогон на вершине и разобранная папка задачи в ветке; лежащая папка означает идущую работу.
- **Открытие PR отбивается, пока ветка везёт папку своей задачи.** Открытие — последняя точка, где отказ ещё виден исполнителю.
- **Свои открытые заявки перечитываются в трёх местах: перед пушем, при взятии задачи и после
  каждого известного слияния.** Заявка устаревает без действия автора: влита соседняя работа —
  остальные в ту же секунду отстали. Приём — `git-workflow-freshness`.
- **Конфликтующая своя заявка чинится первым действием хода, а новая работа до этого не
  берётся.** Пока отданное конфликтует, влить его человек не может. Работой здесь считается
  заведение задачи, заведение ветки, перевод колонки в работу и открытие заявки; всё, чем
  конфликт чинится, идёт как прежде.
- **Конфликтующий открытый PR — расхождение сверки очереди работ.** Конфликт приезжает чужим
  слиянием и по списку не виден: метку хостинг показывает только внутри самого PR.
...
- **Сборка входит в набор наравне с линтом и юнитами.** Линтер типов не читает, а юниты читают
  ...
  есть до мержа.
- **На машине, где раннеров несколько, любой путь от домашнего каталога общий.** Соседнее задание переписывает умолчание готового шага под свою версию. Каталог установки, имя контейнера и имя сборщика называются по проекту и постоянно; временный каталог не годится — в нём живёт хранилище пакетов.
- **Набор гейта пуша не бывает уже набора конвейера.** Шаг конвейера, которому в наборе нет ни
  строки, ни объявленного исключения с причиной, отбивает пуш, а не печатается рядом:
...
- **Набор гейта зовёт умолчание пакета, а не перечисляет его строками.** Переписанный строками,
  он оставляет завтрашнюю дыру: заведённая пакетом проверка до дерева не доедет. Отсеивать из
  умолчания законно, но поимённо и с причиной.
- **Итоговый набор перед пушем читается разбором состояния, а не сборкой в уме.** Раздел «набор
  перед пушем» печатает его целиком и называет рядом то, что умолчание печатало, а в набор не попало.
- **Причина исключения, называющая задачу, судится на живость этой задачи.** Мёртвый номер делает
  исключение бессрочным молча. Спрашивается тем же ярусом, что состояние задачи у гарда.
- **Сверка раскладки стоит в наборе гейта пуша наравне с линтом и сборкой.** Правка мимо
  источника копится молча, а сверки нет в конвейере.
- **После вливания главной ветки набор проверок пересматривается по тому, что ветка везёт
  теперь.** Проверять по тому, что правил автор, — значит проверять половину: отвечает ветка
  целиком.
- **Главная ветка берётся удалённой ссылкой — и в словах, и в действиях.** Локальная — снимок
  вчерашнего дня, и молчит она об этом. От `origin/main` идут и сравнение, и основание новой
  ветки, и счёт влитого: при локальной влитая ветка числится невлитой, а уборка кончается
  списком невлитого, которого нет.
- **Правка кода отдаётся человеку открытым PR, а не запушенной веткой.** Ветка во входящие не
  приходит и обсуждения не имеет: до открытия PR правки для человека нет. Открывается он тем же
  ходом, которым исполнитель говорит, что работу отдаёт.
...
- **Тело заявки пишется тем же ходом, которым она открывается, и рядом с образцом.** Заготовка
  накануне разошлась с ним на два раздела и строку связи: готовый на вид текст второй раз не
  читает никто, а гарды судят заголовок, номер ветки и папку задачи.
...
- **Черновик не снимается с ветки, которая не сливается.** Зелёный прогон говорит «не сломано»,
  сливаемость — «кнопку можно нажать», и владельцу нужно второе. Спрашивается у хостинга полем
  `mergeable`; из локального вливания она не выводится.
- **Указателю, в который ветки только дописывают строки, объявляется сложение обеих сторон.**
  Конфликт там не спор: обе допись нужны целиком. Метку сливаемости объявление **не гасит** — она
  гаснет, когда ветка вобрала главную и это уехало на хостинг.
...
- **Активная запись клиента хостинга выбирается на машину, а не на дерево, и машинная запись
  подставляется на вызов, а активной не делается.** Вход клиентом под машинную запись уводит все соседние сессии на машине: сосед с правами на один репозиторий читает свою задачу как несуществующую, а изнутри дерева промах не виден. Подстановка обязательна и при уже активной записи.

- **Автор PR не может быть его ревьювером.** Запрос разбора на себя GitHub принимает и молча не
  создаёт. Считаются только запрос и отзыв не от автора.
- **Вызов клиента хостинга идёт из дерева, а связка команд исхода не проверяет.** Вне дерева клиент отказывает, а следующее звено увозит на хостинг остаток.
- **Метки, исполнитель и ревьювер PR ставятся вызовами `gh api`, а не `gh pr edit`:** тот
  отвечает отказом про Projects.
- **Борда правится запросом GraphQL по идентификатору борды, а не по имени владельца.**
- **Почта машинного коммита копируется из компаньона, а не набирается по памяти.** Адрес
  `<число>+<логин>@users.noreply.github.com` сходится по числу: с чужим коммит уезжает подписанным
  посторонним. Ту же строку держит профиль дерева, и гард отбивает пуш.
- **Личность машинной записи подтверждается ответом хостинга, а не узнаванием строки:** её
  спрашивают токеном, и логин с числом приходят вместе.
- **Сценарии гардов задают настройки git сами, а не берут их с машины.** Подпись с машины уводит git в агент ключей, и заблокированный агент роняет набор; автор, почта и подпись идут флагами `-c`.

- **Каждый коммит вклада ветки подписан машинной записью, и это проверяется набором пуша.** Гард
  судит коммит, назвавшийся ею; подписанный человеком проходит мимо.
- **Состояние заявки перечитывается у хостинга сразу после публикации.** Открытие и постановка
  ревьювера отвечают нулём и тогда, когда не сделали ничего.
- **Автор открытой заявки и наличие у неё ревьювера сверяются очередью работ.** До слияния оба
  промаха не видны: открытая человеком заявка ревьювера не получит никогда.
- **Ревьюверы спрашиваются вызовом REST, а не выборкой клиента.** Её поля собирает GraphQL, прав
  на который у записи нет: «сошлось» неотличимо от «спросить было нечем».

- **Рабочее дерево не опустошается ради прогона инструмента.** Сравнение идёт на второй копии:
  прятанье уводит незакоммиченную работу туда, где её не видит ни состояние дерева, ни сверка.
...
- `git-workflow-pr-ready` — доведение отданного: снятый черновик, разбор красного, волна по череде.
