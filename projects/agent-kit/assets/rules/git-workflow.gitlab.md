...
- **Черновик не снимается, пока у MR нет разбора.** Гард поставки смотрит запрошенного ревьювера
  ...
  запрос и отзыв не от автора: разбор на самого себя разбором не бывает.
...
- **Задача, оставшаяся в первом списке, к поставке не готова.** Гард поставки называет её список
  и команду перевода: по очереди работ такая задача читается как невзятая, хотя работа по ней
  сделана и выложена. Имя первого списка дерево называет само — не названо, список не судится
  вовсе: списки каждое дерево зовёт своими словами, и выдуманное имя не совпало бы ни с чем.
...
которую печатает команда заведения задачи. Задачу, оставшуюся в первом списке, гард при этом
называет — и на заведении ветки, и на открытии MR; прочие расхождения списка находит сверка
очереди, уже после того, как MR открыт.
