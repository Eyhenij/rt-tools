...
- **Работа, которая разбирает папку задачи, своей папки не заводит.** Иначе она снимает одну и
  оставляет вторую, сверка очереди краснеет на неё, и следующая работа повторяет то же — круг не
  кончается. Замысел на диске требует гард, и требует он его только под правку кода приложения;
  разбор папок туда не доходит, а понимание такой работы целиком помещается в тело отчёта.
