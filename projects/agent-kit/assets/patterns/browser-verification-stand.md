...
Команды ниже зовут порты именами — `API_PORT`, `SITE_PORT`, `ADMIN_PORT`, `SSR_PORT`, порты
стенда и порты прокси. Номеров у паттерна нет и быть не может: раскладка стендов у каждого дерева
своя, а дерево с одним приложением, обязанное назвать восемь чужих номеров, называет их выдуманными
— и подстановки перестают значить что-либо вовсе. Номера дерево объявляет своим профилем: в
переменных окружения либо разделом надстройки при этом паттерне, где стоят его же готовые команды
с именами целей раннера.

Роль каждого имени — то, о чём говорит проза: порт приёмника, порт сайта, порт админки, порт
сервера отрисовки. Дерево, у которого такого приложения нет вовсе, соответствующие разделы просто
не читает.
