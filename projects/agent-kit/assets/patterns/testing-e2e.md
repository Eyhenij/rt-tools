...
Имена целей раннера и номера портов у каждого дерева свои: паттерн их не знает и держать не может.
Дерево называет их своим профилем — переменными окружения либо разделом надстройки при этом
паттерне, где стоят готовые команды с его именами.

По умолчанию конфиг идёт на порт стенда сайта и подхватывает уже поднятый сервер. С `BASE_URL`
свой сервер не стартует вовсе.
...
- **Двойник, переехавший из спеки в общий файл набора, выходит из-под исключения проверки
  повторов.** Она пропускает файлы спек и читает всё остальное, включая двойники и настройки, а
  ловит имя объявления — не значение и не назначение. Настройка, лежавшая в спеке под общим именем,
  после переезда сталкивается с одноимённой в другом слое, и проверка краснеет на файле, которого
  правка не касалась: разбор такого красного стоит отдельного хода. Имя при переезде получает
  приставку своего набора; строка в список известного не заводится — он только сокращается.
