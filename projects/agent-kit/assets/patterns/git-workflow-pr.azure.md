...
kind: pattern
rule: git-workflow
description: Паттерн правила git-workflow. Брать на открытие PR и всё, что с ним связано, — формат заголовка, черновик и его снятие, привязка к рабочему элементу, ревьювер и исполнитель, образец тела с разделом об оставшемся шаге, чтение состояния PR, перевод элемента в состояние разбора, чеклист проверок до снятия черновика. Не брать на заведение элемента, ветки и коммит — это паттерн git-workflow-commit.
---
