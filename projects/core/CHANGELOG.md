...
- **rt:core:** набор входов компонента, созданного в коде, проверяется сборкой: имена берутся у
  самого класса и отбираются по типу поля, а значением служит то, что вход принимает снаружи.
  Компонент, поднятый разметкой, обе ошибки — чужое имя и чужой тип — ловит проверкой шаблона;
  созданный в коде не ловил ни одной. Ставятся входы своей функцией: тип, объявленный и никем не
  применённый, не проверяет ничего.

- **rt:core:** служба часового пояса отвечает поясом читателя, а вне браузера — согласованным
  `UTC`, не спрашивая окружения вовсе. Пояс машины, отдающей страницу, выглядит настоящим ответом
  и врёт про читателя: разницу в несколько часов замечает только тот, у кого дата съехала на день.
  Читается пояс при каждом обращении — читатель переезжает и правит настройку системы, не
  перезагружая вкладку.

- **rt:core:** `*rtLet` называет значение именем внутри куска разметки: выражение стоит в шаблоне
  один раз, а дальше читается именем — умолчанием или именем директивы. Показом директива не
  распоряжается: содержимое рисуется при любом значении, пустом в том числе, и условие, написанное
  рядом, она не отменяет. Представление создаётся один раз, поэтому смена значения не роняет
  состояние того, что внутри: набранный текст, открытую панель, положение прокрутки. Тип значения
  доезжает до содержимого — строгая проверка шаблона знает его без приведения.

### BREAKING CHANGES

- **rt:core:** импорты переехавшего из `@rt-tools/utils` переставляет `pnpm run migrate:utils-imports <каталог>` — он же
  переименовывает `POSITION_ENUM` в `EPosition` по ходу перестановки, так что оба перехода делаются одним прогоном.
  Смешанное объявление он делит надвое и оставляет в прежнем пакете то, что там и осталось.
...
- **rt:core:** `MessageBusEvent` → `IMessageBusEvent` and `Primitive` → `IPrimitive`, matching the `I`-prefix convention.
...
- `isNil` - Type guard function for null/undefined checks
- `MessageBus` - Event bus for component communication
- `PlatformService` - Service for detecting browser/server platform
- `WINDOW` - Injection token for safe window access
