...
@Component({
    ...
    imports: [MatIcon, MatIconButton, MatTooltip, MatMiniFabButton, BlockDirective, ElemDirective, ModDirective],
...
export class RtuiTableHeaderCellComponent<SORT_PROPERTY> {
    ...
    public sortModel: InputSignal<SortModel<SORT_PROPERTY>> = input.required<SortModel<SORT_PROPERTY>>();
    ...
    private handleClick(): void {
        if (this.sortModel()?.propertyName) {
            this.sortChange.emit({
                propertyName: this.sortModel().propertyName,
                sortDirection: this.#getNextSortOrder(),
            });
        }
    }

    #getNextSortOrder(): ListSortOrderType {
        if (this.sortModel().sortDirection === LIST_SORT_ORDER_ENUM.ASC) {
