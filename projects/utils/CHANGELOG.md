...
- **rt:utils:** импорты переехавшего переставляет `pnpm run migrate:utils-imports <каталог>`: он берёт каталог, обходит
  его файлы кода и меняет адрес пакета у тех объявлений, где стоят только уехавшие имена, а смешанные делит надвое.
  Приложения он не знает: единственное его знание о предмете — таблица «имя → пакет», и переименование перечисления
  положений едет вместе с адресом. `--dry` называет то же, ничего не записав; импорт пакета целиком он не трогает и
...
- prefix the interfaces and generic type helpers with I ([bcb5e76](https://github.com/nickmdf/rt-tools/commit/bcb5e76c064b82d46f152a6af807b94b2010384d)), closes [#234](https://github.com/nickmdf/rt-tools/issues/234)
...
## [0.0.8](https://github.com/nickmdf/rt-tools/compare/rt-utils@0.0.7...rt-utils@0.0.8) (2026-07-14)
