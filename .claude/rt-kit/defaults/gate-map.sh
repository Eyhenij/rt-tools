#!/usr/bin/env bash
# rt-kit v0.16.1 · defaults/gate-map.sh · 0a6219521950 · правится надстройкой, не здесь
        ...
        edit)
            case "$target" in
                ...
                */eslint-rules/*) printf '%s\n' 'typescript-conventions' ;;
                ...
                # Схема хранилища, её миграции и настройка клиента: порядок каталогов
                # лексикографический, а метку времени ставит инструмент в момент заведения —
                # цепочка ломается молча и падает только накатом с нуля, то есть уже после
                # слияния. Настройка клиента стоит здесь же: её читает тот же накат, а правилась
                # она без правила под рукой. Правило живёт при поставке.
