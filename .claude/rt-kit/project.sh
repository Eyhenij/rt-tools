...
# Форма номера задачи в заголовке отчёта. Умолчание пакета ждёт `[КЛЮЧ-123] …` — форму трекера
# с ключом задач; здесь задачи это issue на GitHub, у них ключа нет, и номер стоит в хвосте:
# «Что сделано (#312)». Гард сверяет заголовок этой формой, а вот номер из него он извлекает
# зашитой формой ключа — значит совпадение номера ветки и заголовка здесь не сверяется ничем.
RT_TASK_TITLE_RE='\(#[0-9]+\)$'

# Состояние задачи по номеру. Умолчание пакета получает номер от гарда, а тот читает его из
# имени ветки формой `КЛЮЧ-123`. Здесь ветка зовётся `<род>/<номер>-<имя>` — форма профилем
# разрешена, но номер из неё гард не достаёт, и проверка задачи молча выключается: гард
# спрашивает про задачу «#» и получает «такой нет». Достаём номер сами.
rt_task_state() {
    local number="$1"

    if [ -z "$number" ]; then
        number="$(git rev-parse --abbrev-ref HEAD 2>/dev/null | sed -nE 's#^[a-z]+/([0-9]+)-.*#\1#p')"
    fi

    [ -z "$number" ] && return 0

    # `numbered` — про трекер, где номер задачи виден только в её заголовке. Здесь задача это
    # issue на GitHub: номер и есть её имя, он стоит в очереди работ, в ветке и в заголовке
    # отчёта. Дублировать его в тексте названия нечем и незачем.
    rt_task_state_default "$number" | jq -c '.numbered = true' 2>/dev/null
}
