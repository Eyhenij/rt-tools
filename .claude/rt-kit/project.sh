...
# Кто придёт к хостингу по этому токену. Спрашивается тем же файлом, который читает подстановка
# выше: файла на месте нет — значение пустое, клиент отвечает от залогиненной записи владельца,
# и гард поставки видит это по логину, а не по тексту команды.
#
# Клиент зовётся полным путём: в оболочке владельца это имя занято псевдонимом менеджера паролей,
# и вызов по имени ждал бы подтверждения, которого гарду никто не даст.
rt_pull_token_login() {
    local token
