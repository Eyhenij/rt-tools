...
#
# Проверка повторов стоит рядом со сверкой спеков и по той же причине: конвейер её не гоняет, и
# до этой строки её красное не отбивало ничего. Своё перечисление под уже объявленным набором ни
# линт, ни сборка, ни тесты не видят — каждая копия сама по себе исправна.
rt_push_checks() {
    cat <<'EOF'
