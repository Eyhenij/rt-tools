...
```bash
npm run task:new -- --epic-of <номер эпика> --title '<что не так>' --slug <короткое-имя> --label enhancement < тело.md
```

Эпик называется всегда: правка слоя правил идёт эпиком, как и всякая другая. Если владелец
принял правку вне эпика, вместо номера идёт `--outside-epic «слово владельца»`.
