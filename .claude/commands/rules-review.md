...
**Своя команда этого дерева, и пакет её не везёт.** Судятся исходные тексты ресурсов, а лежат
они только здесь: у дерева, которое пакет ставит, есть разложенная копия выбранных ресурсов, а
не семья целиком, и путей, по которым команда ходит, в нём нет вовсе — обход по ним вернул бы
пустоту, неотличимую от «такого закона не бывает».
