...
kind: rule
law: verifiability
description: Готовый код и порядок проверки компонента кита — спека на TestBed, история с визуальным снимком, обход всех историй, замер вычисленных значений. Брать при заведении или правке *.spec.ts рядом с компонентом, при разборе упавшего снимка витрины, при правке .storybook/test-runner.ts, прогонщика снимков, обхода историй и эталонов в __snapshots__, а также когда надо решить, чем именно подтверждается правка вёрстки. Правило этого дерева; «зачем» — в правилах testing и browser-verification.
---
...
- **Волна показов закрывается обходом всех историй, а не цветом прогона.** Зелёные спеки,
  ...
  ничего, кроме обхода — `pnpm run test:stories:v2`, разобран в `visual.md`.
...
5. Число в отчёте: сколько тестов исполнилось, сколько снимков сошлось, что показал замер.
