...
- **A frame of a node beyond the window is cured the same way.** — `projects/ui-kit-v2/.storybook/test-runner.ts:fitViewportToNode` — the window is widened to the root of the showing before an ordinary frame. The base window is 1280×720, and almost every matrix of the kit is higher. The first kit has no such step: it shoots the whole page.
