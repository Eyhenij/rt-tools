...
| У кадров экрана нет своего шага ни в гейте, ни в конвейере.                                 | Шаг «E2E admin» в `.github/workflows/ci.yml` и та же команда в наборе гейта пуша — `.claude/rt-kit/project.sh`. Кадры снимаются внутри него; отдельного шага у них нет, и видно это списком шагов, а не проверкой.                                                        |
| Плывущее в кадре экрана убирается поимённо, а не покрывается порогом.                       | `apps/message-bus-admin-e2e/playwright.config.ts:expect.toHaveScreenshot` — `threshold` и `maxDiffPixelRatio` держатся у нуля; там же назван цветовой профиль браузера. Причины расхождения закрыты в засеве стенда, а не порогом.                                        |
