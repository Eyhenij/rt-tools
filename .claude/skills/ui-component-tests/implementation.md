...
- **A consumer of the showcase brings back an index the showcase lost, and refuses by the index rather than by the frames.** — `tools/showcase-probe.mjs:ensureIndex`, the refusal text — `tools/showcase-probe.mjs:complaintAboutIndex`. It is called by `tools/story-sweep-v2.mjs:ownShowings` and `tools/visual-snapshots-v2.mjs:requireOwnShowcase`, before either reads the index. The set `tools/tests/showcase-probe.test.sh` judges the mechanics over a substituted index; scenario `SC-UKV-142`.
