...
- **Which PRs the pipeline wakes for is named by the tree, and it sets the order of handing in.** — `.github/workflows/ci.yml:branches` — the trigger is narrowed to `main`: the single runner stands on the owner’s machine, and a PR into an epic branch would take a whole run for checks the push gate has already done there. What is lost by it is written in the comment next to the trigger.
