...
---
<!-- rt-kit v0.20.0 · patterns/git-workflow-pr.github.md · 35c1b2bc7613 · правится надстройкой, не здесь -->
...
которая ждёт конца прогона и обещает отдать его исход кодом, возвращает ноль и на красном:
дождалась она конца, а не успеха, и «ноль» здесь означает «ожидание удалось». Прочитанный как
исход, он делает зелёным то, что упало, — и сказанное владельцу о зелёном прогоне оказывается
про красный. Спрашивают поэтому состояние и заключение прогона своим вызовом, тем же, каким
читают состояние заявки.
