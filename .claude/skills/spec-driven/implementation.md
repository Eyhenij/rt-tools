...
| Привязка не ведёт в код, который никто не зовёт.                                     | `tools/check-specs.mjs:codeOf` — комментарии вычищаются, и символ, объявленный в своём файле и больше нигде не встречающийся, называется мёртвой привязкой.                               |
...
| Якорь сверяется по сырому тексту файла, и комментарий засчитывается наравне с кодом. | `tools/check-specs.mjs:fileHasSymbol` — существование символа ищется словом по всему файлу, а живость считается только у объявленного в коде.                                             |
