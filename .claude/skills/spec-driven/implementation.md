...
| Номер сценария выдаётся один раз и повторно не используется.                  | `tools/check-specs.mjs:parseScenarios` собирает номера домена, но повторную выдачу не судит: номер, отданный второму сценарию, для неё такой же законный. Держится чтением и разбором отчёта. |
...
| Спек объявляет законы, которые применяет, и связь сверяется в обе стороны.    | Строка `**Законы:**` в `spec.md` каждого спека; сверяет `pnpm run check:specs` в обе стороны.                                                                                                 |
