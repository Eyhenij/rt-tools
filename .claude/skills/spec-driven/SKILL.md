...
---
<!-- rt-kit v0.17.0 · rules/spec-driven.md · f4d63cadd165 · правится надстройкой, не здесь -->
...
- **Первая колонка компаньона — текст статьи, скопированный, а не пересказанный.** Ключ связи —
  сам текст, поэтому строка, написанная по смыслу статьи, разрывает связь, оставаясь на вид
  заполненной. Проверка это подтверждает: она считает статьи без адреса и о статье, адрес которой
  описывает не её, не знает ничего. Статье о действиях, требующих отдельного слова владельца,
  поставили привязкой гард, держащий действие не из её перечня: совпал род, а не перечень, и
...
- **Таблица процедур сверяется с декораторами в обе стороны, и право сверяется вместе с
  именем.** Иначе процедура, которую домен обслуживает, но забыл описать, видна только в
  декораторе, а право, разошедшееся со спеком, — нигде.
...
- **Проверка, прибитая к имени ресурса, ломается на его делении, и знать об этом надо до
  правки.** Имя правила и имя паттерна стоят не только в текстах, но и в проверках дерева, в
  пробах их наборов и в таблицах, которые связывают состояние с ведущим его паттерном. Деление
  ресурса поэтому начинается с поиска его имени по всему дереву, а не по слою правил: найденное
  правится тем же изменением. Иначе красное приходит по одной проверке за прогон, и каждая
  выглядит отдельной поломкой — так деление одного правила уронило три подряд.
