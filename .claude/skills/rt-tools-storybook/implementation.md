...
- **A component that takes its size from its parent is given one by a box of the showing, not by a rule on its block class.** — `projects/ui-kit-v2/src/lib/components/aside/stories/component/test-aside-matrix.component.ts:TestRtAsideMatrixComponent` — the five panels stand in a box of their own. The former rule by the block class reached only the host, declared `display: contents`. The panel measured 0 by 0 at a declared 320. `projects/ui-kit-v2/src/lib/components/chat/stories/component/test-chat-matrix.component.ts:TestRtChatMatrixComponent` — the nine boxes give the chat its width. The chat takes their full height.
- **The surface under such a component is the box too, when the component takes its background from the consumer.** — `projects/ui-kit-v2/src/lib/components/chat/stories/component/test-chat-matrix.component.ts:TestRtChatMatrixComponent` — the box carries the ground, the border and the radius. The chat draws its own ground only when expanded over the page.
