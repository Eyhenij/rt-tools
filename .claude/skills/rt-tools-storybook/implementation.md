...
- **The arg-driven page is called `Playground` everywhere, and a family without one names its reason.** — `tools/check-kit-coverage.mjs:hasPlayground` reads the family's stories folder and looks for the page as text. The reasons lie under the key `noPlayground` of `tools/kit-coverage-allowlist.json`. The check refuses a family without the page, and also a reason left behind the page that appeared.
...
- **A component that does not give its width to its host collapses in a half, and the row cell is then given a width by name.** — `projects/ui-kit-v2/src/showcase/story-row.component.ts:slotWidth` — the row's cell takes the named width, and the row wraps instead of squeezing the cell. The pagination host measured 8 pixels inside a half while its content drew 300: four cases lay on top of one another and the frame was green. `projects/ui-kit-v2/src/lib/components/workspace/stories/component/test-workspace-matrix.component.ts:TestRtWorkspaceMatrixComponent` — the three rows of the workspace take 56rem, and the box inside the cell takes the cell's full width. The named width alone was not enough: the box is a flex item and shrank to its content, so the cell measured 896 while the box measured 557. Without either, the cell of the widths row came out 150 points on a window of 768, against panels declared at 320 and 360.
- **A showing of one instance asks the pair for the width of its half.** — `projects/ui-kit-v2/src/showcase/story-presets.component.ts:fill` — the input draws the modifier on the pair's host. The styles then give the showing the cross-axis size of the half. Eighteen showings of one instance carry it. The confirmation popover does not: it holds a width of its own and stood the same on the page.
- **A showing that gets no pair says why, in the markup next to it.** — **Not checked by anything.** A machine cannot tell a deliberate refusal from a forgotten wrap: both are a matrix without the pair's host. Three such showings stand in the tree today — the icon catalogue, the compact page header and the floating message, each with its reason written in the template above the markup. The message is pinned to the window: in a half it leaves the half whole, both halves stay empty, and the second instance lies over the first.
- **A component that takes its size from its parent is given one by a box of the showing, not by a rule on its block class.** — `projects/ui-kit-v2/src/lib/components/aside/stories/component/test-aside-matrix.component.ts:TestRtAsideMatrixComponent` — the five panels stand in a box of their own. The former rule by the block class reached only the host, declared `display: contents`. The panel measured 0 by 0 at a declared 320. `projects/ui-kit-v2/src/rich-editor/lib/components/chat/stories/component/test-chat-matrix.component.ts:TestRtChatMatrixComponent` — the nine boxes give the chat its width. The chat takes their full height.
- **The surface under such a component is the box too, when the component takes its background from the consumer.** — `projects/ui-kit-v2/src/rich-editor/lib/components/chat/stories/component/test-chat-matrix.component.ts:TestRtChatMatrixComponent` — the box carries the ground, the border and the radius. The chat draws its own ground only when expanded over the page.
- **The width of a cell is measured again the day the component learns to shrink into it.** — `projects/ui-kit-v2/src/lib/components/file-card/stories/component/test-file-card-matrix.component.ts:TestRtFileCardMatrixComponent` — four widths by measurement instead of one for the family. The matrix of sizes and the row of disabling take 24rem, the row of interaction 22rem, the rows of one button 17rem.
- **A component pinned to the window needs a box that is its containing block, and a relative box is not one.** — `projects/ui-kit-v2/src/lib/components/bottom-sheet/stories/component/test-bottom-sheet-matrix.component.ts:TestRtBottomSheetMatrixComponent` — the five demonstration boxes of the sheet carry `contain: paint`. Four showings of the family drew a dashed line instead of the sheet, and the sheet itself lay across the page under them all. A measurement over 546 stories found no other such node in the tree.
- **A component whose height comes from the window is shown inside a box that clips.** — `projects/ui-kit-v2/src/lib/components/workspace/stories/component/test-workspace-matrix.component.ts:TestRtWorkspaceMatrixComponent` — the five demonstration boxes of the workspace carry a clip, and what is drawn stops at the box. The workspace takes `calc(100dvh - …)`, so under a window grown for the shot it measured 720, 2500 and 7046 points in turn.
