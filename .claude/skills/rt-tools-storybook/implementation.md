...
| Провайдер, без которого компонент не поднимается, стоит в `preview.ts`, а не декоратором одной истории. | Держится разбором: `projects/ui-kit-v2/.storybook/preview.ts` — единственный общий инжектор второй витрины; ни линтер, ни тайпчек декоратор истории с провайдером не отличают. |
