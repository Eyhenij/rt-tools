...
- **Заголовок истории начинается уровнем атомарного дизайна, а не общим разделом.** Уровней
  ...
  панель, таблица, переписка, рабочий стол, каркас. Общего корня над уровнями нет, порядок
  разделов задаёт `storySort` — по алфавиту молекулы встали бы перед организмами.

- **Уровень выводится из состава компонента, а не из ощущения его сложности.** Признак один и
  спрашивается о самом компоненте: из чего он собран.

    | Уровень       | Признак                                                                                             | Примеры                              |
    | ------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------ |
    | **Atoms**     | не собран ни из одного другого компонента кита — разбирать дальше нечего                            | кнопка, значок, поле ввода, крутилка |
    | **Molecules** | собран из атомов и решает одну задачу; перекрытия не открывает и списком записей не живёт          | поле с подписью, вкладки, карточка   |
    | **Organisms** | самостоятельный кусок экрана: открывает перекрытие, держит своё состояние или живёт списком записей | таблица, окно, боковая панель        |
    | **Templates** | целый экран из готовых компонентов; осей входов у него нет вовсе                                     | рабочий стол, страница списка        |

    Спорное решается сверху вниз: открывает перекрытие или живёт записями — организм, даже
    если разметки в нём мало; ни того ни другого, но внутри стоит другой компонент кита —
    молекула; кита внутри нет — атом. Число входов, длина файла и «это же простой компонент»
    признаками не являются: по ним один и тот же компонент попадает у двух авторов на разные
    уровни, а разошедшийся уровень тянет за собой переименование эталонов.

- **Дочерняя часть стоит в группе своего родителя; уровень ей даёт тот же признак состава.**
  Шапка окна — рядом с окном, пункт меню — рядом с меню, и чаще всего это тот же уровень: часть
  целого собрана не сложнее целого. Но собирающий компонент законно стоит ступенью выше своей
  части — плашка оповещения атом, а стопка плашек над страницей молекула, — и группа при этом
  остаётся общей. Разъехавшаяся группа и есть промах: часть теряется в списке там, где её ищут
  рядом с целым.
