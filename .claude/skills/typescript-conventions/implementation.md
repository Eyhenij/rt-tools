...
| ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Род объявления виден по префиксу имени, и это держат три правила линтера.      | **Не исполняется.** Правил имён в `eslint.config.mjs` нет ни одного. Приставка `I` у интерфейсов и суффикс `_ENUM` у перечислимых наборов выдержаны по дереву, но нарушение поймает только разбор.          |
| Источник, за которым следят, назван суффиксом.                                 | `tools/lint-rules/rules/require-source-suffix-for-subjects.cjs:REQUIRED_SUFFIX` — отказ линтера; нарушений в дереве нет.                                                                                    |
...
| Файл не длиннее 500 строк, и считаются все строки — пустые и комментарии тоже. | `tools/check-file-size.mjs:LIMIT` — предел общий с текстами и стилями, считаются все строки.                                                                                                                |
...
| Двухступенчатое приведение `as unknown as` запрещено правилом линтера.         | **Не исполняется.** Своего правила на двойное приведение в конфиге нет. Ближайшее — `@typescript-eslint/no-non-null-assertion` и `no-unnecessary-type-assertion`, обе отказом.                              |
