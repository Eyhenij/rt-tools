...
| Отметка об устаревании ставится вместе с обходом потребителей.        | Образец — снятый с употребления набор `.c-button`: отметка стояла в его файле, пока файл был, а карта перехода на `.rtui-btn` осталась в `projects/ui-kit/src/styles/TOKENS.md`.                                     |
