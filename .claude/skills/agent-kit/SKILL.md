...
Слияние идёт по заголовку, и совпавший заголовок замещает раздел целиком. Пакет, дописавший в
такой раздел новое утверждение, до дерева его не довозит: `sync --check` зелен — он сверяет
разложенное с тем, что собрал сам, — заголовки на месте, а утверждения нет. Сверка заголовков
ловит переименование раздела, а не пополнение.

Поэтому после подъёма версии читается вывод `doctor`: он называет каждый замещённый раздел —
ресурс и заголовок. Это места, где пакетного текста дерево не видит; что именно пакет в них
дописал, сравнивается с историей самого пакета. Замещённых разделов нет — строки нет вовсе.
