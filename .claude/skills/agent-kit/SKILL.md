...
---
<!-- rt-kit v0.16.1 · skills/agent-kit.md · e4b80a44ed2e · правится надстройкой, не здесь -->
...
Оба значения приходят от владельца приёма: адрес он называет, а токен выдаёт одноразовым кодом
приглашения на это дерево — код заводит дерево в приёме и кладёт токен в файл, названный ключом
`token`. Потребитель ни того ни другого не придумывает: приём — своя служба, и до приглашения
отправка отказывает по устройству, а не по недонастройке. Отказ при этом называет ключ, и
читатель уходит в конфиг — то есть туда, где ответа нет.
