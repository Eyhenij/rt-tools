...
---
<!-- rt-kit v0.23.0 · skills/agent-kit.md · f2f47cbfaf03 · правится надстройкой, не здесь -->
...
Записи предложений в день обновления никто не открывает: в руках у исполнителя разложенный ресурс
и надстройка, а на запись ничто не указывает. Пометка не видна в собранном тексте, переживает
слияние по заголовкам и читается машиной: раскладка перечисляет разделы, чья статья в приехавшей
редакции уже есть. Снимает их человек — пометка держит одну статью, а раздел мог обрасти и другим.
Раздел без пометки считается постоянным: снимать его приезд редакции не предлагает.
...
- **Своё, стоящее рядом с разложенным, повторяет его механику молча.** Проверка, гард и утилита
  ...
  исходников и снимок долга приезжают модулем настройки проверок. Второе такое же заводится без
  сигнала: повтора не видит ни сверка раскладки — файл не разложен, — ни проверка повторов,
  которая ищет совпадение текста. Признак один, и он читается: до того как в своей проверке
  заводится разбор списка, чтение настройки или сравнение с накопленным, смотрят, что
  экспортирует модуль настройки проверок.
- **Правило, загруженное инструментом, бывает показано не целиком, и обрыв ничем не помечен.**
  ...
  кончается осмысленным пунктом. Бьёт обрыв ровно по хвосту, а хвост — то место, где дерево
  отменяет умолчания пакета: разделы надстройки сливаются по совпавшему заголовку и новыми уходят
  в конец. Отсюда порядок: решение о порядке работы принимается после того, как показанные строки
  сверены с длиной файла одной командой, а расходятся они — файл дочитывается чтением.
