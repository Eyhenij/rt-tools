...
---
<!-- rt-kit v0.16.1 · skills/agent-kit.md · 06e816708977 · правится надстройкой, не здесь -->
...
без неё дерево не скажет, какие из его надстроек исправленная редакция закрыла, и надстройка
остаётся навсегда, молча замещая исправленный раздел пакета. Снимать нечего — так и пишется;
пустой четвёртая строка не бывает.
...
одноразовым кодом приглашения — код заводит дерево в приёме и кладёт токен в файл, названный ключом `token`. Выданный
в админке токен кладётся тем же входом и в сеть не идёт вовсе. До приглашения отправка отказывает по устройству, а не
по недонастройке.
...
- **От отказа ключом `skip` это отличается тем, что ничего не убирает.** Отказ не кладёт файл
  роли вовсе, и гард при ней продолжает отбивать; здесь наоборот — роль на месте, молчит гард.
- **Настройка, которую не прочитать, роль не выключает.** Не нашлось `jq`, нет настройки, сломан
  разбор — гард судит, как судил: обратный выбор гасил бы слой правил молча.
- **Ключ, написанный не списком, ловит раскладка, а не гард.** Строка вместо списка читается
  гардом как «роль включена»; отказывает на этом разбор настройки, называя ключ.
- **Выключение живёт в настройке дерева, а не в настройке агента.** Вырезанное из настроек агента
  руками теряется на первой же правке того файла; здесь оно объявлено в одном месте.
