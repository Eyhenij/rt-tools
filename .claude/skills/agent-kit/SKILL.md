...
---
<!-- rt-kit v0.14.0 · skills/agent-kit.md · c7cce655fdd5 · правится надстройкой, не здесь -->
...
## Ловушки гейта правил

- **Гейт берёт пути у пишущего куска команды целиком, и путь, названный в записываемом тексте,
  требует правила наравне с целью записи.** Второй ярус гейта собирает пути у тех кусков, которые
  пишут, а тело записи от своей команды не отрывается: путь записи интерпретатора стоит именно
  там. Из-за этого запись документа, в прозе которого названы файлы фронтового слоя, требует
  правил этого слоя, а поиск по дереву, сцепленный с записью, отдаёт свои пути как цели.
  Сузить разбор нечем: `p='<путь>'` и текст привязки лежат в теле оба в кавычках, и признака
  между ними нет — а без тела гейт перестанет ловить запись через интерпретатор, ради которой
  ярус и заведён. Приём: запись и текст с путями разводятся на два вызова, либо текст кладётся
  во временный файл вне дерева и переносится оттуда.
- **Отбитие гейта стоит ровно столько же, сколько настоящее срабатывание.** Обойти отказ нечем —
  он кончает ход, — и правило грузится независимо от того, к правке оно или нет. Отбитая команда
  повторяется целиком: гейт судит текст, а не остаток.
