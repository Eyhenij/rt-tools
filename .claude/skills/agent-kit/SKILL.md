...
---
<!-- rt-kit v0.16.1 · skills/agent-kit.md · 5655da038cd6 · правится надстройкой, не здесь -->
...
прочитан: вторая статья о том же дороже, чем её отсутствие. Отбитый блок остаётся лежать с
отметкой «отбито» и причиной.

Четвёртая пишется затем, что предложение уезжает наружу, а надстройка остаётся здесь: без неё
дерево не скажет, какие из его надстроек исправленная редакция закрыла, и надстройка остаётся
...
а токен выдаёт одноразовым кодом приглашения — код заводит дерево в приёме и кладёт токен в файл,
названный ключом `token`; выданный в админке кладётся тем же входом. До приглашения отправка
отказывает по устройству, а не по недонастройке.

**Признак дерева считает отправитель, а не человек.** Он выводится из адреса репозитория:
дерево, заведённое своим словом, приёмник не узнаёт и отвечает отказом на каждой отправке. Что
уедет и под каким признаком, печатает прогон отправки вхолостую.
...
Отправка повторяется, и оба исхода значат для отправителя одно — груз доехал. Отметка только на
принятое объявила бы неотправленным то, что дошло раньше, и файл остался бы без неё навсегда.

**Чистое дерево после отправки значит, что весь груз в приёме уже лежал:** ставить отметки было
не на что, а не отправка не сработала.
...
называть можно — он в истории есть.
...
того же — исполняется отправкой в тот же ход.** Сухой прогон отправкой не является: следа наружу
он не оставляет. Отметки, которые отправка пишет в файлы предложений, ложатся вторым коммитом в
ту же ветку — это их место, а не повод отложить. Стережёт это гард предложения на завершении
хода.
