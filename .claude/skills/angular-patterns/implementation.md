...
| Подписка гасится вместе с владельцем.                       | `tools/lint-rules/rules/require-take-until-destroyed.cjs:module` — отказ на любой `.subscribe()` без завершающего оператора в трубе; 0 нарушений в дереве, единственное отступление подписано комментарием у ручки Redux DevTools. |
| Источник действия носит суффикс `Source` в имени.           | `tools/lint-rules/rules/require-source-suffix-for-subjects.cjs:REQUIRED_SUFFIX` — отказ на поле-источник без суффикса; наблюдаемое, поднятое из него через `asObservable()`, суффикса не носит.                                    |
