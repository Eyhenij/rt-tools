...
- **A field of an answer the whole application shares is checked at the place of assembly, not beside one operation.** — `apps/message-bus-admin-e2e/src/invites-list.spec.ts:SC-MB-409` — the refusal code is read from the screen the person sees. The tests next to the operations stayed green while the code reached no answer.
...
- **A test about a value's absence is built on a value that cannot exist, not on one that is merely not filled in yet.** — `libs/message-bus-admin/common/core/util/src/lib/admin-text.service.spec.ts:ключНиОткуда` — the key is made up on the spot. Before it the cases held `columnTree`, and the filled English set turned them red.
...
- **A new check's list is written together with the check for a dead entry in it.** — `tools/check-admin-texts.mjs:dead` — an accepted place nothing answers to refuses the check. It costs the same as a new word past the dictionary.
