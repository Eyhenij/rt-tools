...
---
<!-- rt-kit v0.16.1 · rules/git-workflow.github.md · 052ac939eb64 · правится надстройкой, не здесь -->
...
- **Ветка следующей работы отводится от предыдущей, пока череда не прервалась.** `git checkout -b
  <КЛЮЧ>-<номер>-<slug> <предыдущая ветка>` вместо `origin/main`. Отводится от главной только
  первая работа череды и та, что предыдущей не касается. Иначе первое же слияние делает
  расходящимися все остальные разом — замер в холодной части.
...
- **Череда вливается снизу вверх, и порядок стоит в теле каждой заявки.** Владелец вливает по
  списку, а родство веток по списку не видно: строка «стоит на #<номер>» — единственное место,
  где он это прочтёт.
- **Нижняя ветка череды историю не переписывает — ни `rebase`, ни силовой отправкой.** Хостинг
  закрывает заявку верхней как слитую при пустой главной. Отставшая нижняя чинится вливанием
  главной в неё и дальше вверх.
- **Расхождение внутри череды разбирается тем, кто ветвится.** У него обе правки под рукой и обе
  свои; у владельца, разбирающего то же при слиянии, нет ни одной.
...
- **Свои черновики судятся все разом, а не тот один, чья ветка сейчас взята.** Проверка по
  текущей ветке молчит там, где работа брошена. Спрашиваются все открытые черновики машинной
  записи, и каждый судится двумя признаками — зелёный прогон на его вершине и разобранная папка
  задачи в его ветке; лежащая папка означает идущую работу. Дерево, не назвавшее машинной записи,
  этого не получает.
...
- **Набор гейта пуша не бывает уже набора конвейера.** Шаг конвейера, которому в наборе гейта
  нет ни строки, ни объявленного исключения с причиной, отбивает пуш, а не печатается рядом:
  напечатанное предупреждение исполнитель читает как разрешение.
...
- **Набор гейта зовёт умолчание пакета, а не перечисляет его строками.** Переписанный строками,
  он закрывает сегодняшнюю дыру и оставляет завтрашнюю: проверка, которую пакет заведёт
  следующей редакцией, до дерева не доедет, и не хватится её ничто — полнота набора судится по
  именам шагов конвейера, а проверки, которой нет и в конвейере, там нет тоже. Отсеивать из
  умолчания законно, но поимённо и с причиной у каждого рода: своя строка покрывает то же
  строже либо зовёт названный скрипт.
...
- **Указателю, в который ветки только дописывают строки, объявляется сложение обеих сторон.**
  Конфликт там не спор: обе допись нужны целиком.
...
- **Личность вызова, открывающего заявку, стережёт гард поставки, а не память исполнителя.** Она
  приходит окружением, и в тексте команды её видно только явной подстановкой токена — её гард и
  требует. Ответ хостинга об авторе он спрашивает на снятии черновика: влитую заявку не
