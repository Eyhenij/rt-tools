...
---
<!-- rt-kit v0.14.0 · rules/git-workflow.github.md · d607ccf61277 · правится надстройкой, не здесь -->
...
- **Свои черновики судятся все разом, а не тот один, чья ветка сейчас взята.** Проверка по
  текущей ветке меняет предмет суждения вместе с переключением и молчит ровно там, где работа
  брошена: прогон по оставленной заявке доходит, черновик стоит, главная уходит вперёд, и через
  два часа работа встречает владельца конфликтом. Спрашиваются поэтому все открытые черновики
  машинной записи, и каждый судится двумя признаками — зелёный прогон на его вершине и
  разобранная папка задачи в его ветке; лежащая папка означает идущую работу. Дерево, не
  назвавшее машинной записи, этого не получает: спрашивать список не у кого.
...
- **Конфликтующий открытый PR — расхождение сверки очереди работ.** Конфликт приезжает в
  ...
  устаревает в ту минуту, когда владелец влил соседнюю работу. Гард судит один ход, а PR стоит в
  очереди днями, и по списку конфликт не виден вовсе: метку хостинг показывает только внутри
  самого PR. Непосчитанная сливаемость расхождением не считается — хостинг считает её заново
  после каждой правки главной, и строка на «ещё не посчитано» краснела бы на каждой вершине.
...
- **Указателю, в который ветки только дописывают строки, объявляется сложение обеих сторон.**
  Конфликт там не спор: обе допись нужны целиком, а человека он зовёт при каждом слиянии — за
  один заход одна строка разрешалась шесть раз в шести ветках. Объявление снимает ручное
  разрешение, но **метку сливаемости на странице заявки не гасит**: хостинг считает её своим
  приёмом и настроек слияния не читает — гаснет она, когда ветка вобрала главную и это уехало на
  хостинг. Обещать «конфликтов больше не будет» нельзя: не будет ручной работы.
- **Правка, доведённая до коммита, доводится до хостинга тем же ходом.** Владелец видит прежнее
  состояние и читает его как «не сделано ничего», а сделанное лежит в рабочем дереве, где его не
  видит никто; разрешённый конфликт — частный случай. Оставшееся в дереве называется причиной —
  словом владельца, а не списком остатков, отданным ему как его же хвост.
- **Черновик не снимается, пока у PR нет разбора.** Гард поставки смотрит запрошенного ревьювера
  ...
  без него клиент берёт PR текущей ветки, и требование снималось бы одним пробелом. Возврат в
  черновик — `gh pr ready --undo` — требования не получает: он делает то же, чего гард добивается.
...
- **Личность вызова, открывающего заявку, стережёт гард поставки, а не память исполнителя.**
  ...
  требования не получает. Ответ хостинга об авторе он спрашивает на снятии черновика: это
  последний ход, где промах исправим — влитую заявку не переоткрыть, а автора у неё не сменить.
  Прежде обе стороны держались статьёй, и промах повторился на третий день.
...
- **Каждый коммит вклада ветки подписан машинной записью, и это проверяется набором пуша.** Гард
  поставки судит только коммит, назвавшийся ею, и ищет расхождение в числе служебного адреса;
  подписанный человеком целиком проходит мимо него. Там, где человек не коммитит вовсе, это
  промах: пять коммитов одной стопки уехали подписанными владельцем, который правки не делал.
- **Состояние заявки перечитывается у хостинга сразу после публикации, а не берётся из кода
  ...
  или не сделали ничего: клиент отвечает от той записи, чей токен ему дали, а запрос разбора на
  автора хостинг принимает молча и не создаёт.
...
- **Ревьюверы спрашиваются вызовом REST, а не выборкой клиента.** Её поля собираются запросом
  GraphQL, а тот требует области чтения организации, которой у машинной записи нет: под её
  токеном проверка падает целиком, без него проходит зелёной — и «сошлось» перестаёт отличаться
  от «спросить было нечем».
- **Силовая отправка ветки, стоящей под другой в стопке, закрывает её заявку как слитую.**
  Хостинг считает заявку слитой, когда вершина её ветки становится достижима из базы, и отправка
  нижней ветки это устраивает. Состояние лживое: в главной ветке правки нет, а задача остаётся
  открытой.
