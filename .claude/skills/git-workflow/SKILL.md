...
---
<!-- rt-kit v0.13.0 · rules/git-workflow.github.md · 075adcf35ce0 · правится надстройкой, не здесь -->
...
- **Прогон, вытесненный из очереди конвейера, сверка называет отдельной строкой.** Ждущего
  прогона группа очереди не бережёт: следующий встающий вытесняет прежний, и в списке тот
  выглядит упавшим, хотя ветку не проверял ни строчкой. Отличает их число заданий — у
  вытесненного оно ноль. Строка называет обе команды по порядку: чтение прогона, потом
  перезапуск.
