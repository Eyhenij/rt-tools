...
---
<!-- rt-kit v0.22.0 · rules/git-workflow.github.md · 7da160fdcf60 · правится надстройкой, не здесь -->
...
- **Сверка раскладки стоит в наборе гейта пуша наравне с линтом и сборкой.** Правка мимо источника
  копится молча, а сверки нет в конвейере.
...
- **Автор PR не может быть его ревьювером.** Запрос разбора на себя GitHub принимает и молча не
  создаёт: разбор выглядит запрошенным. Считаются только запрос и отзыв не от автора.
- **Вызов клиента хостинга идёт из дерева, а связка команд исхода не проверяет.** Вне дерева
  клиент отказывает, а следующее звено связки увозит на хостинг остаток отказавшего: тело
  читается обратно тем же ходом.
- **Метки, исполнитель и ревьювер PR ставятся вызовами `gh api`, а не `gh pr edit`:** на старой
  борде тот отвечает отказом про Projects.
...
- **Почта машинного коммита копируется из компаньона, а не набирается по памяти.** Адрес
  `<число>+<логин>@users.noreply.github.com` сходится по числу: с чужим числом коммит уезжает
  подписанным посторонним. Ту же строку держит профиль дерева, и гард отбивает пуш.
...
- **Сценарии гардов задают настройки git сами, а не берут их с машины.** Подпись с машины уводит
  git в агент ключей, а заблокированный агент роняет весь набор: автор, почта и подпись идут
  флагами `-c` прямо в команде.

- **Каждый коммит вклада ветки подписан машинной записью, и это проверяется набором пуша.** Гард
  судит только коммит, назвавшийся ею: подписанный человеком проходит мимо.
...
- **Автор открытой заявки и наличие у неё ревьювера сверяются очередью работ.** До слияния оба
  промаха не видны: заявка, открытая человеком, ревьювера не получит никогда — и выглядит как
  заявка, которой его не поставили.
- **Ревьюверы спрашиваются вызовом REST, а не выборкой клиента.** Её поля собирает GraphQL, прав
  на который у машинной записи нет: «сошлось» не отличить от «спросить было нечем».
