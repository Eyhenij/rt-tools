...
---
<!-- rt-kit v0.24.0 · rules/git-workflow.github.md · cb767e562a72 · правится надстройкой, не здесь -->
...
- **Незаданный ключ отбивает работу с очередью на месте.** Модуль борды отказывает при первом же
  вызове и называет, где ключ задаётся. Умолчания нет намеренно: заголовок из пустого значения
...
- **Открытие PR отбивается, пока ветка везёт папку своей задачи.** Кнопку слияния нажимает
  человек там, где гардов нет: открытие — последняя точка, где отказ ещё виден исполнителю.
...
- **Сборка входит в набор наравне с линтом и юнитами.** Линтер типов не читает, а юниты читают
  ...
  есть до мержа.
- **На машине, где раннеров несколько, любой путь от домашнего каталога общий.** Соседнее задание
  переписывает умолчание готового шага под свою версию, пока наше стоит между шагами. Каталог
  установки, имя контейнера и имя сборщика называются поэтому по проекту и постоянно, а временный
  каталог решением не бывает — в нём живёт хранилище пакетов.
...
- **Итоговый набор перед пушем читается разбором состояния, а не сборкой в уме.** Раздел «набор
  перед пушем» печатает его целиком и называет рядом то, что умолчание печатало, а в набор не
  попало: дерево, писавшее надстройку вслепую, дописывало в неё повтор.
...
- **Вызов клиента хостинга идёт из дерева, а связка команд исхода не проверяет.** Вне дерева
  клиент отказывает, а следующее звено увозит на хостинг остаток отказавшего.
...
- **Сценарии гардов задают настройки git сами, а не берут их с машины.** Подпись с машины уводит
  git в агент ключей, а заблокированный агент роняет набор: автор, почта и подпись идут флагами
  `-c` в команде.
