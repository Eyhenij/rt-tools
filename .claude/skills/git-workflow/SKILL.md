...
---
<!-- rt-kit v0.18.0 · rules/git-workflow.github.md · e5b7684b867e · правится надстройкой, не здесь -->
...
- **Метки, исполнитель и ревьювер PR ставятся вызовами `gh api`, а не `gh pr edit`.** На старой
  борде `gh pr edit` отвечает отказом про Projects.
...
- **Почта машинного коммита копируется из компаньона, а не набирается по памяти.** Служебный адрес
  `<число>+<логин>@users.noreply.github.com` сопоставляется по числу: коммит с чужим числом уезжает
  подписанным посторонним. Ту же строку держит профиль дерева, и гард отбивает пуш.
- **Личность машинной записи подтверждается ответом хостинга, а не узнаванием строки.** Спрашивают
  её токеном, ответом о себе: логин и число приходят вместе.
- **Сценарии гардов задают настройки git сами, а не берут их с машины.** Подпись, включённая на
  машине, уводит git в агент ключей, а заблокированный агент роняет весь набор: автор, почта и
  подпись идут флагами `-c` прямо в команде.

- **Каждый коммит вклада ветки подписан машинной записью, и это проверяется набором пуша.** Гард
  судит только коммит, назвавшийся ею; подписанный человеком проходит мимо, а там, где человек не
  коммитит вовсе, это промах.
- **Состояние заявки перечитывается у хостинга сразу после публикации, а не берётся из кода
  возврата.** Открытие и постановка ревьювера отвечают нулём и тогда, когда не сделали ничего.
- **Автор открытой заявки и наличие у неё ревьювера сверяются очередью работ.** До слияния оба
  промаха не видны: заявка, открытая человеком, ревьювера не получит никогда, и выглядит это как
  заявка, которой его просто не поставили.
- **Ревьюверы спрашиваются вызовом REST, а не выборкой клиента.** Её поля собирает GraphQL, а он
  требует прав, которых у машинной записи нет: «сошлось» не отличить от «спросить было нечем».

- **Рабочее дерево не опустошается ради прогона инструмента.** Сравнение с прежним состоянием
  идёт на второй копии: прятанье уводит незакоммиченную работу туда, где её не видит ни состояние
  дерева, ни сверка.
