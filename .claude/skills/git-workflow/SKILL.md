...
---
<!-- rt-kit v0.17.0 · rules/git-workflow.github.md · 1cd22e0351c7 · правится надстройкой, не здесь -->
...
- **Задача, оставшаяся в первой колонке, PR не открывает.** По очереди работ она читается как
  невзятая, хотя работа сделана и выложена. Имя первой колонки дерево называет само; не названо —
  колонка не судится.
- **На борде стоят задачи, а не PR о них.** Карточка PR колонки не имеет и из очереди не уходит
  никогда; заводит её правило борды или рука, и то и другое выключается. Находит такие карточки
  сверка очереди.
