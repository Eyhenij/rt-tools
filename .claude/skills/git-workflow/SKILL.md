...
---
<!-- rt-kit v0.16.1 · rules/git-workflow.github.md · 9ddce897f857 · правится надстройкой, не здесь -->
...
- **Ветка следующей работы отводится от предыдущей, пока череда не прервалась.** `git checkout -b
  <КЛЮЧ>-<номер>-<slug> <предыдущая ветка>` вместо `origin/main`. От главной отводится первая
...
- **Свои черновики судятся все разом, а не тот один, чья ветка сейчас взята.** Проверка по
  текущей ветке молчит там, где работа брошена. Каждый черновик машинной записи судится двумя
  признаками — зелёный прогон на его вершине и разобранная папка задачи в его ветке.
...
- **Набор гейта зовёт умолчание пакета, а не перечисляет его строками.** Переписанный строками,
  он закрывает сегодняшнюю дыру и оставляет завтрашнюю: заведённая пакетом проверка до дерева не
  доедет, и не хватится её ничто. Отсеивать из умолчания законно, но поимённо и с причиной у
  каждого рода.
- **Сверка раскладки стоит в наборе гейта пуша наравне с линтом и сборкой.** Правка мимо
  источника копится молча и всплывает на чужой работе: раскладка отказывает по правленому файлу
  целиком. Статьёй выше это не покрывается — сверки нет в конвейере.
...
Первый ярус остаётся и работает без сети: локальная ссылка отвечает, отстала ли ветка от
лежащего в дереве, удалённая — не протухла ли сама ссылка.
