...
---
<!-- rt-kit v0.16.1 · rules/git-workflow.github.md · 83961b4a4b94 · правится надстройкой, не здесь -->
...
- **Готовность отданной работы живёт до следующего слияния в главную.** Влитая соседняя делает
  свою конфликтующей, а сказанное час назад «доведено до ready» читается как действующее. После
  каждого слияния сливаемость своих заявок спрашивается заново — одним вызовом на все.
- **Черновик при зелёном прогоне на вершине — расхождение сверки.** Зелёная страница черновика
  владельцу ничего не разрешает: кнопка заблокирована хостингом. Гард закрывает это внутри хода,
  между ходами заявку находит сверка.
...
- **Открытие PR отбивается, пока ветка везёт папку своей задачи.** Уборка стоит до открытия:
  кнопку слияния нажимает человек там, где гардов нет. Открытие — последняя точка, где отказ ещё
  виден тому, кто может его выполнить.
...
- **Почта машинного коммита копируется из компаньона, а не набирается по памяти.** Служебный
  адрес — `<число>+<логин>@users.noreply.github.com`, и сопоставляется он по числу: коммит с чужим
  числом уезжает подписанным посторонним. Ту же строку держит профиль дерева, и гард отбивает пуш
  при расхождении.
