...
---
<!-- rt-kit v0.14.0 · rules/git-workflow.github.md · 2fd1ba145453 · правится надстройкой, не здесь -->
...
- **Черновик при зелёном прогоне на вершине — расхождение сверки.** У черновика кнопка слияния
  ...
  котором всё серое, читается как «работа не сделана». Гард снятия черновика закрывает это
  только внутри хода: между ходами — ночью, в чужой сессии, после закрытого разговора — заявка
  стоит без него, и находит её сверка.
- **Свои черновики судятся все разом, а не тот один, чья ветка сейчас взята.** Уход в соседнюю
  ветку заявку не закрывает: прогон по ней доходит, черновик остаётся, главная уходит вперёд, и
  через два часа работа встречает владельца конфликтом. Проверка по текущей ветке этого не
  видит вовсе — она меняет предмет суждения вместе с переключением ветки и молчит ровно там, где
  работа брошена. Спрашиваются поэтому все открытые черновики машинной записи, и каждый судится
  двумя признаками: прогон на его вершине завершён успехом и папку своей задачи ветка больше не
  везёт. Ход работы у чужой ветки при этом не читается — лежащая папка означает, что работа там
  идёт, и черновик при ней законен. Дерево, не назвавшее машинной записи, этого не получает:
  спрашивать список не у кого.
