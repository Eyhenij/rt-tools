...
- **Условие, известное в начале работы, спрашивается в начале.** Заведение ветки отбивает
  основание без вершины главной и чужую почту в подписи. На пуше и на открытии PR те же проверки
  остаются вторым рубежом, но стоят дороже: основание чинится вливанием с конфликтом, подпись —
...
- **Заведённая задача подтверждается ответом очереди работ, а не выводом команды заведения.**
  Напечатанный номер значит «вызов прошёл»: команда может завести задачу и не довести её до
  борды. Спрашивается очередь по номеру, и ответ читается колонкой и исполнителем.
...
- **Отказ читается прежде, чем его обходят вторым способом.** Он бывает единственным ранним
  признаком ограничения записи, и обход вызовом другого рода уносит признак с собой: расхождение
  всплывает у владельца. Ограничение от исчерпания отличает ответ о пределах — там ноль.
...
- **Черновик при зелёном прогоне на вершине — расхождение сверки.** У черновика кнопка слияния
  заблокирована хостингом: зелёная страница владельцу ничего не разрешает. Гард закрывает это
  внутри хода, между ходами заявку находит сверка.
...
- **Открытие PR отбивается, пока ветка везёт папку своей задачи.** Уборка стоит до открытия:
  кнопку слияния нажимает человек там, где гардов нет. Отказ на слиянии остаётся вторым рубежом,
  но открытие — последняя точка, где отказ ещё виден тому, кто может его выполнить.
- **Конфликтующий открытый PR — расхождение сверки очереди работ.** Конфликт приезжает чужим
  слиянием, без единого действия автора, и по списку не виден: метку хостинг показывает только
  внутри самого PR. Непосчитанная сливаемость расхождением не считается.
...
- **Автор PR не может быть его ревьювером.** Запрос разбора на самого себя GitHub принимает и
  молча не создаёт. В разборе считаются только запрос и отзыв не от автора.
...
- **Почта машинного коммита копируется из компаньона, а не набирается по памяти.** Служебный
  адрес — `<число>+<логин>@users.noreply.github.com`, и сопоставляется он по числу: коммит с
  чужим числом уезжает подписанным посторонним, а изнутри промах не виден. Ту же строку держит
  профиль дерева, и гард отбивает пуш при расхождении.
...
- **Состояние заявки перечитывается у хостинга сразу после публикации, а не берётся из кода
  возврата.** Открытие заявки и постановка ревьювера отвечают нулём и тогда, когда не сделали
  ничего.
...
- **Ревьюверы спрашиваются вызовом REST, а не выборкой клиента.** Её поля собираются запросом
  GraphQL, а тот требует прав, которых у машинной записи нет: «сошлось» перестаёт отличаться от
  «спросить было нечем».
