...
---
<!-- rt-kit v0.22.0 · rules/git-workflow.github.md · d4260cbb3b03 · правится надстройкой, не здесь -->
...
- **Ветка следующей работы отводится от предыдущей, пока череда не прервалась.** `git checkout -b
  <КЛЮЧ>-<номер>-<slug> <предыдущая ветка>` вместо `origin/main`; от главной — только первая работа
  череды. Иначе первое же слияние делает расходящимися все остальные разом.
...
- **Условие, известное в начале работы, спрашивается в начале.** Заведение ветки отбивает основание
  без вершины главной и чужую почту в подписи. На пуше те же проверки стоят дороже: основание
  чинится вливанием с конфликтом, подпись — переписыванием ветки.
...
- **Сценарии гардов задают настройки git сами, а не берут их с машины.** Подпись с машины уводит
  git в агент ключей, а заблокированный агент роняет набор: автор, почта и подпись идут
  флагами `-c` прямо в команде.
...
- **Состояние заявки перечитывается у хостинга сразу после публикации.** Открытие и постановка
  ревьювера отвечают нулём и тогда, когда ничего не сделали.
- **Автор открытой заявки и наличие у неё ревьювера сверяются очередью работ.** До слияния оба
  промаха не видны: заявка, открытая человеком, ревьювера не получит никогда.
- **Ревьюверы спрашиваются вызовом REST, а не выборкой клиента.** Её поля собирает GraphQL, прав
  на который у машинной записи нет: «сошлось» неотличимо от «спросить было нечем».
