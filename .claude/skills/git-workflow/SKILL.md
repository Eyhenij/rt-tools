...
---
<!-- rt-kit v0.23.0 · rules/git-workflow.github.md · f17e47018c9e · правится надстройкой, не здесь -->
...
- **Заявка, чья база — не главная ветка, проверяется тем же набором, что и заявка в главную.**
  Условие запуска конвейера читает базу, и заявка в соседнюю ветку не поднимает его вовсе: на её
  странице нет ни красного, ни зелёного, а пустое поле проверок читается как ожидание очереди.
  Стопка заявок законна, и цена у неё одна: проверку с них снимает не решение автора, а условие
  запуска, о котором никто не спрашивал. Спрашивается оно до открытия первой заявки стопки — один
  вызов по описанию конвейера, — и заявка без прогона на вершине не сливается, даже когда её
  содержимое разобрано и одобрено.
...
- **Конфликтующая своя заявка чинится первым действием хода, а новая работа до этого не
  берётся.** Пока отданное конфликтует, влить его человек не может, и каждая следующая заявка
  прибавляет к очереди ещё одну, которую придётся догонять. Работой здесь считается заведение
  задачи, заведение ветки, перевод колонки в работу и открытие заявки; всё, чем конфликт чинится,
  идёт как прежде.
...
- **После вливания главной ветки набор проверок пересматривается по тому, что ветка везёт
  ...
  целиком.
- **Главная ветка берётся удалённой ссылкой — и в словах, и в действиях.** Локальная протухает в
  минуту, когда её подтянули, и молчит об этом: это снимок вчерашнего дня. От `origin/main` идут
  и сравнение, и основание новой ветки, и счёт влитого, и подтягивание — смешанные в одной
  команде, ссылки дают промах, изнутри выглядящий верным. Влитая ветка при локальной числится
