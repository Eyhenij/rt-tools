...
- **Work begins with the epic, and the branch of a task is taken from the epic branch.** — `tools/task-new.mjs:epicBranchOf` — the creation command prints the branch line with the branch of the epic as its base. The base is read from the header of the epic plan. The branch itself is created by a call of its own, and the delivery guard reads that base against the main branch: of the epic it knows nothing. Held further by the order of the pattern `git-workflow-commit`.
- **The PR of a task has the epic branch as its base — `gh pr create --base <ветка эпика>`.** — **Not checked by anything yet.** The guard judges the number in the title, the folder of the task and the freshness of the base, and lets any base through. Held by the order of the pattern `git-workflow-pr`.
- **The PR of an epic into the main branch opens after all its tasks are merged and their folders are taken apart.** — **Not checked by anything yet.** The task folder guard reads the branch of one task; that the tasks of an epic are over is known to the epic plan and to the work queue. Held by the state `задачи-эпика-кончились` of the rule `task-flow`.
