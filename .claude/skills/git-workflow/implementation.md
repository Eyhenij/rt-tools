...
- **Work begins with the epic, and the branch of a task is taken from the epic branch.** — **Not checked by anything yet.** The delivery guard reads the base of a branch against the main branch and knows nothing of the epic of a task; the epic of a task is named by the card and by the epic plan. Held by the order of the pattern `git-workflow-commit`.
