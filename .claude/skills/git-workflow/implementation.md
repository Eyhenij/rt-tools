...
- **A PR in a chain has the previous branch as its base, not main.** — `.claude/hooks/git-guard-delivery-epic.sh:rt_epic_pull_base` — inside an epic the base is read from the command text and matched against the epic's branch: accepted are the epic branch itself and any branch standing on it, that is a neighbouring branch of the same chain; a missing `--base` counts as main, which is exactly the case the article is about. Outside an epic the base is checked by nothing — which of the two is right is known only by whoever branched, and for work unrelated to the previous one main is lawful.
...
- **`--hard` is not taken to drop a commit — that is `--soft`.** — `.claude/hooks/git-guard-discard.sh:hits` — the discarding verb is read from the command text. The tree is read by `git status --porcelain` in the directory of the call. The bypass is the comment `# discard: <reason>` in the same command.
...
- **The main branch is merged into the epic branch, and the epic branch into the branches of its tasks — while the work runs, not before the hand-over.** — `.claude/hooks/git-guard-delivery.sh:remote_main` — the tip of the main branch is asked of the remote ref and matched against the current branch's ancestors. The freshness of a branch of an epic is judged by `.claude/hooks/git-guard-delivery-epic.sh:rt_epic_base`. It asks the tip of the base too, so a merge not sent yet answers by the base.
...
- **An open task standing in a closing column is a discrepancy.** — `tools/check-board.mjs:CLOSING_STATUSES` — «✅ Done» and «Deployed», by the keys `done` and `deployed` of `board.statusOptions`. A line per such task; it names the return move `npm run task:move -- <number> <column>`.
...
- **The epic card moves by the same command, on the epic number, at two moments.** — `tools/board.mjs:moveTask` — the call on the epic number right after the first task's branch is created. The second call — right after the PR of the epic opens. No guard reads the epic card's column.
