...
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Второй экземпляр уже поднятого приложения не поднимается.      | Приложения здесь нет: единственный стенд — витрина, и поднимает её агент сам (`pnpm run storybook`, `pnpm run storybook:ui-kit-v2`). Гарда второго экземпляра не разложено, держится порядком: `lsof -nP -iTCP:6006 -sTCP:LISTEN` и то же по `6007` до подъёма. Занятый порт означает уже поднятую витрину, а не повод взять соседний; отдельный экземпляр под прогон снимков поднимается своим портом и передаётся через `STORYBOOK_URL`. |
