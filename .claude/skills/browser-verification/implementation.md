...
- **Готовое из набора, взятое под данные другого размера, меряется, а не разглядывается.** — **Не проверяется ничем.** Ни линтер, ни сборка, ни снимок витрины не знают, каким значением компонент наполнят у потребителя; снимок ловит это только там, где для нового места заведён свой кадр
- **Прокси стенда подставляет то, по чему приложение узнаёт арендатора запроса.** — **Не применимо.** Арендаторов у библиотеки нет: единственный стенд — витрина, и запрос в ней ничьим не бывает.
