...
---
<!-- rt-kit v0.23.0 · rules/doc-style.md · e4ba364fbdd1 · правится надстройкой, не здесь -->
...
- **Файл, положенный раскладкой, пары не требует.** Автор у него в дереве-потребителе один —
  пакет, и документ о нём живёт там же. Требование пары читает шапку раскладки: она стоит в
  каждом ведомом файле и отличает его от написанного здесь надёжнее любого перечня путей. Иначе
  первая же раскладка упирается в обход на весь свой объём, а обход, объявленный однажды на сотню
  файлов, снимает требование и со всех будущих правок этих файлов руками.
  <!-- rt-when: *.md -->
...
- **У текста есть адресат, и слог выбирается по нему, а не по тому, что писалось до него.**
  Правила, законы и спеки читает тот, кто внутри слоя правил; задачу, заявку и ответ в чате —
  человек снаружи. Текст, написанный сразу за правкой спеки, наследует её слог молча: изнутри он
  читается хорошо написанным — слова точны и называют ровно то, что названо, — а пустыми они
  оказываются для того, кто их читает. Адресат проверяется прежде, чем написана первая строка.
