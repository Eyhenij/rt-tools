...
| Своя работа не ложится в ветку чужой задачи.                                                              | **Не проверяется ничем.** Гард ведения работы судит объявленное состояние, а не то, чья это ветка: папка чужой задачи по имени ветки лежит и снимает отказ так же, как своя. Держится хуком запуска сессии — он называет ветку и говорит, есть ли у неё папка |
