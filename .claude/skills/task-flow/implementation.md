...
| Находки разбора ждут владельца, а в пакет уезжает только сводка наблюдений.                                | `npx agent-kit propose` шлёт сводку и то, что лежит в `.claude/rt-kit/proposals/`; находки разбора кладёт туда человек — после разбора |
