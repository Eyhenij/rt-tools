...
- **Which epic this working copy leads is read from the table of assignments, and the owner writes it.** — `tools/tree-assignment.mjs:assignmentFault` — the table is `.claude/rt-kit/assignments.md` in the main branch. The name of the copy lies in `.claude/rt-kit/tree-name` outside history. The refusal comes from `.claude/hooks/tree-assignment-guard.sh` on a created task and a moved card, and from `.claude/hooks/git-guard-delivery.sh` on a branch. The same reading is printed at session start by the `SessionStart` hook in `.claude/settings.local.json`
- **An assignment stands until the owner moves it, and a closed epic in the row is not an order.** — `.claude/hooks/git-guard-tree-assignment.sh:rt_assignment_stale` — the state of the epic is asked of the board by `rt_task_state`. The board is silent — there is no refusal
