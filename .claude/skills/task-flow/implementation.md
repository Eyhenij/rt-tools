...
- **Два требования — два гарда, и снять одно можно, не теряя второго.** — `.claude/hooks/task-flow-draft-guard.sh:draft` — договорённость спрашивает свой гард; папку задачи, замысел и состояние — `.claude/hooks/task-flow-guard.sh:progress`, и общий разбор вызова у обоих один — `.claude/hooks/task-flow-context.sh:rt_task_flow_context`
...
- **Договорённость требуется по путям правки, а не по оценке задачи.** — `.claude/hooks/task-flow-context.sh:rt_tf_candidates` — судятся пути правки; обход — строка о поведении в шапке замысла, и читает её `.claude/hooks/task-flow-draft-guard.sh:plan`
