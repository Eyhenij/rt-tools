...
- **Work begins with the epic, and a task outside one is not taken.** — `tools/board-epics.mjs:checkTasksOutsideEpics` — the audit names an open task whose body declares neither an epic nor the owner's word about work outside one. The work-start guard knows nothing of the epic of the work: the card is judged after it is created, not before.
