...
- **A task of an epic is linked to the epic card as a sub-issue, not only named in the plan.** — `tools/task-new.mjs:epicOfCard` — the creation command links the task to the epic card right after it puts it on the board. What is created past the command, and every task created before this order came in, is named by `tools/board-epics.mjs:checkEpicSubIssues`: it reads the makeup from the plan and asks the hosting for the links of the epic card.
