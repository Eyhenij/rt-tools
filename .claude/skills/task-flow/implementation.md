...
- **Work begins with the epic, and a task outside one is not taken.** — **Not checked by anything yet.** The work-start guard reads the owner's word about starting work and knows nothing of the epic of that work; the epic of a task is named by the card and by the epic plan in `docs/plans/`.
- **The epic branch is taken before the first task of the epic, not with it.** — **Not checked by anything yet.** The delivery guard judges the base of a branch against the main branch: a task branched from the main branch instead of the epic branch is lawful to it.
- **The PR of the epic opens when its last folder is taken apart, and not a task earlier.** — `.claude/hooks/git-guard-delivery-folder.sh:rt_delivery_open_folder` — the guard refuses opening while the branch carries a task folder; that the tasks of the epic are over it does not read, and that is held by the state `задачи-эпика-кончились`.
