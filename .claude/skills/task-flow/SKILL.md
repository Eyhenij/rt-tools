...
---
<!-- rt-kit v0.23.0 · rules/task-flow.md · ca6e475eccab · правится надстройкой, не здесь -->
...
следующую задачу, а не на открытый PR. Блокированная задача следующей не считается по той же
причине. Следующая берётся из очереди работ, а очередь спрашивается командой: «задач нет» —
утверждение о дереве, и подтверждается оно выводом, а не памятью исполнителя.
**Открытая карточка о взятости работы не говорит.** Карточку закрывает слияние: невзятой задачу
делает отсутствие следа в дереве, а не строка в списке. Открытые номера сверяются с описанием
прошлого — на закрытую работу там лежит запись, названная её номером.
...
- **Гард судит объявленный переход, а не наличие файлов.** Пустой замысел лежит так же, как
  ...
  коммитом, означает отданную работу.
...
- **На команду владельца ответ начинается с результата, а не с намерения и не с его обоснования.**
  Обоснование при уже принятом чужом решении читается его оценкой. Исключение одно — исполнение
  упёрлось в препятствие: тогда называется препятствие, а не отношение к задаче.
- **Заход работу не начинает сам.** Начало требует слова владельца в этом же заходе: передача,
  ...
  том, работать ли. Судится это формой реплики, а не смыслом.
...
- **Слово владельца об устройстве — постановка, а не решение.** Названное им обычно уже живёт в
  ...
  ними до правки. Разошлось прочтение — спрашивается владелец.
...
- **Папка задачи едет в ветку коммитом, а не живёт в одном рабочем дереве.** Незакоммиченная, она
  проходит все правки без отказа, а отказ приходит в последней точке, когда замысел уже снят. Вне
  истории законен один черновик без номера.
- **Папка задачи разбирается последним коммитом до открытия PR, а не после одобрения.** Человек
  вливает, как только видит зелёное, и закрывающему коммиту места не остаётся. Цена: замысла на
  диске нет, и признак отданной работы гард берёт из истории ветки.
...
- **Указание работать по правилу — это указание делать его шаги, включая меняющие историю.**
  Отметка этапа, коммит, пуш и открытие заявки предписаны правилом: спросить надо о том, чего в
  нём нет.
...
- **Ожидание одной части этапа остановкой этапа не бывает.** Независимые от ожидаемого части
  делаются тем же ходом; отбитая команда читается так же — сперва делается всё, что мимо неё.
- **Находка, сделанная посреди этапа, сверяется с условиями выхода замысла до первой правки.**
  ...
  идёт в раздел о том, чего работа не делает, и заводится задачей. Сверяется перечень, а не
  ощущение «это же то же самое»: оно приходит именно тогда, когда предмет соседний.
...
- **Работа, заказанная словами, становится задачей в очереди тем же ходом.** Даже если делать её
  будут не сейчас. Черновик папки очередью не бывает: номера у него нет, и знает о нём один
  заход, который его завёл. Отложенная работа называет срок вслух — отложенная молча читается
  обеими сторонами как сделанная.
...
- **Замысел эпика называет, как стоят ветки его задач, наравне с их порядком.** Расстановок две:
  ...
  ничего. Стопка стоит дороже, и цена у неё названная: заявка в соседнюю ветку не запускает
  ...
  заново. Записанная в замысле расстановка делает это решением эпика, а не решением того, кто в
  этот день заводит ветку.
...
Ничто из самого разбора не проверяется. Гард видит правку файла и не знает ни о разведке до
первого вопроса, ни о шести обязательных, ни об ответах на них: пустая таблица проходит так же,
как заполненная.
