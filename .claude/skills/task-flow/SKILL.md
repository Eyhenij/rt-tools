...
---
<!-- rt-kit v0.10.0 · rules/task-flow.md · 8fded44686eb · правится надстройкой, не здесь -->
...
- **Порог, на котором инструмент сжимает контекст, дерево задаёт само и сводит с порогом
  остановки.** Там, где передачу пишет хук перед сжатием, порог сжатия — это и есть порог, на
  котором заход закрывается. Оставленный инструменту, он приходит почти у предела окна: заход
  останавливается стражем на своей доле и до сжатия не доживает, а значит и передачи не
  получает. Задаётся он теми же двумя числами, что читает страж, — размером окна и долей, — и
  сведены они или нет, говорит разбор состояния раскладки. Сжатие при этом приходит чуть раньше
  остановки: долю инструмент считает от рабочего окна, из которого уже вычтен запас на ответ.
  Порядок именно такой и нужен — заход сжимается сам, а страж остаётся страховкой на случай,
