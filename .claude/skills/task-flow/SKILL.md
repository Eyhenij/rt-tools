...
---
<!-- rt-kit v0.23.0 · rules/task-flow.md · 345e667bf9cc · правится надстройкой, не здесь -->
...
**Открытая карточка о взятости работы не говорит.** Карточку закрывает слияние, и в заходе, где
заявки не открываются, список открытых не убывает ни на строку, сколько работ ни сделай. Невзятой
задачу делает отсутствие следа в дереве, а не строка в списке: открытые номера сверяются с
описанием прошлого — на закрытую работу там лежит запись, названная её номером. Сверка стоит одной
команды и отвечает про обе стороны сразу, а список открытых — ни про одну.
...
- **Правка кода приложения отбивается, пока работа не дошла до состояния, в котором код
  правится.** Гард хода работы требует три вещи: папку задачи по имени ветки, замысел в ней и
  объявленное в ходе работы состояние. Договорённость о продукте требует свой гард на те же
  события.
- **Гард судит объявленный переход, а не наличие файлов.** Пустой замысел лежит так же, как
  ...
  `разбор-кончился`. Четвёртый путь не состояние, а история ветки: папка, разобранная её
  коммитом, означает отданную работу, и правка по замечаниям идёт без замысла на диске.
...
- **Именем состояния считается только слово из перечня.** Своё слово не говорит ни о входе, ни о
  выходе, ни об обязательном действии.
...
- **Что требует слова владельца, берётся списком, а не оценкой на месте.** Оценку «это безопасно»
  назначает тот, кому она удобна. Недовольство владельца простоем разрешением не бывает.
- **На команду владельца ответ начинается с результата, а не с намерения и не с его обоснования.**
  Владелец, поставивший задачу, мнения о ней не запрашивал, и обоснование при уже принятом чужом
  решении читается его оценкой: авторство переписывается на исполнителя. Исключение одно —
  исполнение упёрлось в препятствие: тогда называется препятствие, а не отношение к задаче.
- **Заход работу не начинает сам.** Начало требует слова владельца в этом же заходе: передача,
  состояние из хука запуска и назначенный эпик говорят, что делать, если работать, и молчат о
  том, работать ли. Судится это формой реплики, а не смыслом: правка кода после пустой реплики,
  одного слова или одного пути ход не заканчивает.
...
- **Указание владельца действует до его отмены, и новый факт против него — строка в ответе о
  цене, а не новый вопрос.** Переспрашивают то, чего указание не покрывает; обстоятельство,
  меняющее цену указания, но не смысл, называется вслух, и работа идёт дальше. Меню, где два
  варианта из трёх предлагают отменить решение владельца, и есть такая отмена. Правило о развилке
  говорит, КАК задавать вопрос, и молчит о том, задавать ли его вовсе.
...
- **Слово владельца об устройстве — постановка, а не решение.** Названное им обычно уже живёт в
  дереве под этим самым словом — именем на экране, разделом спека, полем модели, — и сверяется с
  ними до правки. Разошлось прочтение — спрашивается владелец: истолкованное по ближайшему коду
  выглядит исполненной просьбой ровно до приёмки.
...
- **Папка задачи едет в ветку коммитом, а не живёт в одном рабочем дереве.** Незакоммиченная,
  она проходит все правки без отказа, а отказ приходит в последней точке, когда замысел уже снят:
  признак отданной работы берётся из истории. Вне истории законен один черновик без номера.
...
- **Открыв PR, исполнитель называет владельцу номер, чего ждёт и что сделает следом.** Ждёт он
  ...
  заблокированной у черновика кнопке слияния.
...
- **Гард замысла — нижняя граница требования, а не его предел.** Он требует папку только под
  правку кода приложения, а статья выше — под любую работу. Прочитанный как признак, он
  становится разрешением работать без замысла везде, куда не смотрит.
- **Указание работать по правилу — это указание делать его шаги, включая меняющие историю.**
  Отметка этапа, коммит, пуш и открытие заявки правилом предписаны: спросить надо о том, чего в
  нём нет. Ход, доведший работу до готовности и вставший перед коммитом, её не отдал, а спрятал.
- **Взятая задача ходом не кончается.** Заведение задачи, ветки, колонки и папки — подготовка:
  она переводит работу в состояние записанного замысла, где действие уже другое. Держит это страж
  выхода хода, называя в отказе первый этап.
- **Ожидание одной части этапа остановкой этапа не бывает.** Независимые от ожидаемого части
  ...
  команда читается так же: сперва делается всё, что мимо неё.
- **Находка, сделанная посреди этапа, сверяется с условиями выхода замысла до первой правки.**
  ...
  командами, которые этот слой зовёт, откатываются порознь. Не названа ни одним условием выхода —
  идёт в раздел о том, чего работа не делает, и заводится задачей. Своего признака у этого нет и
  быть не может: ощущение «это же то же самое» приходит именно тогда, когда предмет соседний, —
  поэтому сверяется перечень, а не ощущение. Однажды такую правку остановил посторонний отказ по
  роду файла, а не сверка с замыслом: без него она уехала бы в чужую ветку молча.
...
- **Работа, заказанная словами, становится задачей в очереди тем же ходом.** Даже если делать её
  будут не сейчас. Черновик папки очередью не бывает: номера у него нет, лежит он вне истории и
  невидим ни борде, ни сверке очереди, ни следующему заходу — знает о нём один заход, который его
  завёл. Порог брошенного разбора эту потерю не ловит: работа проигрывает соседним поручениям в
  тот же час, а неделя проходит потом. Отложенная работа называет срок вслух — отложенная молча
  читается обеими сторонами как сделанная.
...
- **Задачи эпика заводятся все разом, тем же ходом, что и сам эпик.** Заведение по одной прячет
  объём. Номера возвращаются в раздел порядка той же правкой: без них «взять следующую» отвечает
  названием, а не карточкой.

- **Замысел эпика называет, как стоят ветки его задач, наравне с их порядком.** Расстановок две:
  каждая ветка от главной либо стопкой — каждая от предыдущей. Порядок задач о ней не говорит
  ничего: задачи, идущие одна из другой по коду, законно живут ветками от главной, пока правка
  следующей не опирается на код предыдущей. Стопка стоит дороже, и цена у неё названная: заявка в
  соседнюю ветку не запускает конвейер, объявленный на базу главной; слияние базовой ветки
  закрывает заявку следующей как слитую, хотя её правок в главной нет; конфликт от чужого слияния
  разрешается в каждой ветке стопки заново. Записанная в замысле расстановка делает это решением
  эпика, а не решением того, кто в этот день заводит ветку: выбранная молча, она собирает всю цену
  и не показывает ни одной её части.
...
- **Дешёвый шаг закрытия идёт раньше дорогого, а прогон — после вливания.** Вливание
  договорённости и приведение текстов стоят минуты, прогон — окно захода; идущий до вливания, он
  смотрит то, что в главную не поедет.
...
- **Открытие PR и снятие черновика отбиваются, пока ветка везёт папку своей задачи.** Слияние
  нажимает человек на хостинге, где гардов нет: снятый черновик он читает приглашением влить.
...
Ничто из самого разбора не проверяется. Разговор с владельцем инструментом не идёт: гард
...
последней реплики владельца, а не смысл — вопрос о другом предмете, названный прежними словами,
он отобьёт. Отказ называет законный ход: назвать то, чего в прежнем ответе владельца нет.
