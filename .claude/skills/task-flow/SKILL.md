...
---
<!-- rt-kit v0.16.1 · rules/task-flow.md · 2478a8709a3c · правится надстройкой, не здесь -->
...
- **Что требует слова владельца, берётся списком, а не оценкой на месте.** Оценку «это
  безопасно» назначает тот, кому она в эту минуту удобна, и меняется она вместе с настроением
  разговора, а не со свойствами команды: за один заход одна и та же команда была сначала слишком
  опасной, чтобы её позвать, а через два хода — достаточно безопасной, чтобы позвать без спроса.
  Недовольство владельца простоем разрешением не бывает: оно говорит, что стоять не следовало, а
  не что можно то, чего нельзя было ходом раньше.
...
- **Папка задачи едет в ветку коммитом, а не живёт в одном рабочем дереве.** Гард спрашивает её
  ...
  открытии заявки, когда папка уже разобрана. Второе следствие тише — ход работы, живущий в
  рабочем дереве, не виден ни владельцу, ни следующему заходу. Вне истории законен один
...
- **Папка задачи разбирается последним коммитом до открытия PR, а не после одобрения.** Кнопку
  ...
  зелёное: закрывающему коммиту места не остаётся. Цена — замысла с этой минуты на диске нет, и
...
- **Открыв PR, исполнитель называет владельцу три вещи: номер, чего ждёт и что сделает следом.**
  Ждёт он прогона, следом идёт снятие черновика. Зелёный прогон однозначным не бывает: он
  говорит, что не сломано, и молчит о заблокированной у черновика кнопке слияния.
...
- **Задачи эпика заводятся все разом, тем же ходом, что и сам эпик.** Заведение по одной прячет
  ...
  названием, а не карточкой. Метку эпика несёт только его карточка.
...
- **Находки разбора ждут владельца, а в пакет уезжает только сводка наблюдений.** Предложение —
  заготовка правки чужого дерева: уехавшее без разбора, оно становится работой того, кто её не
