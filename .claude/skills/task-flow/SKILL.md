...
---
<!-- rt-kit v0.14.0 · rules/task-flow.md · 3a32aff0c495 · правится надстройкой, не здесь -->
...
- **Договорённость требуется по путям правки, а не по оценке задачи.** `apps/**` и `libs/**`
  ...
  принимается.
- **Своя работа не ложится в ветку чужой задачи.** Ветка названа номером — значит за ней стоит
  свой замысел, свой разбор просьбы и своя отдача; правка, приехавшая в неё со стороны, уедет
  вместе с чужой работой или не уедет вовсе. Узнаётся такая ветка тем же, чем и всякая другая, —
  папкой задачи по её имени: сказанное хуком запуска про отсутствие папки — признак чужой ветки,
  а не предупреждение о чужой работе.
