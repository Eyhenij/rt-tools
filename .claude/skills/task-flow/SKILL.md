...
---
<!-- rt-kit v0.20.0 · rules/task-flow.md · 7fc230a781be · правится надстройкой, не здесь -->
...
- **Строка состояния, переведённая вперёд, — то же объявление намерения, только машиночитаемое.**
  Пустоты за ней не видно никому: состояние объявляется тем ходом, в котором его обязательное
  действие начато делом, а не тем, в котором о нём отчитались.
- **Отказ по состоянию называет обязательное действие объявленного состояния.**
...
- **Что требует слова владельца, берётся списком, а не оценкой на месте.** Оценку «это безопасно»
  назначает тот, кому она удобна, и меняется она с настроением разговора, а не со свойствами
  команды. Недовольство владельца простоем разрешением не бывает.
...
- **Указание работать по ходу работы покрывает все его шаги, включая меняющие историю.** Отметка
  этапа, пуш и заявка предписаны им самим: спрашивают о том, чего в нём нет.
...
- **Слово владельца об устройстве — постановка, а не решение.** Названное им обычно уже живёт в
  дереве под этим самым словом: у него есть имя на экране, раздел в спеке и поле в модели. Прежде
  чем строить, слово сверяется с ними, а разошлось прочтение с деревом — спрашивается владелец, до
  правки: истолкованное по ближайшему коду, оно выглядит исполненной просьбой ровно до приёмки.
...
- **Папка задачи едет в ветку коммитом, а не живёт в одном рабочем дереве.** Незакоммиченная,
  ...
  в последней точке, когда замысел уже снят. Вне истории законен один черновик без номера.
...
- **Открыв PR, исполнитель называет владельцу три вещи: номер, чего ждёт и что сделает следом.**
  Ждёт он прогона, следом идёт снятие черновика. Зелёный прогон говорит, что не сломано, и молчит
  о заблокированной у черновика кнопке слияния.
...
- **Указание работать по правилу — это указание делать его шаги, включая те, что меняют
  историю.** Отметка сделанного этапа, коммит, пуш и открытие заявки правилом предписаны, и
  отдельного слова владельца на каждый из них не требуется: спросить надо о том, чего в правиле
  нет. Ход, который довёл работу до готовности и встал перед коммитом, работу не отдал, а спрятал.
...
- **Ожидание одной части этапа остановкой этапа не бывает.** Части, которые от ожидаемого не
  зависят, делаются тем же ходом, а владельцу называется, что сделано и что осталось на его шаг.
  Отбитая гардом команда и отклонённый вызов читаются так же: сперва делается всё, что мимо них.
...
- **Эпик не закрывается по признаку, подтверждённому только чтением.** Признак, проверяемый
  глазами, называется проверенным лишь вместе с командой или замером и их выводом.
...
- **Замысел эпика лежит там, где его найдут без сети и после мержа.** Карточка порядка задач не
  ...
  правила. Имя, адрес и способ, названные владельцем по ходу, дописываются туда же тем ходом,
  каким приняты.
...
- **Что действует на дерево, а не на правку, лежит вне индекса.** Путь к образцу и разрешение
  работать вне эпика ветке не принадлежат: записанные файлом репозитория, они в ней и остаются,
  а следующая, отведённая от главной, их не несёт. Место записи — рядом с передачей захода.
- **Закрытая работа разбирается правилами, и это шаг закрытия, а не отдельная просьба.** Что
  ...
  кончается правкой слоя правил или предложением наружу; ведёт его роль разбора закрытой задачи,
  если дерево её разложило.
...
- **Дешёвый шаг закрытия идёт раньше дорогого, а прогон — после вливания.** Вливание договорённости
  и приведение текстов стоят минуты, прогон — окно захода. Идущий после вливания, он проверяет и его
  самого: идущий до, он смотрит то, что в главную не поедет.
...
- **Открытие PR и снятие черновика отбиваются, пока ветка везёт папку своей задачи.** Слияние
  нажимает человек на хостинге, где гардов нет: снятый черновик он читает как приглашение влить и
  кнопку нажимает, не дожидаясь коммита уборки. Судится содержимое ветки, а не рабочее дерево.
