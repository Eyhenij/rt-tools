...
---
<!-- rt-kit v0.10.0 · rules/task-flow.md · 4be1146c676a · правится надстройкой, не здесь -->
...
- **Передача прошлого захода приходит в контекст тем же запуском, что и состояние работы.**
  Берётся она по имени текущей ветки: чужая, поданная как своя, описывает работу, которой в
  этом дереве нет. Передачи нет — вход об этом молчит: ветка, по которой заход ещё не
  закрывался, это обычное начало работы, а не поломка.
- **Карта хода приходит тем же запуском и лежит своим файлом, а не вынимается из правила.**
  ...
  зашитая в хук — расходится с правилом без единой правки. Короче правила она обязана быть:
  этим она и полезна, и предел ей назначает проверка дерева.
- **Вход в заход подаётся на всех четырёх запусках, а не только после сжатия.** Заход после
  обрыва и заход после очистки начинают с того же пустого места, и разница между ними
  исполнителю не видна вовсе.
...
- **Заполнение окна захода стережёт гард, а не память исполнителя.** На первом пороге он
  ...
  стража не получает.
- **Порог сжатия контекста дерево задаёт само, теми же числами, что читает страж.** Там, где
  передачу пишет хук перед сжатием, это и есть порог закрытия захода; оставленный инструменту,
  он приходит почти у предела окна, и остановленный раньше заход до передачи не доживает.
  Сведены пороги или нет, говорит разбор состояния раскладки.
