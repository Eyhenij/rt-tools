...
---
<!-- rt-kit v0.8.2 · rules/task-flow.md · 45cb038e3f9a · правится надстройкой, не здесь -->
...
flowchart TD
    ...
    E --> G[Задача, ветка, папка задачи по имени ветки]
    F --> G
    ...
    K --> L[Пока PR ждёт разбора, берётся следующая задача]
    L --> M{Разбор и прогон кончились}
    M -->|Да| N[Папка задачи разбирается последним коммитом: разбор просьбы — в описание прошлого, замысел — прочь]
    N --> O[Черновик снимается, слияние нажимает человек]
```
