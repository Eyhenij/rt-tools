...
---
<!-- rt-kit v0.16.1 · rules/task-flow.md · 16af8ee51641 · правится надстройкой, не здесь -->
...
захода состоянием тоже не бывает: заход
...
- **Папка задачи разбирается последним коммитом до открытия PR, а не после одобрения.** Кнопку
  слияния нажимает человек на хостинге, и вливает он, как только видит зелёное, — закрывающему
  коммиту места не остаётся вовсе. Цена: замысла с этой минуты на диске нет, и признак отданной
  работы гард берёт из истории ветки.
...
- **Замысел эпика лежит там, где его найдут без сети и после мержа.** Карточка говорит, что эпик
  есть, но порядка задач не держит, а папка задачи держала бы его до слияния первой из них.
...
- **Закрытая работа разбирается правилами, и это шаг закрытия, а не отдельная просьба.** Что
  ...
  а дерево, её не разложившее, ведёт разбор само.
...
- **Ветка, снёсшая папку, обязана прибавить запись в архив.** Снести дешевле, чем разобрать, и
  первым уходит разбор просьбы — единственная запись слов владельца. Что именно увезено, судит
  владелец.
- **Обход — строка `Task-folder-skip: <причина>` в PR или в самой команде.** Работа, вливаемая
  частями, папку до конца не разбирает. Пустая причина обходом не считается, а сам обход снимает
  отказ, но не гасит строку сверки очереди — иначе он через месяц становится рабочим путём.
