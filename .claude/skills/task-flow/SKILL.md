...
---
<!-- rt-kit v0.16.1 · rules/task-flow.md · e7541d955b9b · правится надстройкой, не здесь -->
...
**Требует:** `hooks/task-flow-guard.sh`, `hooks/task-flow-draft-guard.sh`, `hooks/task-context-load.sh`, `hooks/grill-gate.sh`, `hooks/window-fill-guard.sh`, `hooks/turn-exit-guard.sh`
...
- **Правка кода приложения отбивается, пока работа не дошла до состояния, в котором код
  ...
  события.
- **Гард судит объявленный переход, а не наличие файлов.** Пустой замысел, положенный ради
  снятия отказа, лежит так же, как написанный, поэтому отказ снимает объявленное состояние —
  `этап-идёт`, `этапы-кончились`, `разбор-кончился`. Четвёртый путь не состояние, а история
  ветки: папка, разобранная её коммитом, означает отданную работу, и правка по замечаниям идёт
  без замысла на диске.
...
- **Два требования — два гарда, и снять одно можно, не теряя второго.** Одним файлом отказ от
  требования договорённости снимал заодно папку задачи — то, о чём дерево не просило. Судят
  порознь: строка о неизменном поведении снимает договорённость, а не требование дойти до
  правки кода.
- **Влитая договорённость ветку не запирает.** После вливания директории «предложено» на диске
  ...
  истории ветки. Иначе последний коммит PR закрывал бы дорогу правкам по замечаниям.
...
- **Папка задачи заводится под любую работу, без исключений.** Исключение, у которого есть хоть
  одна законная форма, исполняется как разрешение. Заводится она всегда и до первой правки:
  сколько заходов уйдёт на работу, заранее не знает никто.
- **Папка задачи едет в ветку коммитом, а не живёт в одном рабочем дереве.** Гард спрашивает её
  с диска, и папка, ни разу не закоммиченная, проходит все правки без единого отказа — а признак
  отданной работы берётся из истории, и там её нет. Отказ приходит в последней точке, на
  открытии заявки, когда папка уже разобрана своими руками: чинить нечего, замысел снят, и
  собирать его приходится заново по памяти. Второе следствие тише: ход работы, живущий в
  рабочем дереве, не виден никому — владелец видит ветку без единого следа того, что в ней
  делается, а следующий заход находит пустоту вместо «Где стоим». Вне истории законен один
  черновик без номера: заводить в неё нечего, пока нет имени.
- **Папка задачи разбирается последним коммитом до открытия PR, а не после одобрения.** Кнопку
  слияния нажимает человек на хостинге, куда гард не достаёт, и вливает он, как только видит
  зелёное, — закрывающему коммиту места не остаётся вовсе. Цена перестановки: замысла с этой
...
- **Взятая задача ходом не кончается.** Заведение задачи, ветки, колонки и папки — подготовка, а
  ...
  другое. Граница состояния выглядит законченным куском лучше всякой вехи, а владельцу отчёт о
  взятой задаче неотличим от остановки. Держит это страж выхода хода, называя в отказе первый
  этап замысла.
...
- **Папка задачи заводится черновиком и получает номер командой.** До конца разбора неизвестно,
  ...
  шапку раскладки: оставленная в копии, она отбивает первую же правку разбора просьбы.
- **Брошенный разбор виден.** Черновик старше недели перечисляет сверка очереди работ — задачи
  за ним ещё нет, и спросить о нём некого.
- **Следующая задача берётся из замысла эпика, а список очереди работ спрашивается только там,
  где эпика нет.** Список отсортирован номером, и по нему первая задача чужого эпика неотличима
  от своей. Замысел открывается перед взятием; кончившийся эпик называется владельцу тем же
  ходом, которым берётся работа вне его.
...
- **Задачи эпика заводятся все разом, тем же ходом, что и сам эпик.** Заведение по одной прячет
  объём: на борде эпик из десяти задач выглядит одной. Номера возвращаются в раздел порядка той
  же правкой — без них «взять следующую» отвечает названием, а не карточкой. Метку эпика несёт
  только его карточка.

- **Замысел эпика лежит там, где его найдут без сети и после мержа.** Карточка говорит, что эпик
  есть, но порядка задач не держит; папка задачи держала бы его ровно до слияния первой из них.
  Каталог называет компаньон правила: замысел, положенный каждым заходом заново, теряет решения
  предыдущих.
- **Сборка по образцу начинается с чтения самого образца, а не пересказа о нём.** Пересказ лежит
  в разборе просьбы и в замысле эпика; они читаются, но образцом не считаются. Повторяемая часть
  открывается целиком — обходом каталогов, а не одним файлом, за которым пришли.
...
- **Закрытая работа разбирается правилами, и это шаг закрытия, а не отдельная просьба.** Что
  ...
  сутки этого нет ни у кого. Разбор кончается правкой слоя правил или предложением наружу. Ведёт
  его роль разбора закрытой задачи, если дерево её разложило; не разложившее ведёт разбор само.
- **Разбор закрытой работы уходит в фон, а исполнитель берёт следующую задачу.** Роль работает
  своим ходом и ничего не спрашивает; держать ради неё заход незачем. Сводку собирают до запуска,
  вернувшиеся находки принимают одним ходом: записать и продолжить прежнее.
- **Находки разбора ждут владельца, а в пакет уезжает только сводка наблюдений.** Предложение —
  ...
  заказывал. Сводка наблюдений уезжает всегда: она говорит, чем пользовались, и мнением не
  бывает.
- **Дешёвый шаг закрытия идёт раньше дорогого, а прогон — после вливания.** Вливание
  договорённости и приведение текстов стоят минуты, прогон — окно захода: потративший его на
  прогон упирался в порог заполнения на четырёх строках привязки. Прогон, идущий после вливания,
  вдобавок проверяет и его самого — идущий до, он смотрит то состояние дерева, которое в главную
  ветку не поедет.
...
- **Обход — строка `Task-folder-skip: <причина>` в PR или в самой команде.** Работа, вливаемая
  частями, папку до конца не разбирает. Чтение из команды работает и без сети. Пустая причина
