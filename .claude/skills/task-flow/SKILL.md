...
---
<!-- rt-kit v0.23.0 · rules/task-flow.md · 59b984b5e811 · правится надстройкой, не здесь -->
...
- **На команду владельца ответ начинается с результата, а не с намерения и не с его обоснования.**
  Обоснование при уже принятом чужом решении читается как его оценка. Исключение одно —
  исполнение упёрлось в препятствие: тогда называется препятствие.
- **Заход работу не начинает сам.** Начало требует слова владельца в этом же заходе: передача,
  состояние из хука запуска и назначенный эпик говорят, что делать, и молчат о том, работать ли.
...
- **Слово владельца об устройстве — постановка, а не решение.** Названное им обычно уже есть в
  ...
  правки. Разошлось прочтение — спрашивается владелец.
...
- **Папка задачи едет в ветку коммитом, а не живёт в одном рабочем дереве.** Незакоммиченная,
  она проходит все правки без отказа, а отказ приходит в последней точке, когда замысел уже
  снят. Вне истории законен один черновик без номера.
...
- **Взятая задача ходом не кончается.** Заведение задачи, ветки, колонки и папки — подготовка:
  она переводит работу в состояние записанного замысла, где действие уже другое. Это проверяет
  гард завершения хода, называя в отказе первый этап.
