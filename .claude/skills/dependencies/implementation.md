...
- **Версия пакета записана точным номером.** — `package.json:dependencies` — ни одной записи с `^` или `~`; строка запроса в `pnpm-lock.yaml` совпадает с манифестом, и `pnpm install --frozen-lockfile` принимает лок без правки. Подмены чужих версий в `pnpm-workspace.yaml` остаются диапазонами намеренно: их смысл — «не ниже этой версии» для транзитивных зависимостей, которых в манифесте нет вовсе, и точный номер там закрепил бы уязвимую ветку.
- **Подменённые версии чужих зависимостей собраны в один список, и его пересматривают при каждом обновлении.** — `pnpm-workspace.yaml:overrides` — восемнадцать записей, все под уязвимости сборочного графа; список пересматривается при каждом подъёме версий.
- **Свежая версия сначала выдерживается, а нужная раньше срока выписывается отдельно.** — **Не исполняется** до конца: выдержка задана `minimumReleaseAge`, а отдельного списка «нужно раньше срока» в дереве нет — исключение пишется строкой в том же файле.
- **Переформатируется только то, чьё форматирование проверяет линтер.** — `.claude/rt-kit/project.sh:rt_lint_for` — набор линтеров дерева; переформатируется то, что они читают, и ничего сверх того.
- **Пакет, который везёт оформление, поднимается своей задачей.** — **Не исполняется** здесь: дерево пакетов оформления не читает, оно их публикует. Отдельная задача заводится командой заведения задачи; проверки на снятые свойства у дерева нет намеренно — свои свойства оно объявляет само.
