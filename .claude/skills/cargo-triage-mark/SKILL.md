...
kind: pattern
rule: cargo-triage
description: Паттерн правила cargo-triage. Брать при разборе приехавшего груза: готовые вызовы чтения и отметки — забрать неразобранное, сухой прогон, пачка записей за вызов, приём починки и версия выпуска, разбор отбитой строки.
---
...
Ставится он шагом состояния `влито` — там, где слияние уже случилось, а ход о задаче ещё идёт;
готовый вызов стоит в паттерне `task-flow-archive`. Отставшую отметку находит `npm run
cargo:fixed`: она читает взятые в работу и называет те, у которых работа кончилась — заголовок
статьи стоит в источниках либо полный ключ назван в описании прошлого. Ключ подставляется
полным: восьми знаков команда отметки не принимает.
...
```bash
npm run cargo:mark -- --state released --postmortem <файл> --release '0.10.1'
```

Версия — та, которой назван выпуск, увёзший починку. Не номер редакции приёмника и не время
выкатки.

Называется она одним номером: имя пакета колонка уже знает — оно стоит у самой записи, — а
приписанное второй раз, оно разъезжается формами. Две формы одного выпуска, `rt-agent-kit@0.17.0`
и `@rt-tools/agent-kit 0.17.0`, простояли в приёме рядом, и отбора по версии на них не собрать
ни одного. Приём этого не судит: длину версии он держит, а форму оставляет дереву.

## Чужая запись: закрывает издатель редакции

Своим токеном чужая запись не двигается вовсе: приём берёт дерево из токена и отвечает по ней
так же, как по ненайденной. Записи соседа, чья правка вошла в редакцию пакета, закрывают другой
командой — под входом человека, той же парой учётной записи службы, какой читают груз:

```bash
npm run cargo:close -- --state fixed --proposal <признак в приёме> --fix 'статьёй правила'
npm run cargo:close -- --state released --proposal <признак в приёме> --release '0.17.0'
```

Признак записи в приёме — не ключ отметки: тем свою запись двигает её дерево, этим чужую
закрывает издатель. Печатает его чтение груза — строкой `в приёме <признак>` рядом с записью.

Закрытием ставятся только `fixed` и `released` и только вперёд: `in_work` означает взятую
дереву работу, и ставит его дерево.
