---
name: browser-verification-measure
kind: pattern
rule: browser-verification
description: Pattern of rule browser-verification. Load when a conclusion about layout needs a number behind it — ready-made measurements, a computed value broken down over all nodes, a narrow screen through an iframe, pitfalls of the computer tool. Not for raising a stand — pattern browser-verification-stand.
---
<!-- rt-kit v0.25.0 · patterns/browser-verification-measure.md · b69a9bc335f6 · правится надстройкой, не здесь -->

# Замер вместо взгляда

Паттерн правила `browser-verification`. Что при этом должно быть верно — закон
`docs/constitution/verifiability.md`.

## Когда брать

- Правилась вёрстка, и надо подтвердить результат.
- Экран выглядит не так, а искать нечего: поиск по коду отвечает «чисто».
- Проверяется узкий экран.

## Вывод подкрепляется числом

`getComputedStyle`, `getBoundingClientRect`, контраст, совпадение центров, попадание во
вьюпорт. «Выглядит нормально» результатом проверки не бывает.

Замер отвечает только на тот вопрос, который задали. Совпадение перечисленных свойств ничего
не говорит о правиле, которого в списке замера нет: строки попапа профиля сошлись с образцом
по отступам, кеглю и скруглению, а фон на наведении образец в этом месте не красит вовсе —
полноширинная подсветка держалась два круга при верных числах. Если исходники образца
доступны, расхождение ищется чтением, а замер остаётся проверкой результата.

Вид элемента, которого сегодня не видно ни на одном экране, замером не подтверждается, и
правило о нём остаётся гипотезой.

## Состояние ожидания меряется на задержанном запросе

Кнопка в ожидании, скелет списка и полоса загрузки живут ровно столько, сколько идёт запрос: на
стенде это доли секунды, и застать их вызовом замера нечем. Задерживается сам запрос, а не замер.

Приём из трёх частей, и третья обязательна: сетевой вызов подменяется, ответ придерживается на
несколько секунд, а кончается отказом. Ответ настоящий оставил бы на стенде настоящую запись —
так в базу стенда и уехала лишняя строка.

```javascript
globalThis.__realFetch ??= globalThis.fetch;
globalThis.fetch = (input, init) => {
    const url = typeof input === 'string' ? input : input.url;
    if (!url.includes('<кусок адреса, который держим>')) {
        return globalThis.__realFetch(input, init);
    }
    return new Promise((resolve) => {
        setTimeout(() => resolve(new Response('{}', { status: 500 })), 5000);
    });
};

'подменено';
```

Снимается подмена присваиванием сохранённого обратно: страница живёт до перезагрузки, и
забытая подмена уводит следующий замер.

Признак того, что попали именно сюда: кнопка на снимке в обычном виде, а запрос в списке
сетевых вызовов уже ушёл. Собирать приём заново каждый раз дороже, чем прочитать его здесь: за
один эпик он собирался трижды.

## Значение, которого в коде нет

Гарнитуру, `line-height`, цвет и `appearance` элементам формы задаёт браузер, и в дереве этих
значений нет. Поиск по коду на такой дефект отвечает «чисто», линт и сборка молчат. Ищется
разбивкой вычисленного значения по всем узлам страницы, а не замером у пары элементов:

```javascript
[...document.querySelectorAll('*')].reduce((acc, el) => {
    const key = getComputedStyle(el).fontFamily;
    acc[key] = (acc[key] ?? 0) + 1;
    return acc;
}, {});
```

Так на дашборде админки нашёлся 91 контрол из 106, набранный не той гарнитурой, и 13 из 87 на
главной сайта. Счёт годится любому наследуемому свойству: смотрится не одно значение, а число
узлов с неожиданным.

## Замер по всем порогам разом

Замер по одной ширине за вызов даёт числа разных минут: между вызовами страница
перерисовывается, и сравнивать их нельзя. Ширины поэтому гоняются пачкой в одном заходе —
каждая живёт своим кадром, замер снимается со всех сразу:

```javascript
await (async (address, widths, row) => {
    const frames = widths.map((width) => {
        const frame = document.createElement('iframe');
        frame.style.cssText = `width:${width}px;height:900px;border:0;position:fixed;left:-9999px`;
        frame.src = address;
        document.body.appendChild(frame);
        return { width, frame };
    });

    await Promise.all(frames.map(({ frame }) => new Promise((done) => (frame.onload = done))));

    return frames.map(({ width, frame }) => {
        const view = frame.contentWindow;
        const doc = frame.contentDocument;
        const cells = [...doc.querySelectorAll(row)];

        return {
            asked: width,
            got: view.innerWidth,
            heights: [...new Set(cells.map((el) => Math.round(el.getBoundingClientRect().height)))],
            scrolls: doc.documentElement.scrollWidth > doc.documentElement.clientWidth,
            escaped: cells.filter((el) => el.getBoundingClientRect().left > view.innerWidth).length,
        };
    });
})('/screen', [360, 599, 600, 904, 905, 1239, 1240, 1440], '.field');
```

Ответ читается по трём признакам, и первый не про вид:

- **высоты соседей одной строки равны между собой** — набор значений в `heights` длиной больше
  единицы означает, что поля встали разного размера;
- **поперечной прокрутки нет** — `scrolls` истинно там, где содержимое шире кадра;
- **ни одна левая граница не выходит за ширину кадра** — `escaped` больше нуля. На снимке этого
  не видно вовсе: элемент за правым краем выглядит отсутствующим.

Ширина кадра меньше заказанной на полосу прокрутки, поэтому сравнивают с `got`, а не с числом
из списка: порог, названный по заказанной ширине, на кадре оказывается по другую сторону от
себя.

## Узкий экран

`resize_window` не работает, когда Chrome в полноэкранном режиме: инструмент рапортует успех,
`innerWidth` не меняется, медиазапросы остаются десктопными. Узкие ширины проверять во
вложенном iframe нужной ширины — внутри него `matchMedia` считается от рамки; при
`width: 375px` и `border: 2px` внутренний `innerWidth` равен 371.

Правка числа элементов в контейнере — это правка раскладки: она проверяется при 375, а не
только кодами ответа.

## Событие ввода подделкой значения не заменяется

**Прокрутка, поставленная присвоением, события не даёт.** Значение прокрутки меняется, а подписчик
не срабатывает ни на документе, ни на окне, ни на корневом узле: признак, который приложение
считает по событию, остаётся прежним, и сделанная правка читается как несделанная — два захода
подряд разбирали исправный слушатель.

Отсюда две проверки, и ни одна не заменяет другую: механика проверяется настоящим колесом, а
раскладка под признаком — признаком, поставленным руками. Класс ставится на хост, и меряется то,
что от него зависит; порог при этом не участвует.

Признак того, что случай именно этот: слушатель поставлен, значение прокрутки изменилось, счётчик
событий нулевой. Разбирать после этого сам слушатель не нужно — он исправен.

## Ловушки инструмента `computer`

- Координаты клика — координаты **скриншота**, а не CSS-пиксели: при вьюпорте 2560 скриншот
  приходит шириной 1568, и клик по «увиденной» координате уходит мимо, давая ложный сигнал.
  Пересчитывать по фактическому масштабу либо целиться через `find`.
- Область `zoom` должна целиком лежать внутри вьюпорта.
- Между кликами обязателен `await`: синхронный цикл «кликнул — прочитал DOM» читает состояние
  до перерисовки и возвращает устаревшие значения.
- `select_browser` протухает через 300 секунд. На длинной проверке это срабатывает посреди
  работы — это не сбой стенда, повторить вызов и продолжить. Закреплённый профиль называет само дерево — переменной окружения либо своим файлом рядом с
  настройками раскладки; спрашивается он у помощника, а не помнится: идентификатор локален для
  машины и в пакет не едет вовсе.

## Долгий прогон запускается в странице, а не в вызове

Вызов инструмента прерывается по своему пределу примерно на сорока пяти секундах, а начатый им
обход продолжается — результата у него уже никто не спросит. Одна ширина из набора экранов идёт
около полутора минут, то есть в вызов не помещается ни одна.

Порядок такой: вызов кладёт обход в переменную страницы и заканчивается сразу, а следующие вызовы
спрашивают у той же переменной, готово ли.

```javascript
globalThis.__probe = { done: false, rows: [] };
(async () => {
    for (const w of [375, 480, 481, 768, 769, 1080, 1081, 1380]) {
        globalThis.__probe.rows.push(await measureWidth(w));
    }
    globalThis.__probe.done = true;
})();

'запущено';
```

Ответ инструмента обрезается примерно на полутора тысячах знаков без пометки: конец сводки
выглядит не оборванным, а отсутствующим — вывод, собранный за один проход и напечатанный разом,
читается как более короткий, чем он есть. Собранное поэтому остаётся в переменной целиком, а
печатается срезами по индексу.

## Поведение роутера воспроизводится нажатиями

Подстановка адреса, `history.pushState` с `popstate` и заход по прямой ссылке поднимают
приложение заново, и накопленного состояния — открытого аутлета, гарда прошлой панели — у него
нет. Чистый проход по адресам читается как «дефект не подтверждается»: панель ленты событий
застревала на первом же нажатии из меню и трижды прошла проверку адресами.

## Частые промахи

- Комментарий в конфиге — гипотеза наравне с прочими. Утверждение о вытеснении записи кэша
  продержалось три круга состязательного разбора кода и было опровергнуто одним `curl`.
- Вывод «дефекта нет, это кэш» закрывает разбор, поэтому принимается только после проверки на
  чистой сборке.
- Дефект в клиентском чанке от компиляции до правки выглядит как дефект кода: признак
  дев-сборки — имена бандла без хеша.
