...
---
<!-- rt-kit v0.23.0 · patterns/browser-verification-measure.md · 6f06ac0b9cc2 · правится надстройкой, не здесь -->
...
Отсюда порядок из двух проверок, и ни одна не заменяет другую: механика проверяется настоящим
колесом, а раскладка под признаком — признаком, поставленным руками. Класс вешается на хост, и
меряется то, что от него зависит; порог при этом не участвует вовсе.

Признак того, что попал именно сюда: слушатель поставлен, значение прокрутки изменилось, счётчик
событий нулевой. Разбирать после этого сам слушатель бесполезно — он исправен.
...
## Долгий прогон живёт в странице, а не в вызове

Вызов инструмента отваливается по своему пределу примерно на сорока пяти секундах, а начатый им
...
Порядок такой: вызов кладёт обход в переменную страницы и кончается сразу, а следующие вызовы
...
Ответ инструмента режется примерно на полутора тысячах знаков, и режется молча: конец сводки
