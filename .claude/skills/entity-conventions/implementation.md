...
принимает его от деревьев. Панелей подробностей три, по одной на раздел груза, и все они стоят
на той же основе кита, что и панель правки: маршрут в аутлете `ro`, чтение записи по
идентификатору из адреса, закрытие навигацией. Поэтому половина статей правила здесь верна
буквой основы, а половина — про запись — предметом не обеспечена вовсе.
...
| сквозные спеки панели         | `apps/message-bus-admin-e2e/src/postmortems-list.spec.ts`, `apps/message-bus-admin-e2e/src/sections.spec.ts`                                |
...
| Мутация завершается перечитанным списком, а не отправленным запросом.               | Не применимо: мутаций в админке нет вовсе — груз читается, а правит его приём. Перечитывание списка при этом есть и зовётся человеком: `libs/message-bus-admin/common/core/data-access/src/lib/admin-list-store.base.ts:retry`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
