...
---
<!-- rt-kit v0.23.0 · patterns/git-workflow-commit.github.md · 6d90d59e4500 · правится надстройкой, не здесь -->
...
**Команды, которой нет, паттерн не заменяет вызовами клиента.** Дерево, взявшее пакет впервые,
получает файлы проверок, но не записи о них в своём манифесте: раскладка правит ресурсы, а
манифест потребителя ей не принадлежит. Отсюда два законных хода и ни одного третьего — завести
команду тем же ходом либо повторить все её шаги поимённо, сверяясь с перечнем выше. Обход
вызовами клиента выглядит исполнением ровно до того шага, который в перечне последний: первым
забывается он, потому что предыдущие уже дали видимый результат. Так карточка и простояла вне
...
завелась вторая ветка со своей папкой задачи и своим замыслом — то есть работа была спланирована
...
Одна строка ответа означает, что работа уже начата: её состояние читается в ходе работы её
папки, а не заводится заново. Пустой ответ говорит только про эту машину — ветка, заведённая на
другой и не отданная хостингу, не видна и отсюда.
...
- **Сверка сразу после добавления отвечает «нет», когда карточка уже стоит.** Очередь работ отдаёт
  ...
  об одной задаче снимает только администратор. Сама команда заведения при этом стоит правки:
  состояние читается сразу за мутацией, без единого повтора, и ложный отказ здесь дороже задержки.
