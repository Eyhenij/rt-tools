...
- **A field added to the answer of a refusal is added where the answer is assembled, not where the refusal is thrown.** — `apps/message-bus/src/app/failure.filter.ts:#bodyOf` — the body of the answer is assembled here. The refusal code was named by every operation and reached no answer until this method learned to put it in.
