...
- **A preset name read from a foreign theme keeps that theme's reference under the dark theme too.** — `tools/build-tokens-v2.mjs:materialDark` — the Material chains of the preset with the light step swapped for the dark answer. The theme-scope file lays them after the dark answers, on the root and on the nodes of the set
