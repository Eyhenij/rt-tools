...
- **The readability threshold is counted in every look the kit can be drawn in, not in the two themes.** — `tools/check-tokens-theme.mjs:LOOKS` — the four looks the pairs are measured in, with the accepted list in `tools/tokens-theme-allowlist.json`
