...
- **Правило кита живёт в слое каскада.** — `tools/check-cascade-layer.mjs:COMPONENT_LAYER` — обёртка файла и порядок подслоёв; правила, вынесенные из слоя нарочно, эта проверка не видит, и стережёт их проба `projects/ui-kit-v2/src/lib/components/aside/rt-aside-overlay.styles.spec.ts:split`
