...
---
<!-- rt-kit v0.16.1 · rules/turn-conduct.md · e1ed897138dd · правится надстройкой, не здесь -->
...
оно уходит в передаче. Порядок задач назначен и лежит записанным, исполнитель по нему идёт, а
тому, ради кого он идёт, состояние очереди иначе не видно ни на одном ходу. Пересказ по памяти
тут не годится вдвойне: задачу закрывает слияние, которое делает владелец, и вчерашнее состояние
врёт про уже влитое.
...
- **Свой незакрытый шаг владельцу не передаётся.** Список остатков, названный аккуратно, читается
  ...
  неотмеченная запись груза и незакрытая колонка чужими шагами не бывают. Формой вопроса это не
  чинится: вопрос законен там, где ответа нет ни в правилах, ни в дереве, а «куда едет ветка,
  которую я сам завёл» — не такой вопрос. Список остатков пишется после того, как из него
  вычеркнуто всё, что исполнитель мог закрыть сам.
...
- **Вариант, поданный владельцу, назван ценой для человека.** Сколько шагов, где человек окажется
  и что ему для этого нужно — без этого варианты выглядят равными, и выбор идёт по доводам со
  стороны кода: «меньше путей», «строку запуска не трогаем». Знание, которое делает вариант
  негодным, пишется в сам вариант, а не остаётся у того, кто его составил.
...
- **Путь к файлу заданием не бывает.** Строка с адресом называет файл, а не действие;
  прочитанная как поручение, она даёт заходу задание, которого владелец не давал. То же и с
  любой репликой, где нет глагола: уточнить дешевле, чем написать полсотни файлов мимо просьбы.
- **Прерывание работы владельцем называется вслух.** Пришло задание, останавливающее начатое, —
  исполнитель говорит, что стоит, на чём остановлено и что будет с прежней работой, и только потом
  берётся за новое. Молчание об этом владелец читает как «прежнее кончилось».
...
- **Снятая папка задачи снимает требование состояния, а ход не кончает.** Ход работы уезжает
  ...
  нельзя — снятая папка означает середину отдачи, а не её конец: между уборкой и заявкой работу не
  видит никто, кроме того, кто её сделал. Дальше ход судится вторым признаком, как всякий другой;
  ход, в котором заявку открыли или прочитали, второй признак пропускает сам.
- **Работа без ветки и без папки задачи судится тем же стражем по второму признаку.** Состояния у
  ...
  кончается и здесь.
- **Взятая задача — ещё не начатая работа, и ход на ней не кончается.** Ветка по номеру заведена,
  колонка сдвинута, номер назван владельцу — а каталога задачи при ветке нет: обязательное
  действие состояния «задача-взята» не сделано ни строкой. Второй признак такой ход отпускает
  целиком: работы в нём много, и «была ли за ход работа» отвечает не на тот вопрос, что «доведена
  ли работа до конца объявленного состояния». Судит это страж отдельным ярусом; ветка без номера
  задачи под него не подпадает, а собранный шаблон папки его снимает — заполненность замысла
  машине не видна.
- **Разведка ходом не кончается, сколько бы её ни было.** Переключение ветки, подтягивание,
  ...
  оставляет работу там же, где она стояла. Признак работы у стража этого прежде не различал: он
  знает первое слово команды, и читающая подкоманда совпадала с ним наравне с коммитом. Разведка
  при этом выглядит работой лучше всего остального — в ней команды, точные числа и проверяемые
  ответы, — поэтому ход, набитый ею, читается полным и заходом, и владельцем. Судятся части
  составной команды по одной: чтение, соединённое с правкой, работой остаётся.
...
- **Гард утверждения ждёт текст ответа, а не судит запись, какой застал.** Текст ложится в запись
  хода не раньше, чем хост зовёт хук, и гард, прочитавший файл первым, видит ход, в котором
  владельцу не сказано ничего. Не дождавшись текста, гард возвращает ход: пустая запись означает
  не «сказать было нечего», а «прочитать нечего».
...
- **Ход, отдавший работу, доводит её до снятого черновика.** Черновик читается владельцем как
  ...
  а не вместо: требование взять её исполняется буквально и оставляет отданное невидимым. Стережёт
  это гард ожидания: ход, открывший заявку, не кончается, пока состояние отданной работы не
  спрошено командой того же хода.
...
- **Ожидание своего же замера ведётся одним ожиданием, а не уведомлением на каждый шаг.**
  Уведомление на событие ставится там, где на каждое событие надо действовать; там, где значим
  только итог, ставится одно ожидание, кончающееся вместе с замером. Иначе замер сам себе
  назначает столько ходов, сколько у него шагов, и каждый из них наполнить нечем: дерево под
  серией трогать нельзя, а ход кончается пересказом того, что придёт следующей строкой.
...
- **У отказа от необратимого действия есть безопасная часть, и она делается.** Требование спросить
  ...
  делится, а не откладывается целиком. Список вариантов, поданный вместо работы, читается как
  работа — тем полнее, чем аккуратнее он составлен: он пронумерован, в нём названы цифры, и именно
  поэтому пустота хода за ним не видна. Владельцу называется, что уже сделано и что осталось за
  его словом, — а не выбор из вариантов вместо и того и другого.
- **Признак необратимости берётся из списка, а не выводится доводом.** Список составлен тем, кто
  ...
  прочитали, и применённый поверх списка довод отменяет список целиком. Отменяет молча: со стороны
  это выглядит осторожностью, а не пропуском шага. Действия, которого в списке нет, исполнитель на
  слово владельца не гейтит, даже если оно уходит наружу; шаг, снятый разбором прошлого промаха, —
  тем более.
- **Ход, в котором исполнитель признал промах, не заканчивается, пока записи о происшествии нет.**
  Отбивает гард происшествия — на завершении хода: к моменту признания промах уже случился, и
  ловить раньше нечего. Признание ловится набором образцов, а не пониманием смысла; промах,
  признанный словами вне набора, гард пропускает, и это его известная граница, а не обещание.
- **Заход, начатый с передачи, входит в работу тем же правилом, что и всякий другой.** Передача
  ...
  память: порядок, записанный только словами, исполняется, пока о нём помнят.
