...
- **A step of the plan that is not done forbids a stop, and a report about the done is not an exception.** — `.claude/hooks/turn-exit-patterns.sh:rt_te_steps_left` — the count is read from the progress
