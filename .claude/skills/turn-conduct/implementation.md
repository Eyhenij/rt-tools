...
- **Someone else's step is of two kinds, and the second never ends by itself.** — Not checked by a machine: a refused permission is indistinguishable to a guard from an ongoing run. What was not passed is named by a section of the request body — it is refused by `.claude/hooks/git-guard-delivery.sh`.
