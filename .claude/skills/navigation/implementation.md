...
Раздел сегодня один — «Обзор», и он пустой: разделы груза заводит своя задача, а пункт без
экрана ведёт туда же, куда ведёт снятый. Прав в админке нет вовсе — ни ролей, ни пресетов, —
поэтому от двухслойного гейтинга правила здесь работает только один слой: вошёл или нет.
...
| пустой раздел «Обзор»   | `libs/message-bus-admin/common/container/feature/src/lib/admin-overview.component.ts`       |
...
- Экран входа стоит вне оболочки, а не разделом внутри неё: меню и шапку невошедшему
  ...
  ветка.
- Раздел «Обзор» живёт в общем домене оболочки, а не в своём: предмета за ним пока нет, и
  домен из шести слоёв под пустой экран дал бы шесть либ, которые нечем наполнить. Уходит
  вместе с первым настоящим разделом.
