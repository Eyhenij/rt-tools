...
Разделов три — разборы происшествий, предложения и сводки деревьев, — и каждый объявлен одной
записью декларации и своими маршрутами. Прав в админке нет вовсе — ни ролей, ни пресетов, —
...
| сквозные спеки адресов  | `apps/message-bus-admin-e2e/src/sign-in.spec.ts`                                                                                                                               |
...
- `pnpm exec nx run message-bus-admin-e2e:e2e` — сквозной набор: раздел открывается прямой
  ссылкой, переживает перезагрузку и возвращается тем же после закрытой панели.
