...
---
<!-- rt-kit v0.24.0 · patterns/component-structure-new.md · 826f70c25901 · правится надстройкой, не здесь -->
...
- У компонента набора готовых компонентов якорь сидит на хосте, а у директивы хост и есть сам
  элемент. Спека компонента целится внутрь — `[qa-dataid="x"] button`, — и у директивы это
  попадает в пустоту: вложенной кнопки под хостом нет. Отличаются они только тем, чем размечен
  тег, и по значению якоря это не видно.
- Замена обёртки-компонента на директиву начинается с поиска якоря по наборам сквозных спек.
  Разметка при такой замене выглядит целой, якорь остаётся прежним, а спека краснеет пустым
  выбором — и краснеет она у того, кто вёрстку не трогал.
