...
- **Глобальный объект приходит токеном `WINDOW`, а не берётся напрямую.** — `projects/core/src/lib/tokens/window.token.ts:WINDOW` — фабрика поднимает представление документа и бросает отказ, если его нет; сведения о браузере приходят тем же путём.
- **Среда проверяется через `PlatformService`, а не через `typeof window`.** — `projects/core/src/lib/services/platform.service.ts:PlatformService` — восемь мест дерева идут через него; проверок «есть ли window» в дереве нет.
- **Прямое обращение к глобальному объекту отбивает линтер.** — **Не исполняется.** Правила линтера на прямое обращение в конфиге нет: роль перечня играют сами токены и служба среды, а новое обращение ловится разбором.
