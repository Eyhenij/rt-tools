...
---
<!-- rt-kit v0.16.1 · patterns/doc-style-write.md · 4960108774dc · правится надстройкой, не здесь -->
...
насколько отстала: трижды за заход состояние общей ветки было выведено из файла, отставшего на
семь коммитов, и все три раза неверно — один вывод доехал до документа задачи и был назван
владельцу фактом.
...
место, а не после счёта по одному образцу. Скан подтверждает ровно тот маршрут, который в него
заложен, и молчит обо всех прочих — тем увереннее, чем точнее образец. Счёт по образцу самого
вызова дал двадцать два места; мимо прошли десять вызовов обёртки, которая берёт значение
доводом, — мест оказалось тридцать два, и неправда сутки простояла в тексте, едущем в историю.

Команда, которой получено число, проверяется на том, что она мерит спрошенное. Инструмент
отвечает на понятый им вопрос и о непонятой части образца молчит: граница слова, не
поддержанная им, дала двести семьдесят восемь совпадений вместо семидесяти девяти, а два
прохода регулярным выражением по одному и тому же коду — тридцать два и сорок два там, где по
синтаксическому дереву двадцать семь.

Число, пришедшее из прошлой сессии, замером не считается: в тексте оно выглядит так же
уверенно, как измеренное, и пересчитывается тем же ходом, которым пишется.
