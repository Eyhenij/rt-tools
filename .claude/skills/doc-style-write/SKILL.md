...
---
<!-- rt-kit v0.16.1 · patterns/doc-style-write.md · 6603c8e1c35a · правится надстройкой, не здесь -->
...
тексту, и глазами он не считается. Поэтому у приёма есть вторая половина — счёт поиском по
набору оборотов, накопленному разбором прошлых работ.

Число отказом само по себе не является: у оборота бывает законное употребление. Отказ — это
когда один оборот стоит в тексте десятками: тогда он не приём, а тик, и читатель перестаёт
замечать его вместе со смыслом. Набор пополняется каждым разбором: оборот, найденный вычиткой,
дописывается в него сразу, иначе следующий исполнитель начинает с пустого места. Где набор
лежит, знает дерево — там же, где стоит его проверка слога.
