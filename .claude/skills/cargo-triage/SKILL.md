...
**Взятие в работу — другое дело, и его судит гард.** У взятия след есть, и он машиночитаемый:
образец папки задачи требует называть ключи записей груза в разборе просьбы полностью. Гард
отметки груза читает их оттуда и не выпускает ход, который взял запись в работу или отдал по ней
работу, пока состояние записи в приёме не переведено. Судятся два момента, а не всякий ход:
между заведением задачи и отдачей работы отметки не требуется — гард, спрашивающий её на каждом
