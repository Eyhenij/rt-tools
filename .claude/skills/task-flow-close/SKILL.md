...
---
<!-- rt-kit v0.17.0 · patterns/task-flow-close.md · 3b838378b859 · правится надстройкой, не здесь -->
...
- **Запись раздела решений, повторяющая статью правила слово в слово, доводом при ней не
  считается.** Правило приходит в контекст само, а спек — только когда его открыли; пара
  расходится молча, и читатель видит два требования там, где оно одно. Такая запись уезжает
  целиком, даже если раздел после этого пустеет; довод, в статье не сказанный, дописывается в
  саму статью.
- **Тело заявки остаётся старше разбора папки.** Разбор — последний коммит ветки, и он делает
  неправдой всё, что тело обещало сделать до слияния: папку, вливание договорённости, приведение
  текстов. Ревьювер читает список оставшегося как оставшееся, поэтому тело правится тем же ходом.
