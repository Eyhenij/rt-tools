...
---
<!-- rt-kit v0.17.0 · patterns/task-flow-close.md · 0bb54bad653f · правится надстройкой, не здесь -->
...
- **Тексты правятся до разбора папки.** Список того, что перечитывать, лежит в замысле, а разбор
  папки его удаляет: после него остаётся только память о том, что задевали.
- **Тело заявки старше разбора папки.** Разбор — последний коммит ветки, и обещанное телом до
  слияния он делает неправдой. Тело правится тем же ходом.
...
- **Раздел «Чего из закона здесь нет» читается глазами, греп тут не помогает.** Искать приходится
  не то слово, которое ждёшь: правило ссылалось на статью, которой в законе нет вовсе, и по слову
  своей темы эта строка находилась — а неправда была в другом.
...
- **Номера сценариев не пересчитываются:** идентификатор — ключ связи с тестами.
...
- **Своя строка сверки очереди ищется по имени, а не читается в общем списке.** Сверка
  отвечает про всё дерево: на одном закрытии неразобранных папок было восемь при одной своей,
  и вывод «расхождения есть» ничего не говорит о том, чьи они. Ищется своя — по номеру задачи
  и имени ветки; чужие строки закрывающего не касаются и его работу не держат.
- **Пара «метка объёма и строка в замысле эпика» рвётся с двух сторон.** Задача, заведённая по
  следу закрытой, метку наследует, а строки о заходах ей никто не пишет; закрытый эпик уносит
  строку разом у всех, кто метку ещё несёт. Сверяются обе стороны, и на закрытии — там метка
