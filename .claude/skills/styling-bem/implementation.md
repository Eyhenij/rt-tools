...
| Класс ставится директивой, а не строкой в атрибуте.                | `rt/require-bem-directives` (`tools/lint-rules/rules/require-bem-directives.cjs`) — отбивает `class="…"`, `[class.x]`, `[ngClass]`; пока замечание, потому что сырой класс остался в мосте к Material. В демонстрационной разметке витрины правило выключено.                                                                                            |
...
| Предупреждение линтера стилей роняет прогон наравне с ошибкой.     | Не исполняется: `pnpm exec stylelint "projects/**/*.scss"` гоняется без порога предупреждений, и замечание оставляет прогон зелёным. Держится тем, что оба собственных правила объявлены отказом.                                                                                                                                                        |
...
- `pnpm exec stylelint "projects/**/*.scss"` — оба собственных правила и набор целиком.
