...
description: Паттерн правила ui-component-tests. Брать при заведении или правке *.spec.ts рядом с компонентом кита — сборщик фикстуры первого кита, общая обвязка второго, хост-обёртка для входа от приложения, рукописный двойник службы и подмены, без которых компонент не поднимается. Не брать для истории и снимка — это паттерн ui-component-tests-visual.
---
