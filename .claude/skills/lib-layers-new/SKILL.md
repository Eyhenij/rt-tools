...
---
<!-- rt-kit v0.24.0 · patterns/lib-layers-new.md · cfa93beb475f · правится надстройкой, не здесь -->
...
**Перемещение — то же действие плюс сброс графа.** Каталог, переехавший `mv`, граф сборщика не
пересчитывает: проект остаётся в кэше по прежнему пути, а по новому его нет вовсе.

```bash
mv <прежний путь либы> <новый путь либы>
npx nx reset
```

Без сброса прогон по всем проектам остаётся зелёным и идёт на один проект меньше: состав он
берёт из кэша графа, а выпавшего проекта там уже нет. Отличить такой прогон от полного можно
только по числу проектов в его выводе — имени пропавшего он не называет, и промах живёт до
первой правки в выпавшей либе.
