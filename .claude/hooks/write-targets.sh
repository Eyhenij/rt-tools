#!/usr/bin/env bash
# rt-kit v0.23.0 · hooks/write-targets.sh · cacda62929dd · правится надстройкой, не здесь
...
rt_write_targets() {
    ...
    # свои пути: команда с правкой одного файла и запуском проверки рядом отбивалась по пути этой
    # проверки. Настоящая запись рядом с заглушённым потоком остаётся видной: снимается
