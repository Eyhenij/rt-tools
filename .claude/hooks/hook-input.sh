#!/usr/bin/env bash
# rt-kit v0.23.0 · hooks/hook-input.sh · 29c29bc10087 · правится надстройкой, не здесь
...
# Прежде принимался только первый, и «пробел внутри значения встречается редко» оказалось
# неправдой ровно там, где признак нужен больше всего: заявки открывают командой
# `GH_TOKEN=$(cat <файл>) gh pr create …`, и её гард поставки не судил вовсе — ни номера ветки,
# ни тела заявки, ни разбора папки задачи. Молчание гарда при этом неотличимо от разрешения.
#
# Подстановка берётся до первой закрывающей скобки, а строка в кавычках — до закрывающей кавычки
# того же вида: вложенных скобок и кавычек образец не разбирает и разбирать не должен — признак
# остаётся образцом, а не разборщиком оболочки. Признак обязан ошибаться в сторону лишнего
# срабатывания: гард, не узнавший вызов, молчит и выглядит исправным, а сработавший на лишнем
# виден сразу и правится.
#
