#!/usr/bin/env bash
# rt-kit v0.8.1 · hooks/proposal-guard.sh · 6f5ec1ca2618 · правится надстройкой, не здесь
# rt-hook: Stop
...
Сухой прогон отправкой не является: он показывает, что уехало бы, и следа наружу не оставляет. Отправка пишет отметки в файлы предложений и делает дерево грязным — при открытом отчёте они ложатся вторым коммитом в ту же ветку, и это их место, а не повод отложить.
