#!/usr/bin/env bash
# rt-kit v0.8.1 · hooks/git-guard-delivery.sh · 416f2bae3254 · правится надстройкой, не здесь
...
#
# Строка обхода начинает строку — свою в теле отчёта или комментарий в конце команды — и
# подстановки не принимает. Без этих двух условий текст, который ОБЪЯСНЯЕТ, что обход называется
# так-то, от самого обхода неотличим: тело отчёта со строкой-примером снимало требование само
