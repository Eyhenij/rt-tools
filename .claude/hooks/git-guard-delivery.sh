#!/usr/bin/env bash
# rt-kit v0.23.0 · hooks/git-guard-delivery.sh · e24e4c01fe9c · правится надстройкой, не здесь
...
# Второй ярус той же личности: кто на самом деле придёт по этому токену.
#
# Подстановка в команде говорит о намерении, а не о результате. Заявка вышла от владельца при
# верной с виду команде: подстановка была, но читала файл по пути, которого на машине нет.
# Пустую строку клиент читает как незаданный токен и отвечает от той записи, которой авторизован.
# У такой заявки нет ревьювера, автора ей не сменить; чинится только закрытием и переоткрытием,
# а закрытие бывает запрещено режимом работы — тогда исполнитель уже ничего не решает.
#
