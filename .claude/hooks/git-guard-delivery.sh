#!/usr/bin/env bash
# rt-kit v0.23.0 · hooks/git-guard-delivery.sh · c4e3ec1a2ce8 · правится надстройкой, не здесь
...
# Второй ярус той же личности: кто придёт по этому токену на самом деле.
#
# Подстановка в команде говорит о намерении, а не об исходе. Заявка вышла от владельца при верной
# с виду команде: подстановка была, но читала файл по пути, которого на машине нет. Пустая строка
# — клиент читает её как незаданный токен и отвечает от той записи, которой авторизован. У такой
# заявки не бывает ревьювера, автора ей не сменить, чинится только закрытием и переоткрытием — а
# закрытие бывает отбито режимом работы, и тогда промах упирается в границу, где исполнитель уже
# ничего не решает.
#
# Спрашивает у хостинга дерево, а не пакет: хостинг, клиент и путь к токену у каждого свои.
# Пустой ответ означает «спросить не удалось» — вызов пропускается, и об этом говорится вслух:
# молчаливый пропуск здесь неотличим от сошедшейся сверки.
if [ -n "$task_bot" ] && command -v rt_pull_token_login >/dev/null 2>&1; then
    ...
    if [ -z "$token_login" ]; then
        printf 'гард поставки: кто придёт по токену, спросить не удалось — ярус ответа хостинга пропущен.\n' >&2
    elif [ "$token_login" != "$task_bot" ]; then
        fault "по токену вызова хостинг отвечает записью «${token_login}», а не машинной «${task_bot}»: подстановка в команде есть, а значение у неё пустое либо чужое — так заявка и выходит от владельца. Проверь, что файл токена на месте и читается${pull_token_hint:+: ${pull_token_hint}}."
    fi
fi
...
# напоминание, а требование — на слиянии: считалось, что папка нужна на диске всё время
# разбора, потому что правку по замечаниям без замысла отбивает гард хода работы. Владелец же
# вливает, как только видит зелёное, и закрывающему коммиту места не остаётся — трижды подряд
# папка уехала в главную неразобранной, а гард слияния этого не видел вовсе: кнопку нажимает
# человек на хостинге, и до него гард не достаёт. Разбор переехал сюда, а замысел после уборки
# гард хода работы берёт из истории ветки, а не с диска.
