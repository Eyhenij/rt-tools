#!/usr/bin/env bash
# rt-kit v0.23.0 · hooks/git-guard-delivery.sh · 80312d7c86fc · правится надстройкой, не здесь
...
# --- снятие черновика --------------------------------------------------------------------
#
# Ревьювер проверяется здесь: хостинг запрос разбора на самого себя принимает молча и не
# создаёт, а до открытия заявки ревьювера нет. Ярус сетевой и молчит так же, как ярус состояния
# задачи: нет ответа — нет требования. Возврат заявки в черновик не проверяется: он снимает с
# работы вид готовой, чего гард и добивается.
