...
#
# Судятся два момента, а не всякий ход. Между заведением задачи и отдачей работы отметки не
# требуется: гард, спрашивающий её на каждом ходу, отбивал бы саму работу.
#
...
verdict="$(tail -n 400 "$transcript" 2>/dev/null | jq -s -r --arg dir "$tasks_dir" --arg br "$branch" '
    ...
    # Взятие: за ход написан разбор просьбы этой ветки либо позвана команда заведения задачи.
    | (($uses | map(((.name // "") | test("^(Write|Edit|MultiEdit)$"))
          and ((.input.file_path // "") | test($dir + "/" + $br + "/grill\\.md"))) | any)
       or ($said | test("task:new"))) as $took
...
case "$verdict" in
    took) moment="взял запись груза в работу"; want="в работе" ;;
    gave) moment="отдал работу по записи груза"; want="готово, и с переходом едет приём починки" ;;
    *) exit 0 ;;
esac
...
reason="BLOCKED by cargo-mark-guard: ход ${moment}, а состояние записи в приёме за этот же ход не переведено — оно осталось прежним.
...
Перевести надо в «${want}»:

    ${mark_cmd} -- --state <состояние> --proposal <признак> --postmortem <признак>
