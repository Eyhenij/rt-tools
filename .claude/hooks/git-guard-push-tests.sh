#!/usr/bin/env bash
# rt-kit v0.23.0 · hooks/git-guard-push-tests.sh · aca4e83c5434 · правится надстройкой, не здесь
...
# Красное бывает двух родов, и гард их не различает: он судит код возврата. Когда падает
# проверка кода — «почини и пушь снова» верно. Когда падает сама проверка, тот же текст велит
# чинить код, которого никто не трогал: разобранный однажды отказ целиком лежал в документах, не
# тронутых ни одним коммитом ветки, а самый старый из них правился двумя месяцами раньше. Закон о
# проверяемости говорит, что сломавшаяся проверка работу не останавливает, — и до этой строки
# хода под это в отказе не было ни одного.
...
Ходов отсюда три: починить названное и повторить вызов; починить саму проверку, если врёт она, — разбор отказа поимённо, показ разбора владельцу и правка проверки; либо принести владельцу цену обхода и ждать его слова. Спорное в список известного не вносится: он копит принятое, а не показания сломанного прибора.
