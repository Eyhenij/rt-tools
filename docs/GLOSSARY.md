<!-- rt-kit v0.26.0 · docs/GLOSSARY.md · 26ca8b464fa9 · правится надстройкой, не здесь -->
# Project glossary
...
- **спек, спека** — описание; файл в `docs/specs/`. В русском такого слова нет ни в каком роде
- **заход** — сессия
- **прогон** — запуск тестов, CI
- **проба** — тест
- **гард, страж** — блокирующая проверка, хук
- **гейт** — проверка перед push
- **раскладка, разложить** — установка файлов из пакета
- **надстройка** — переопределение
- **замысел** — план
- **разбор (просьбы)** — уточнение задачи
- **заявка** — PR
- **сверка** — аудит
- **замер** — измерение
- **компаньон** — файл привязок
- **спутник** — соседний файл
- **очередь работ** — список задач
- **след задачи** — что задача затрагивает
- **отказ в пользу работы** — пропуск при ошибке
- **везёт, несёт в себе (о пакете)** — входит в пакет, содержит
- **отбивает** — блокирует, не пропускает
- **уезжает в пакет** — попадает в пакет
- Метафоры и олицетворение: «ветка отстала на всё, что вливали соседи», «проверка говорит», «команда отвечает по памяти» — так не пишут. Пишут, что произошло и что надо сделать
- **таска, тикет** — задача
