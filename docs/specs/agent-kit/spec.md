...
**Статус:** действует · **Ревизия:** 2026-08-16 · **Префикс сценариев:** `SC-AK`
**Зависимости:** нет
...
- **Отбор** — чем дерево ограничивает раскладку: выбор поимённо, отказ строкой названные виды и свойства дерева.
...
| В договорённости о подписи | Чем это является                                                                                           |
| -------------------------- | ---------------------------------------------------------------------------------------------------------- |
...
- **Открытый PR, чья ветка везёт папку своей задачи, — расхождение сверки.** Гард судит
  папку на слиянии, а слияние нажимает человек в браузере, где хуков нет вовсе: сверка —
  единственное, что успевает сказать слово до слияния. После слияния та же строка чинится уже
...
Отказ гарда открытия PR кода возврата команды не имеет: он отбивает вызов до его исполнения
...
повод, чем закрывается — какие надстройки дерева снимет приехавшая правка — и после отправки
ссылка на заведённую запись. Кладут его два движения — разбор
закрытой задачи и команда фидбэка, — и файл у обоих один: тот, что за сегодняшний день.
...
- **Объявленный список живёт в настройке проверок, ключом `pushGate`.** Проверка читает её
  напрямую, а набор гейта спрашивает у профиля его же оболочкой: два места вместо одного, зато
  каждое отвечает за своё — настройка за объявления, профиль за исполнение. Отвергнута отдельная
  функция профиля: она вернула бы объявления той же оболочкой, что и набор, и расхождение между
  ними перестало бы быть видимым.
- **Имя шага достаётся образцом, который называет дерево.** У каждого хостинга своя форма
  записи; умолчание записано под GitHub Actions, остальные деревья называют свою.
...
- `Q-13` — гасит ли выключатель наблюдений снимок надстроек. Работа идёт допущением «гасит».
  ...
  рассказывает.
- `Q-14` закрыт решением владельца: уезжает текст целиком, и проверка на адрес дерева этот род
  груза не накрывает.
...
- 2026-08-15 — влита договорённость об образцах: род ресурса под то, что копируют в рабочий
  файл, его раскладка и шапка у копии.
...
- 2026-08-13 — влита договорённость об обратной связи слоя правил: наблюдение в дереве вместо
  ...
  отправка в очередь работ пакета, гард влитой главной ветки на открытии PR.
