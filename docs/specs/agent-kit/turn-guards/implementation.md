...
- **Ход, в котором у владельца просят войти или ввести пароль, не заканчивается.** — `projects/agent-kit/assets/hooks/stand-login-guard.sh:asked_re` — набор образцов просьбы; сценарий SC-AK-840
- **Просьба о режиме работы законна и отбою не подлежит.** — `projects/agent-kit/assets/hooks/stand-login-guard.sh:asked_re` — слово о режиме в набор не входит; сценарий SC-AK-841
- **Граница набора образцов названа вслух.** — `projects/agent-kit/assets/hooks/stand-login-guard.sh:ГРАНИЦА` — сказано в шапке гарда; не проверяется ничем: чего гард не видит, тем и не судится
