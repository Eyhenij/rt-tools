...
| Разбор начинается с неразобранного.                                  | Отбор по состоянию у разделов груза админки — договорённость о столбце и отборе по состоянию |
...
| С переходом в «готово» едет приём починки.                           | `projects/agent-kit/src/lib/cargo-state.ts:itemsOf`                                          |
| Приём починки пишется словами правки, а не пересказом разбора.       | Не исполняется машиной: смысл текста не судит ничто                                          |
| «Выпущено» ставится тем, кто публикует редакцию, и тем же движением. | Не исполняется машиной: держится правилом разбора груза                                      |
| Версия выпуска — та, которой назван выпуск, увёзший починку.         | `projects/agent-kit/src/lib/cargo-state.ts:mark`                                             |
| Между «готово» и «выпущено» стоит редакция пакета.                   | Набор состояний: `projects/agent-kit/src/lib/cargo.ts:CARGO_STATES`                          |
| Отметка ставится командой пакета, а не рукой человека в админке.     | `projects/agent-kit/src/bin/agent-kit.ts:runMark`                                            |
