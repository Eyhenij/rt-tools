...
- **The scenario suite runs its fixtures on its own tree, whatever environment stands around it.** — `projects/agent-kit/tests/lib.sh:unset` — the git variables are removed alongside the parsed input fields, before the first fixture. The suite guarding it is `projects/agent-kit/tests/suite-git-env.test.sh`; scenario `SC-AK-979`
