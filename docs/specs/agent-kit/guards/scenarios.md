...
цель записи: имя, названное в тексте, правкой этого файла не является, а отказ по нему кончает
...
### SC-AK-838 — чужой профиль запрещается до вызова, метка при этом не ставится

Дано дерево назвало закреплённый профиль браузера
Когда гард проверяет выбор до вызова
Тогда чужой профиль запрещён, закреплённый разрешён, и метка свежести не поставлена ни в одном
из случаев: до вызова известно только то, что профиль запрошен

Покрыто: `projects/agent-kit/tests/browser-guard-device-id.test.sh`.

### SC-AK-839 — метка свежести ставится по результату вызова

Дано выбран закреплённый профиль
Когда гард проверяет ответ вызова
Тогда при подтверждённом подключении метка ставится; при отказе выбора, отказе текстом и пустом
ответе метка не ставится — иначе гард свежести пропустил бы состав вкладок, переход и снимок
экрана в тот браузер, который расширение считает активным
...
Когда гард судит командную строку
Тогда вызов узнан во всех трёх формах, в том числе за разделителем и полным путём, а упоминание
той же команды в кавычках и поиск по дереву вызовом не считаются
