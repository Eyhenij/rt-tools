...
- **Who raises the stands is answered by the tree, and an unset answer reads as the owner.** — `projects/agent-kit/assets/hooks/dev-server-guard.sh:raiser` — scenario `SC-AK-1091`
- **Where the session raises the stands, only a raise over a taken port is refused.** — `projects/agent-kit/assets/hooks/dev-server-guard.sh:taken_stand_port` — scenario `SC-AK-1091`
