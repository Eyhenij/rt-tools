...
- **Гардом сводка считает того, кто умеет записать свой отбой.** Хук без такой записи в
  наблюдениях не появится ни при каком отрезке, и молчащим он назывался бы неверно: он молчит не
  потому, что не понадобился.
...
- **Гард, который отбивать умеет, а записывать отбой — нет.** Сводка его не называет ни
  отбивавшим, ни молчащим: спросить о нём нечего. Дописать запись в такие гарды — своя работа,
  и на счёт молчащих она не влияет.
