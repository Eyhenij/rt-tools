...
- **A bundle is declared with the area of the tree it holds over — a directory or a list of them.** — `projects/agent-kit/assets/checks/signals.mjs:bundleOf` — the area goes onto the sign by the field `roots`, a file outside it drops out of the walk by `projects/agent-kit/assets/checks/check-reuse.mjs:within`, and the guard `projects/agent-kit/assets/hooks/reuse-first-guard.sh` reads the same field; scenario SC-AK-1085
- **One bundle carries one area, and a second declaration of it is a refusal.** — `projects/agent-kit/assets/checks/signals.mjs:loadSignals` — the refusal is thrown before a single file is read; scenario SC-AK-1086
