...
- **The shadow database is created by the check itself.** — `projects/agent-kit/assets/checks/check-schema-drift.mjs:main` — the client goes to the storage through an adapter and creates nothing; scenario `SC-AK-924`
- **A refusal of the deploy command about an unreachable server is told from a divergence of the migrations.** — `projects/agent-kit/assets/checks/check-schema-drift.mjs:main` — one code stands for both, and after the creation it means a defect of the check; scenario `SC-AK-925`
