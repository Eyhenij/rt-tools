...
### SC-AK-545 — проверка, не сумевшая отработать, отказывает вместо пропуска
...
случаи «предмета нет», «адрес не задан» и «служба не отвечает» по-прежнему отдают ноль своей
строкой
