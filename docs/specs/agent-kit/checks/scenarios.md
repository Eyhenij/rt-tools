...
Дано переменная профиля замещена, и её имени нет ни в одном компаньоне правила
Когда идёт сверка надстроек профиля
Тогда она отказывает и называет переменную по имени: читатель компаньона иначе получает пакетное
умолчание вместо того, по чему дерево работает на самом деле
...
Дано имя замещённой переменной стоит в компаньоне правила
Когда идёт сверка надстроек профиля
Тогда она проходит; имя ищется по всем компаньонам дерева, а не по одному — правило у надстройки
бывает своё
...
Дано переменная профиля задана значением умолчания пакета
Когда идёт сверка надстроек профиля
...
Дано у дерева нет профиля, либо нет умолчаний пакета, либо нет компаньонов правил
Когда идёт сверка надстроек профиля
Тогда она говорит, чего не хватает, и уходит нулём: дерево вправе не держать ни того, ни другого
