...
Когда сверка очереди работ судит её вершину
Тогда она называет базу и говорит, что прогона не будет, а лечится это переносом базы после
слияния нижней заявки; совета перезакрыть заявку нет. Заявка в главную ветку судится прежней
строкой о потерянном событии
