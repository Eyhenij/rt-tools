...
- **У записи списка стоят своя причина и номер задачи, которой она внесена.** Причина прозой на
  ...
  и саму запись.
- **Сторона списка, записанная перечнем строк, отбивает разбор.** У строки нет места ни для
  причины, ни для номера задачи, и перечень читается как список без объяснений.
