...
- **Надстройка профиля, о которой молчит компаньон правила, — расхождение.** Правило требует
  одного, дерево работает по-другому, и оба места законны; но читатель ищет это в компаньоне, где
  перечислено, чем здесь зовётся сказанное правилом. Не названное там замещение оставляет ему
  пакетное умолчание вместо действующего: правило требовало одной формы заголовка заявки, дерево
  замещало её переменной, и гард, читавший форму, номера не доставал вовсе.
...
- **Сверка значения надстройки с текстом правила.** Сказать, верно ли значение, машине нечем:
  она видит, что дерево работает по-своему, и требует назвать это, а не оценивает.
