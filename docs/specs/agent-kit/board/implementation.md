...
- **A branch carrying no task folder is a divergence, and the branch of an epic is not.** — `projects/agent-kit/assets/checks/board-folders.mjs:checkBranchFolders` — the branch history is judged, not its tip: the folder is taken apart by the last commit before the PR opens. Scenarios SC-AK-1115 and SC-AK-1117
- **A branch is recognised as an epic's by the plan inside that branch, not by the plan on disk.** — `projects/agent-kit/assets/checks/board-folders.mjs:namesItselfEpic` — the plans of the branch are listed by their raw paths and read out of it; the header pattern is the one the creating command writes with. Scenario SC-AK-1116
