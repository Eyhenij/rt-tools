...
- **Интерпретатор судится телом, а не именем.** Он пишет тем, что ему подали, — документом на
  входе или кодом доводом; путь, стоящий у него первым доводом, он читает. Судимый по имени, он
  делал записью всякий запуск проверки дерева по пути её файла: заход, гонявший проверку ради
  диагностики, получал требование правила общего кода, ничего в нём не правя, и за две задачи
  таких отбитий набралось около пятнадцати. Цена сужения известна: скрипт, который пишет файлы
  своим телом, гейт больше не судит — его судили при заведении, когда клали в дерево.
...
- **Команда, публикующая тело задачи или заявки, требует правило слога.** Этот текст файлом
  дерева не становится вовсе, а читает его человек — и чаще, чем любой файл: гейт цеплялся за
  расширение правимого файла и на такой команде молчал.
- **Судятся два признака сразу: вызов клиента и тело в доводах.** Одного слова о заявке мало —
  оно попадает в строку любой команды, которая о ней пишет.
