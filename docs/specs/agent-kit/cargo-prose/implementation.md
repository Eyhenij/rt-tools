...
- **The record is written by a role of its own, and it writes no files.** — `projects/agent-kit/assets/agents/cargo-writer.md:tools`
- **The role writes into the ready-made shape, not into one of its own.** — `projects/agent-kit/assets/agents/cargo-writer.md:templates`
- **The role is asked for a text, not for a verdict on the work.** — `projects/agent-kit/assets/agents/cargo-writer.md:substance`
