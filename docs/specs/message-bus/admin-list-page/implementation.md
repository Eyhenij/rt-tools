...
- **A row of the list holds the value and the key of the label, not the ready word.** — `libs/message-bus-admin/accounts/util/src/lib/person.model.ts:stateKey` — the key in the row of the people. The row of the cargo holds the value of the set: `libs/message-bus-admin/postmortems/util/src/lib/postmortem.model.ts:state`
- **The word of a row is put together by the screen on every drawing.** — `libs/message-bus-admin/common/core/util/src/lib/admin-cargo-rows.ts:adminStatedRows` — the word of the state for both sections of the cargo. At the other sections the screen puts it together itself: `libs/message-bus-admin/accounts/feature/list/src/lib/admin-people-list.component.ts:rows`
- **A pure pipe does not translate a cell.** — `libs/message-bus-admin/usage/feature/list/src/lib/admin-usage-list.component.ts:rows` — the kind of the skill was translated by a pure pipe and kept the former word after the switch. The pipe is removed, and the word comes from the rows of the screen
