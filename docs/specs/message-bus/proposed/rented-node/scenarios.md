...
Нумерация общая со спеком домена: следующий свободный номер после `SC-MB-79`.

### SC-MB-80 — груз доезжает до приёмника по имени
...
### SC-MB-81 — обращение по открытому протоколу уводится на защищённое
...
### SC-MB-82 — браузер принимает сертификат приёмника
...
### SC-MB-83 — приёмник поднимается сам после перезапуска узла
...
### SC-MB-84 — груз переживает пересоздание контейнеров
...
### SC-MB-85 — узел поднимает образ того коммита, который выкатывали
...
### SC-MB-86 — миграции накатаны раньше, чем приёмник начал отвечать
...
### SC-MB-87 — выкатка объявляет себя удавшейся только после ответа пробы
...
### SC-MB-88 — чистка оставляет три последних sha и не трогает чужого
...
### SC-MB-89 — загруженный дамп возвращает свод и годность токенов
...
### SC-MB-90 — раздел админки открывается прямой ссылкой
