# Вход в админку и чтение принятого груза

**Статус:** предложен · **Ревизия:** 2026-08-15 · **Префикс сценариев:** `SC-MB`
**Зависимости:** `agent-kit` (груз, который админка читает)
**Законы:** `frontend-application`, `reuse-first`, `verifiability`, `observability`
**Процедуры:** нет — операции объявлены контроллерами приёмника

Законы о списке записей, о правке сущности, о моделях сущностей, о навигации и о доступе эта
договорённость тоже держит, но назвать их в шапке нельзя: в дереве их пока нет — они приезжают
раскладкой, задача #586. Строка шапки правится тем же коммитом, что и раскладка.

## Зачем

Приёмник принимает груз трёх родов и складывает его в хранилище, а прочитать принятое можно
только запросом к базе с самого узла. Служба при этом уезжает в интернет: знающая одни лишь
токены деревьев, она отдавала бы принятое всякому, кто дошёл до её адреса, и не отвечала бы на
вопрос, кто читал.

Договорённость называет две вещи: чем человек представляется приёмнику и что он видит после
входа. Разделов три — по роду груза: разборы происшествий, предложения, сводки.
...
| Учётная запись      | Имя и пароль одного человека. Заводится командой строки запуска приёмника              |
...
| В договорённости          | На экране и в строке запуска                             |
| ------------------------- | -------------------------------------------------------- |
...
- **Правка принятого груза.** Ни в приёмнике, ни в админке.
...
- **Выкатка на арендованный узел, доменное имя и сертификат.** Отдельная задача линии работ
  `docs/plans/agent-kit-observations.md`.
- **Оповещения о приехавшем грузе.** Груз читают, когда за ним приходят.
- **Вторая тема оформления.** Кит умеет обе; вторая заводится тогда, когда её попросят.

## Контракт

Поверхность — операции HTTP рядом с операциями приёма и четыре команды строки запуска. Человек
представляется входом, дерево — токеном; одно вместо другого не принимается.
...
Страница списка запрашивается номером, размером, порядком и отбором по дереву. Ответ несёт строки
и общее число записей: без него пагинация не знает, сколько страниц.

| Команда                             | Что делает                                                       |
| ----------------------------------- | ---------------------------------------------------------------- |
| `message-bus account:add <имя>`     | заводит учётную запись; пароль спрашивается и в журнал не уходит |
| `message-bus account:passwd <имя>`  | меняет пароль записи                                             |
| `message-bus account:disable <имя>` | отключает запись и обрывает её входы                             |
| `message-bus account:list`          | называет имена записей, их состояние и время последнего входа    |

### Коды отказов

Не применимо: приёмник отвечает кодом ответа HTTP, а не именованными кодами домена. Где админка
...
| хранилище недоступно                     | `503` | что прочитать не удалось, и номер обращения             |
| остальное                                | `500` | что прочитать не удалось, и номер обращения             |

## Данные

Хранилище то же, схема правится миграциями. Колонок и индексов договорённость не называет.
...
Язык один — русский, и подписи лежат в словаре приложения, а не в разметке. Оттуда же берутся
подписи кита: у него свой словарь на восемь языков, и без этого пагинация и пустое состояние
выходят по-английски рядом с русскими заголовками. Второй язык дешевле добавить потом.
...
Строка списка на узком экране показывается карточкой — это вход таблицы кита, а не своя разметка.
Экран, собранный мимо неё, узкую раскладку теряет молча.

### Мультиобъектность

Деревьев много, и вошедший видит груз всех: учётная запись принадлежит службе, а не дереву.
Отбор по дереву сужает показанное, а не доступ. Токен дерева при этом по-прежнему открывает
только приём и только своего дерева.
...
- `Q-18` (спек приёмника) — читается ли в админке груз дерева, чей токен отозван. Отзыв закрывает
  приём; чтение прежнего груза эта работа не решает — показывает.

## История изменений

- 2026-08-14 — заведено разбором просьбы владельца о задачах линии работ
  `docs/plans/agent-kit-observations.md`: вход, каркас админки и списочные экраны.
- 2026-08-14 — приведено к состязательному разбору: закрыты недосказанности о носителе входа,
  сортировке, границах страницы, показе чужого текста, отборе по дереву, жизни учётной записи,
  нескольких входах, состоянии списка, языке, поясе времени и отказе, видимом человеку.
- 2026-08-15 — три расхождения с разбором вынесены владельцу и решены им: три списочных экрана
  вместо одного, пять раскладываемых законов вместо трёх, сквозной набор заводится этой работой.
