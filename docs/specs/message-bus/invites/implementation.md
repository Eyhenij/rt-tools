...
- **A refusal of the issuing speaks by the word of the intake and leaves the entered name in the field.** — `libs/message-bus-admin/common/core/util/src/lib/spoken-fault.ts:spokenFaultOf` — the word is read by the shared util of the admin panel and shown by `libs/message-bus-admin/invites/feature/create-aside/src/lib/admin-invite-create-aside.component.ts:submit`
