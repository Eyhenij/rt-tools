...
- **Адрес отправителя в логах.** Приёмник его не пишет и здесь не начинает: за проксировщиком
  это требует и доверия к заголовку, и правки самого приёмника.
...
- **Кеш сборок.** Отвечает ли он успехом после пропажи клиента хранилища — своя работа: сегодня
  пропажа кончается отказом, и класть в кеш нечего.
...
Договорённость об арендованном узле поверхности не меняет ни одной операцией: те же операции
приёма и чтения, тот же заголовок токена, тот же вход человека. Изменились адрес, по которому
эта поверхность доступна, и протокол, которым до неё доходят.
...
Отказ дороги — дело проксировщика: такой запрос до приёмника не доходит вовсе, и своего кода
отказа у него нет.
...
Хранилище лежит на именованном томе узла, а не в каталоге машины владельца. Дамп несёт его
целиком: деревья с признаками, токены с их состоянием, учётные записи, записи месяца со
сводками, предложения и разборы происшествий. Отдельного отбора у выгрузки нет — половина
дампа не отвечает ни на один вопрос, ради которого его снимают.
