...
- **A person without a role is shown as without a role, not as an empty cell.** — `libs/message-bus-admin/accounts/util/src/lib/person.logic.ts:personRoleLabel`
- **The list only reads.** — `libs/message-bus-admin/accounts/shell/src/lib/people.routes.ts:peopleRoutes` — one route and not a single neighbour in the outlet `ro`: there is no panel to open, and the screen declares no row menu either — `libs/message-bus-admin/accounts/feature/list/src/lib/admin-people-list.component.ts:AdminPeopleListComponent`
