...
### SC-MB-281 — задача заведена по записи груза, а состояние записи прежнее

Дано разбор просьбы папки задачи называет ключи записей груза, и ход написал этот разбор либо
позвал команду заведения задачи
Когда гард судит завершение хода
Тогда ход отбивается, отказ называет ключи записей и состояние «в работе»; отметка, сделанная тем
же ходом, ход отпускает

Покрыто: `projects/agent-kit/tests/cargo-mark-guard.test.sh`.

### SC-MB-282 — работа по записи груза отдана, а состояние записи прежнее

Дано ход открыл заявку либо разобрал папку задачи, чей разбор просьбы называет ключи груза
Когда гард судит завершение хода
Тогда ход отбивается и отказ требует состояния «готово» вместе с приёмом починки; закрытие
издателем считается отметкой наравне с обычной

Покрыто: `projects/agent-kit/tests/cargo-mark-guard.test.sh`.

### SC-MB-283 — между взятием и отдачей отметки не требуется

Дано ход правил код и коммитил, а задачи не заводил и работы не отдавал
Когда гард судит завершение хода
Тогда ход отпускается: гард, спрашивающий отметку на каждом ходу, отбивал бы саму работу. Сухой
прогон отметкой при этом не считается — он следа наружу не оставляет

Покрыто: `projects/agent-kit/tests/cargo-mark-guard.test.sh`.

### SC-MB-284 — задача не из груза гарда не получает

Дано разбор просьбы ключей записей груза не называет либо называет признак короче полного
Когда гард судит завершение хода
