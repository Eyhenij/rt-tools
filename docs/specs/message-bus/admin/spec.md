...
**Статус:** действует · **Ревизия:** 2026-08-17 · **Префикс сценариев:** `SC-MB`
**Зависимости:** нет
...
- **Кода приглашения на экране нет никогда.** В хранилище лежит только его хеш, и показать код
  второй раз неоткуда. Строка называет имя будущего дерева, состояние, сроки и признак дерева,
  которое этим приглашением завелось.
- **Отбора по дереву у раздела нет.** Приглашение ждёт дерева, которого ещё нет, и сузить им
  можно было бы одни погашенные.
- **Пустой список называет команду, которой приглашение выдаётся.** Отбора у раздела нет, и
  объяснять пустоту им нечем: пусто здесь означает, что не выдавали.
- **Отзыв — единственная правка, которую админка делает над записями приёмника.** Он доступен
  одному ждущему приглашению: погашенное, просроченное и отозванное уже необратимы, и показанный
  у них пункт обещал бы действие, которого нет.
...
- **Вход создания записи на списочной странице.** Разделу, который ничего не создаёт, кнопка
  создания не нужна; заведённая заранее и никем не включённая, она проверяется только чтением
  кода. Открыто как `Q-27`.
...
| Приглашения          | чтение · страница со строками · пусто · отказ чтения с повтором                   |
