...
| Панель создания         | Панель, где владелец называет имя будущего дерева и получает код приглашения. Открывается своим адресом |
...
| вход создания       | кнопка «Пригласить проект» в тулбаре раздела приглашений         |
...
Оболочка приложения — верхний ряд разделов, попап профиля, тема, язык и экран входа — описана
поддоменом рядом: `docs/specs/message-bus/admin-shell/`.
...
- **Вход создания записи у разделов груза.** Груз админка читает, а не создаёт: разделу, который
  ничего не заводит, кнопка создания не нужна. Раздел приглашений — единственное исключение, и
  вход у него есть.
...
- 2026-08-19 — влита договорённость задачи RT-878 о выдаче приглашения из админки: вход
  создания в тулбаре раздела приглашений, панель создания в аутлете `ro`, единственный показ
  кода и обновление списка тем же движением. Сценарии `SC-MB-154`…`SC-MB-162` переехали
  прежними номерами. Правило «кода приглашения на экране нет никогда» переписано: код виден
  один раз в выдавшей его панели, а в строке списка по-прежнему не бывает. Подписи раздела
  на экране переведены со слова словаря на слово человека — разбор промаха лежит рядом со
  спеком, в `docs/postmortems/2026-08-19-tree-word-shown-to-users.md`.
