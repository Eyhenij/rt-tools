...
The screen stands on the `rt-workspace` of the kit: three columns, dragged by a person, with the
widths remembered under the key `admin-chat-workspace`. The list of the talks is the
`rt-thread-list` of the kit: the choice of a row, the waiting and the empty state are its own
business, and the two narrowings stand in its slots. `AdminChatTalkComponent` is only what one row
holds inside, and it carries no control of its own; the state of a talk it draws by the `rt-tag` of
the kit. The thread and the field of the answer are the `rt-chat` of the kit, and the section keeps
no message view of its own. The properties of a talk and the action over it stand in the
`rt-workspace-details` of the right column, and their content is given by the functions of
`chat-workspace.logic.ts` — the same ones the embedded page takes. The translation into the model of the kit lies in one place —
