...
Покрыто: показ `MenuScrollHint`
(`projects/ui-kit/src/lib/ui-kit/side-menu/stories/side-menu.stories.ts`) — спекой этого не снять,
`var()` разрешает браузер, и в спеке растушёвка совпадает сама с собой при любой схеме. Показ
...
Покрыто: показ `MenuScrollHint`
(`projects/ui-kit/src/lib/ui-kit/side-menu/stories/side-menu.stories.ts`) — он меряет расстояние
от низа полосы до первой строки подвала. До правки полоса обрывалась на 16 пикселей выше.
