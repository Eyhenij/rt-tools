...
- **Ручка тяги ловится шире, чем видна.** Зона захвата и видимая полоса — два разных числа: в
  четыре пикселя курсор попадает не с первого раза, а промах мимо ручки попадает в панель, то есть
  вместо тяги человек нажимает пункт. Зона шире втрое и стоит по центру прежнего места, красится
  при наведении полоса внутри неё — закрашенная целиком зона читалась бы другим оформлением.
