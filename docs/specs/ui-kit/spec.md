...
Домен держит три предмета, и все три пришли договорённостями, написанными до кода: кнопка
копирования у пустой ячейки таблицы, задержка показа спиннера и то, чем закрывается шторка.
Каждый отвечает на свой вопрос о поверхности компонента, а не о проверке показа.
...
Три предмета домена идут подряд: сперва кнопка копирования у пустой ячейки таблицы, затем
задержка показа спиннера, затем источники закрытия шторки.
