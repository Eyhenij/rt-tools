...
Not covered: passing the edge shows only in the raster, and a call does not read the raster. It is held by the measurement at three canvas widths and by the frame `projects/ui-kit-v2/.storybook/__snapshots__/organisms-first-kit-table-datalist--themes.png`.
