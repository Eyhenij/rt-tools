...
- **The frame takes the span of what is drawn, not the box of the root of the show.** — `projects/ui-kit-v2/.storybook/test-runner.ts:spanOfNode` measures the span and cuts it by every ancestor that scrolls or hides its overflow; `projects/ui-kit-v2/.storybook/test-runner.ts:fitViewportToSpan` grows the window to the right and bottom edges of the span. The frame is a shot of the page with that cut. **Checked by nothing** is whether the span agrees with what a person sees. A sweep of the showcase at the base window found 35 showings of 545 whose drawing left the box of the root.
- **Any frame beyond the bounds of the window is shot by a window widened to it, not by a shooting past those bounds.** — a frame of a whole page by `projects/ui-kit-v2/.storybook/test-runner.ts:fitViewportToPage`, a frame by the root of the show by `projects/ui-kit-v2/.storybook/test-runner.ts:fitViewportToSpan`, guarded by the probe `tools/snapshot-window-probe.mjs`; the parameter is declared in `projects/ui-kit-v2/src/showcase/story-snapshot.ts:IStorySnapshotParameters`; scenarios `SC-UKV-57`, `SC-UKV-131`
...
- **A showcase that lost its story index is brought back by its consumer before the run reads it.** — `tools/showcase-probe.mjs:ensureIndex`, the refusal text — `tools/showcase-probe.mjs:complaintAboutIndex`; called by `tools/story-sweep-v2.mjs:ownShowings` and `tools/visual-snapshots-v2.mjs:requireOwnShowcase`; scenario `SC-UKV-142`
- **The showcase starts over a cache without hot update leftovers.** — `tools/showcase-cache-clean.mjs`, called by the showcase commands of both kits in `package.json:scripts`; scenario `SC-UKV-143`
- **The sweep over the stories survives a showing that did not open.** — `tools/story-sweep-v2.mjs:broken` gathers the findings, the wait is `tools/showcase-probe.mjs:openStory` with `fatal: false`; the drawing is waited for by `tools/story-sweep-v2.mjs:DRAW_TIMEOUT_MS`; scenario `SC-UKV-144`
