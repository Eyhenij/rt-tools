...
- **Компонент, чья начинка приезжает позже, объявляет недостроенность сам, а съёмка её ждёт.** — `projects/ui-kit-v2/.storybook/test-runner.ts:PENDING_SELECTOR` и `projects/ui-kit-v2/src/lib/components/rich-editor/rt-rich-editor.component.ts:mounted`; сценарий `SC-UKV-53`
