...
- **A frame of a whole page is shot by a window widened to it, not by a shooting past the bounds of the window.** — `projects/ui-kit-v2/.storybook/test-runner.ts:fitViewportToPage`, the parameter is declared in `projects/ui-kit-v2/src/showcase/story-snapshot.ts:IStorySnapshotParameters`; scenario `SC-UKV-57`
