...
- **The page strip under the list is the kit's page strip, and the material preset gives it the first kit's look.** — `projects/ui-kit-v2/src/styles/tokens.material.mjs:rt-pagination-box-size` — no scenario: the look of the strip is held by the showcase frames of the first kit's list
- **The icons of the filter row and of the row actions are 24 px in the grey of the theme in the material preset.** — `projects/ui-kit-v2/src/styles/tokens.material.mjs:rt-list-table-icon-color` — no scenario: the look of the icons is held by the showcase frames of the first kit's list
- **The buttons of the filter row and of the row actions are round and 36 px in the material preset.** — `projects/ui-kit-v2/src/styles/tokens.material.mjs:rt-list-table-button-radius` — no scenario: the shape is held by the showcase frames of the first kit's list
- **The fields of the filter row are the first kit's outlined Material field in the material preset.** — `projects/ui-kit-v2/src/lib/components/data-table/filter-cell/rt-data-table-filter-cell.component.scss:rt-data-table-filter-cell` — no scenario: the look is held by a measurement against the first kit
- **The copy button of a cell is the first kit's plate in the material preset.** — `projects/ui-kit-v2/src/styles/tokens.material.mjs:rt-list-copy-button-bg` — no scenario: the look is held by a measurement against the first kit
- **The copy button of a cell stays hidden until its cell is hovered, whatever the window width.** — `projects/ui-kit-v2/src/lib/components/data-table/cell/rt-data-table-cell.component.scss:rt-data-table-cell` — no scenario: a showcase frame is taken without hover and shows the button hidden
