...
- **The consumer's own markup goes into the toolbar by two inputs, and neither of them is the search.** — `projects/ui-kit-v2/src/lib/components/dynamic-list/rt-dynamic-list.directives.ts:RtDynamicListToolbarSelectorsDirective` — два маркера, поле поиска рисует семья. Сценарий `SC-UKV-171`
