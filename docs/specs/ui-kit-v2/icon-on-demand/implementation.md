...
- **A filled material drawing that did not arrive is closed by the outlined one of the same set.** — `projects/ui-kit-v2/src/lib/components/icon/rt-icon.registry.ts:#loadInto` — the filled request falls back to the outlined one, and that one to the own; scenario `SC-UKV-364`
