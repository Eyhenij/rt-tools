...
**Статус:** предложено · **Ревизия:** 2026-08-26 · **Префикс сценариев:** `SC-UKV`
...
Отказов наружу поддомен не отдаёт: отказ загрузки одного имени остаётся внутри реестра и виден
как ненарисованный значок. Строку о нём пишет приложение своими приёмами наблюдаемости, а не
кит.
...
- **Q-1.** Каким местом реестра разметка просит имя — решается при замысле. Обе разметки сегодня
  зовут один метод адреса символа, и запуск загрузки в нём — самый короткий шов, но побочное
  действие в вычислении адреса пришлось бы объявить прямо.

## История изменений

- 2026-08-26 — заведено черновиком по заявке #1182.
