...
- **An input that is boolean only in its declared type is not one.** — The list of what is named apart is `tools/boolean-inputs-allowlist.json`, and the check reads it. One input stands in it: `projects/ui-kit-v2/src/lib/components/button/rt-button.directive.ts:pressed` — pressed, not pressed, not said; scenario `SC-UKV-135`
- **The requirement is held by a check, not by the memory of whoever adds an input.** — `tools/check-boolean-inputs.mjs`, standing in the push gate set and in the pipeline as the step «Boolean inputs of kit two»; scenario `SC-UKV-136`
