<!-- rt-kit v0.14.0 · laws/work-conduct.md · e22225788ebb · правится надстройкой, не здесь -->
...
- **Каждая задача объявленного эпика получает свою карточку в очереди работ сразу, а не в день,
  когда за неё берутся.** Замысел эпика держит порядок для того, кто работу ведёт; очередь
  работ — для того, кто на неё смотрит, и эпик, у которого заведена одна карточка из десяти,
  выглядит там работой на один шаг. Объём, не заведённый карточками, не виден ни владельцу при
  решении, что делать раньше, ни соседней работе, которая берёт задачу и не знает, что та уже
  назначена. Номер карточки при этом встаёт в замысел эпика той же правкой: замысел, который
  называет задачу словами, а не номером, на «взять следующую» отвечает названием, и карточку
  под неё всё равно заводят заново — второй раз и с другим номером.
