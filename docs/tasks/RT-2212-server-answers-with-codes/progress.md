...
- **State:** `этап-идёт`
- **Stage:** 3 из 5 — приёмник отвечает кодом
- **Done:** этапы 1 и 2 — договорённость в `proposed/refusal-codes/` и набор из 30 кодов в
  `message-bus-common`, сценарий SC-MB-413 закрыт
- **Next step:** этап 3 — броски отказа в приёмнике называют код, а не русский текст
- **Uncommitted:** нет
...
- **Кодов вышло 30, а не 25.** К отказам, которые видит человек, добавились пять от проверки
...
- Этап 1 закончен: договорённость написана, проверка описаний зелёная и видит все шесть сценариев
  работы.
- Этап 2 закончен: набор кодов лежит в общей либе — 30 кодов, подстановки по имени, разбор тела
  ответа и таблица русских предложений для дерева. `pnpm exec nx test message-bus-common` — 123
