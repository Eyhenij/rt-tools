...
- **Stage:** 6 из 6 — закрыт
- **Done:** все шесть этапов закрыты — замер записан восьмым разделом замысла эпика, исключения
  перечислены поимённо, `pnpm run check:all` — код 0
- **Next step:** разобрать папку задачи последним коммитом и открыть заявку черновиком
- **Uncommitted:** нет
...
- **Замер встал восьмым разделом замысла эпика, а не девятым.** Задача 11 эпика, RT-1857, не
  влита и своего замера не положила: девятым разделом он оказался бы с дырой на месте восьмого.
  Затронут этап 6.
...
- Этап 5 сделан: двадцать пять замыслов `docs/plans/` и решение `docs/adr/0002-…` переведены;
  ...
  — длиннее предела 0.
- Этап 6 сделан: замер записан восьмым разделом `docs/plans/rules-layer-language.md` —
  документы дерева легче на 28% в байтах, строк с кириллицей 62 против 20 392, и все они
  перечислены поимённо. По ходу нашлось пропущенное этапами 2–4: `docs/specs/README.md`,
  указатель доменов, переведён той же правкой. `pnpm run check:all` — код 0.
