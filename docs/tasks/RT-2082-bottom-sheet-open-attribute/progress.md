...
- **Uncommitted:** нет
- **Waiting for the owner:** да — закрывать ли работу сейчас. Владелец повернул сеанс на сторонний
  заказ: поднять сторибук первого кита в отдельном ворктри. Закрытие своей волей не начато —
  сведение текстов, полный набор проверок и открытый запрос видны снаружи и стоят отдельного хода
- **PR:** ещё не открыт
...
- Сеанс повёрнут на сторонний заказ: сторибук первого кита поднят в отдельном ворктри на порту 6006,
  рабочее дерево не тронуто. Закрытие работы не начиналось — спрошено у владельца
