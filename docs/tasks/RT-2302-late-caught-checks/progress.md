...
- **State:** `этап-идёт`
- **Stage:** 4 of 6 — Narrow stories are shot narrow
- **Done:** stages 1–3 — one refusal names every red light check; the token check reads interpolated scales; the showcase rule names where icon fonts live, and the gate map asks for it
- **Next step:** 4.1 — `Mobile` and `MobileActiveMenu` declare `snapshotViewport`
- **Uncommitted:** no
...
- [ ] 4.3 The snapshot pattern names the narrow-story trap
- [ ] 5.1 Every text names `pnpm run storybook:ui-kit-v1`
- [ ] 5.2 The browser-verification companion says how to find who raised a showcase
- [ ] 6.1 The doc-style override gets a section of its own about consumers
- [ ] 6.2 The layout is checked against the package
