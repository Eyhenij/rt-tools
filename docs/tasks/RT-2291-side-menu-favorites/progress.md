...
- Found on the way: the showcase on 6006 loads the static Material Icons font, which has no `arrows_outward` — the handle there shows the word, measured glyph height 280 px. A consumer that loads its own Material Symbols subset needs `arrows_outward` and `star_border` in it; named to the owner.
