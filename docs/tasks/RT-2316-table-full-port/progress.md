...
- **Done:** задача заведена и привязана к эпику 1870; слово владельца и сверка лежат в
  ...
  Этап 5.1 закрыт: договорённость стала подобластью спеки второго кита с теми же номерами
  сценариев. Спутник написан заново — 90 правил, у каждого место в дереве и сценарий; проверка
  договорённостей зелёная, из 110 расхождений не осталось ни одного. `Q-TP-1` закрыт замыслом
  задачи, который это место и называет. Три долга остались объявленными: 270 — частичное
  покрытие, 275 и 313 — с причинами.
  На этом же этапе общая проверка дерева вскрыла шесть ошибок сборки библиотеки в обеих семьях:
  их не видели ни типовой прогон пакета, ни тесты, и семьи ни разу не собирались библиотекой.
  Условия отбора были объявлены по ключу записи вместо имени свойства колонки, имя свойства
  давало символ и число, шаблон значка списка расходился родовым типом, пункт ряда страниц —
  типом номера. Всё вылечено, сборка зелёная.
- **Next step:** этап 5.2 — ворота перед отправкой и отправка ветки.
- **Uncommitted:** ничего.
...
- [x] 5.1 The agreement merged into the kit spec
