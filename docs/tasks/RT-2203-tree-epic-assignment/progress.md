...
- **Stage:** 3 из 3 — тексты слоя правил
- **Done:** этап 1 сделан: таблица назначений `.claude/rt-kit/assignments.md` в истории, имя копии
  ...
  Этап 3 сделан: поддомен `docs/specs/agent-kit/tree-assignment/` с тремя сценариями и привязками,
  две статьи в правиле `task-flow` и строки в его спутнике
- **Next step:** разобрать папку задачи последним коммитом, отправить ветку, открыть PR
...
- Разложенный `tools/tree-assignment.mjs` форматтер правил на каждом коммите: исходник пакета
  лежит в `.prettierignore`, а разложенная копия — нет, и раскладка после этого краснела. Вылечено
