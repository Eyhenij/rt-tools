...
- **State:** `этап-идёт`
- **Stage:** 5 of 5 — Кадры витрины и слово рядом с компонентом
- **Done:** этап 1 закрыт — тяга идёт указательными событиями с захватом на ручке, перо и палец
  ...
  переехала в своё состояние `SubMenuResize` рядом с ходьбой по клавиатуре. Этап 3 закрыт —
  наружу уходит нарисованное число, а не натянутое. Этап 4 закрыт — ширина меняется с
  клавиатуры, и диктор называет её вместе с обоими пределами.
- **Next step:** пересъём кадров витрины, если ручка изменила вид, и слово в `CONTEXT.md`.
...
- **Этап 3 сделан.** Наружу уходило натянутое число, а нижний предел ширины держит оформление
  потребителя — кит его не знает. Тяга влево за этот предел панель останавливала, а счёт нет:
  потребитель хранил отданное, возвращал его входом и не видел перемены. Теперь по концу тяги
  замеряется сама панель и наружу уходит большее из двух: оформление держит предел максимумом, и
  нарисованное никогда не меньше натянутого. Замер идёт до сброса натянутой ширины — сбросив её,
  панель перерисуют, и мерить будет нечего.
  Проверено: `pnpm exec nx test @rt-tools/ui-kit --testFile=src/lib/ui-kit/side-menu/` — 110 из 110
  зелёные. Линтер кита чист, `npm run check:specs` — код выхода 0.
- **Этап 4 сделан.** Ручка встала в обход табуляцией: стрелки двигают ширину шагом в 16, `Home` и
  `End` ведут к пределам кита. Просьба о ширине уходит наружу на каждое нажатие — тяга держит её до
  конца жеста, потому что жест есть, а у клавиши его нет; событий начала и конца тяги клавиша
  поэтому не рождает. Диктор получил `aria-valuenow`, `aria-valuemin` и `aria-valuemax`; ширины
  потребитель не назвал и ничего не тянули — чисел нет вовсе, кит их не выдумывает.
  Проверено: `pnpm exec nx test @rt-tools/ui-kit --testFile=src/lib/ui-kit/side-menu/` — 116 из 116
  зелёные. Линтер кита чист, `npm run check:specs` — код выхода 0.
