...
- **State:** `этап-идёт`
- **Stage:** 3 of 5 — Наружу уходит то число, которым панель нарисована
- **Done:** этап 1 закрыт — тяга идёт указательными событиями с захватом на ручке, перо и палец
  ...
  в описание домена. Этап 2 закрыт — наружу уходят два события, начало и конец тяги; тяга
  переехала в своё состояние `SubMenuResize` рядом с ходьбой по клавиатуре.
- **Next step:** наружу должно уходить то число, которым панель нарисована.
...
@rt-tools/ui-kit` — чисто. `npm run check:specs` — код выхода 0, SC-UK-69 и SC-UK-70 закрыты
  проверками.
- **Этап 2 сделан.** У меню появились два выхода — `subMenuResizeStart` и `subMenuResizeEnd`.
  Потребитель накрывает по ним кадр чужого адреса и снимает накрытие; прежде ему оставался класс
  ручки, внутреннее дело кита. Конец приходит и на отпускании, и на отнятом средой указателе.
  Состояние тяги уехало в `menu/sub-menu-resize.ts` — тем же приёмом, каким рядом лежит ходьба по
  клавиатуре: компонент снова упёрся в предел длины.
  Проверено: `pnpm exec nx test @rt-tools/ui-kit --testFile=src/lib/ui-kit/side-menu/` — 108 из 108
  зелёные. Линтер кита чист, `npm run check:specs` — код выхода 0.
