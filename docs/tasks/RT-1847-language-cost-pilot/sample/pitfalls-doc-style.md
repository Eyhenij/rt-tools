...
- **A pass over the glossary by eye does not find the word.** "Wording aligned with the
  ...
  pass and stayed on the next line of the same file. A word from the "we do not write" column is
  purged by grep over the whole tree, not by proofreading. The form is set precisely: "spec" — the
