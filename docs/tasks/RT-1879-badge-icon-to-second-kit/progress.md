...
- **Stage:** 5 of 5 закрыт — переход с первого кита написан
...
- **Next step:** слить договорённость в описание домена, догнать тексты, прогнать набор проверок,
  разобрать папку задачи и открыть PR в ветку эпика.
- **Uncommitted:** нет.
...
- **Этап 4 сделан.** В матрицу витрины пришли две оси: ступени размера крест-накрест со значком и
  ...
  **Кадр нашёл то, чего не увидела проба.** Подпись не урезалась при верных правилах стиля: ей не
  ...
  из 589).
- **Этап 5 сделан.** На странице-обзоре метки последним разделом встал переход с первого кита:
  таблица прежних и новых имён — селектор, экспортируемый класс, шесть входов, четыре перечисления,
  два помощника счёта цвета и пять классов, — и шесть пунктов о том, что приложению придётся
  сделать сверх переименования. Все пять видов старого перечисления легли на палитру тега
  поимённо; двум входам — жирной подписи и внешнему признаку нехватки места — соответствия нет, и
  это названо прямо.
  Проверено: `node tools/verify-ui-kit-v2-docs.cjs` — 76 страниц-обзоров без расхождений;
  `check-showcase-links` — 348 связей, мёртвых нет; предел длины файлов расхождений не называет.
