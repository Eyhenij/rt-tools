...
- **State:** `этап-идёт`
- **Stage:** 4 из 5 — показ и чтение
- **Done:** этапы 1–4: поддомен со спекой и восемью сценариями; пятое состояние в трёх наборах, миграция и порядок переходов; колонка причины, её разбор в теле правки и два отказа до сети; столбец причины в двух списках админки и печать причины командой чтения груза
- **Next step:** этап 5 — привязки спутника поддомена, ответ о кодах отказа в спеке и статья правила `cargo-triage`
- **Uncommitted:** нет
