...
августа, — и каждый раз с новым предложением дописать статью. Девять разборов в обоих деревьях
за пять дней описывают один и тот же ход: работа встала там, где должна была продолжиться. В
этом дереве разбор о замысле, написанном ради снятия отказа гарда, повторился в тот же день, что
...
| ---------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------- |
| Ход кончается вместо того, чтобы идти    | 19       | Артефакт шага создан — ветка, папка, колонка, PR, — и ход на этом кончился; работы по шагу нет        |
| Артефакт поставки не в том состоянии     | 12       | PR открыт от вчерашней вершины, без ревьювера, не той записью, влит раньше уборки, ветка воссоздана   |
| Слой правил чинится не там, где сломано  | 7        | Надстройка затёрта записью файла, пакетный текст правлен внутри надстройки, отбитая правка легла мимо |
| Неудобное глушится, а не чинится         | 6        | Проверка заглушена, исключение линтера предложено вместо починки, красное списано на машину           |
| Сказанное владельцу расходится с деревом | 5        | Готовым названо лежащее на диске, снятым — снятое только локально, проверенным — часть набора         |

Пять групп покрывают 49 разборов целиком. Продуктовые дефекты — их два — стоят в группе
