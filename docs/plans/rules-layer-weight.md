...
| №   | Задача                                    | Почему здесь                                                                                      |
| --- | ----------------------------------------- | ------------------------------------------------------------------------------------------------- |
| 1   | Цена входа считается командой             | без числа ни одна следующая задача не докажет выигрыш; та же команда отвечает на вопрос о языке   |
| 2   | Описания скилов обрезаны                  | 41 КБ в каждом заходе без исключения; ни от чего не зависит и даёт больше всех на один заход      |
| 3   | Форма сжатой статьи объявлена             | механизм, на котором стоят четыре следующие задачи: что остаётся в правиле, а что уезжает в холод |
| 4   | Показовое правило сжато целиком           | образец для остальных и первое настоящее число экономии; берётся самое тяжёлое из грузимых        |
| 5   | Крупные правила сжаты по образцу          | основной объём; нужен образец из задачи 4                                                         |
| 6   | Паттерны сжаты по образцу                 | пятьдесят файлов и 454 КБ источника; тот же приём, что и у правил                                 |
| 7   | Законы сжаты                              | 121 КБ; идут после правил — статья закона правится, когда видно, во что она разворачивается ниже  |
| 8   | Словарь и карта хода обрезаны             | 38 КБ на старте каждого захода; независима от прочих                                              |
| 9   | Текст отказа гарда сокращён               | отказ вклеивает перечень статей поверх правила, которое грузится следом; сжатая статья дешевле    |
| 10  | Предел длины правила пересчитан по новому | закрепляет достигнутое: без предела текст отрастает обратно. Раньше покраснеет на несжатом        |
...
ниже числа, назначенного владельцем. Ни одна проверка дерева при этом не краснеет, и перечень
принятого долга по длине пуст.
...
- **Q-1. На каком языке слой окажется после эпика.** Кириллица токенизатору дороже латиницы, и
  владелец разрешил перевод, если он экономит. Задача 1 считает одно правило в двух языках и
  кладёт число на стол; решение о переводе всего слоя — словом владельца после этого числа.
  Перевод необратим по чтению: слой ведёт и читает владелец.
...
- **Q-4. Что делать с проверкой слога, если слой уедет на другой язык.** Она судит русскую
  прозу. Вопрос встаёт только при утвердительном ответе на Q-1 и решается той задачей, которая
  первой уводит текст с русского.
