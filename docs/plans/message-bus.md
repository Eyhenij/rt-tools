...
| договорённость | `docs/specs/message-bus/proposed/rented-node/`        | **не влита** — приёмник по ней работает с конца прошлого месяца |
...
Правила `proposed/rented-node/` дописываются в спек домена, сценарии переезжают с прежними
номерами, привязки проставляются на код, который уже есть. Каталог снимается.

Закрыта, когда `docs/specs/message-bus/proposed/` пуст, `npm run check:specs` не даёт новых
